...
After training the model, a new observation, $\xx^*$, can be fed to the *prediction algorithm*, which is a mathematical strategy that uses the model to make a prediction, $\haty = \hatg(\xx^*)$\index{model predicting}.
...
Let $\{(x_i^*, y^*_i)\}^m_{i=1}$ be data previously unseen by the model trained in Box 1.
