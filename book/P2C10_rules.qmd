...
\end{dcases}
$$
where $r_i, l_i$ are the boundaries on interval-censoring.

These values are inflated as necessary if truncation is present in the data (@sec-surv-obj).
...
## Beyond single-event
<!-- FIXME: ARE THESE BEST REFERENCES? WOULD LIKE ACCESS TO SCHOOP 2011 -->
The most common methods to extend these measures to competing risks is by summing the cause-specific probabilities [@vanGeloven2022, @Lee2018a].
...
where $Y$ is the random variable representing the time-to-event and $E\in \{1,\ldots,k\}$ is the random variable with realisations $e$, which denotes one of $k$ competing events that can occur at event time $Y$.
