...
<!-- TODO: this reads weird. Its not really "testing" if its minimized, it just calculates the difference?-->
...
where now $\calP$ denotes a set of probability distributions over $\PReals$. $L_S$ is *proper* if for any distributions $p_Y, p$ in $\calP$, and for any random variables $Y \sim p_Y$, and $C$ taking values in $\PReals$; with $T := \min(Y,C)$ and $\Delta := \II(T=Y)$; it holds that, $\EE[L_S(p_Y, T, \Delta)] \leq \EE[L_S(p, T, \Delta)]$.
...
When censoring is marginally independent, the Graf score consistently estimates the mean square error [@Gerds2006].
...
$$ {#eq-rcll}
<!-- TODO: \hatf is use don RHS but not input on LHS -->
...
<!-- TODO: ordering not consistent \hatS -> \log(\hatF) ... -->
...
Analogously to the ISBS, the absolute survival loss consistently estimates the mean absolute error when censoring is marginally independent [@Schmid2011], but there are also no proofs or claims of properness.
...
<!-- TODO: sec-surv-models-crank broken in this branch -->
...
Recently, an all-cause logarithmic scoring rule has been proposed which makes use of the IPC weighting in the ISBS [@Alberge2025]:
