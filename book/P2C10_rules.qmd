...
Note that unlike other chapters, there is not a separate section for 'beyond right-censoring' here as the main results all fit within @sec-logloss.
...
In a regression setting, the *integrated Brier score* (IBS), also known as the continuous ranked probability score, is the integral of the Brier score for all real-valued thresholds [@Gneiting2007] and hence allows predictions to be evaluated over multiple points as
...
IPC weights, $W_i$ are defined such that observations are either weighted by $\KMG(\tau)$ when they have not yet experienced the event or by their final observed time, $\KMG(t_i)$, otherwise.
...
Fortunately, as the score is deeply embedded in the literature, experiments have demonstrated that scores generated from using the IGS only differ very slightly to a strictly proper alternative [@Sonabend2022b].

### Logarithmic losses {#sec-logloss}
...
1. If an observation is censored at $t_i$ then they are guaranteed not to have experienced the event up until that time, hence $S(t_i) = 1$; correspondingly (@eq-rcll) becomes $-\log(1) = 0$.
2. If an observation experiences the event at $t_i$ then the likelihood of the event is certain at that time and as such $f(t_i) = \infty$ or $p(t_i) = 1$ in the discrete case; therefore (@eq-rcll) equals $-\infty$ or $0$ for continuous and discrete time respectively.
...
However, to our knowledge it does not appear used in practice and nor is there a practical benefit over other losses.

### Absolute Losses

The final class of losses considered here can be viewed as analogs of the mean absolute error in an uncensored setting.
The absolute survival loss, developed over time by @Schemper2000 and @Schmid2011 is similar to the IGS but removes the squared term:
...
Analogously to the IGS, the absolute survival loss consistently estimates the mean absolute error when censoring is uninformative [@Schmid2011] but there are also no proofs or claims of properness.
The absolute survival loss and IGS tend to yield similar results [@Schmid2011] but in practice the former does not appear to be widely used.
...
For example, instead of evaluating a probabilistic prediction with the IGS over $\NNReals$, one could compute the Brier score at a single time-point, $\tau \in \NNReals$, only.
...
Example prediction error curves are provided in @fig-eval-pecs for the IGS where the the Cox PH consistently outperforms the SVM.
