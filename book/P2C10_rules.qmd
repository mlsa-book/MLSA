...
In the simplest terms, a scoring rule compares two values and assigns them a score (hence 'scoring rule'), formally we'd write $L: \Reals \times \Reals \rightarrow \ExtReals$.
In machine learning, this usually means comparing a prediction for an observation to the ground truth, so $L: \Reals \times \calP \rightarrow \ExtReals$ where $\calP$ is a set of distributions.
...
Analogously to classification losses, a survival loss $L: \calP \times \PReals \times \bset \rightarrow \ExtReals$ is *proper* if for any distributions $p_Y, p$ in $\calP$, and for any random variables $Y \sim p_Y$, and $C$ taking values in $\PReals$; with $T := \min(Y,C)$ and $\Delta := \II(T=Y)$; it holds that, $\EE[L(p_Y, T, \Delta)] \leq \EE[L(p, T, \Delta)]$.
