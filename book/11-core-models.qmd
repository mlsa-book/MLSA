...
# Core Estimators, Models, and Methods {#sec-classical}

In a predictive setting, it can be easy to dismiss survival models that have been around for decades and instead favor testing of more modern 'machine learning' tools.
This would be a mistake.
First, most machine learning survival algorithms build on these models as a central component, for example by using non-parametric estimators (@sec-classical-nonpar) and/or assuming a proportional hazards form (@sec-classical-cox).
For this reason, this book refers to them as core models.
Second, on low-dimensional data (small number of variables), these core models often outperform machine learning models [@Beaulac2020; @Burk2026].
Finally, even on high-dimensional data\index{high-dimensional data}, several papers have demonstrated that augmenting core models (@sec-classical-improving) can yield models that outperform machine learning alternatives [@Spooner2020; @Zhang2021].
...
Finally, the chapter presents core methods (@sec-classical-improving) that are used throughout survival analysis and can improve the predictive performance of core models, including non-linear modeling, feature selection, regularization, and ensemble learning.

## Non-parametric estimators {#sec-classical-nonpar}

Non-parametric estimators\index{non-parametric estimator} have already been introduced in @sec-surv-estimation-non-param, therefore this section is brief and focuses only on how these estimators can be used as predictive models.
...
@fig-p3c13-km-test illustrates the Kaplan-Meier estimator fit on the `rats` [@datarats] dataset.
The left image shows the estimator as a step function, with steps occurring at event times (some examples in green dashed lines).
At censoring times, the estimator stays constant (examples in blue dotted lines).
The right image displays the estimator as a predictive tool.
...
This estimator is the analogue to the sample mode and sample mean baseline estimators\index{baseline model} in classification and regression respectively.
...
![Using the Kaplan-Meier estimator as a predictive tool. The left image shows the estimator fit to data, the green dashed lines are steps in the function at event times and the blue dotted lines are censoring times in the training data and no steps occur. The right image demonstrates using the estimator as a predictive tool by reading off the survival probability at given times.](Figures/classical/fig-p3c13-km-test.png){#fig-p3c13-km-test width=100% fig-align="center" fig-alt="Two graphs both with time on the x-axis and survival probability on the y-axis. Both graphs show the same step function decreasing from S(tau)=1 at tau=0 to around S(tau)=0.8 at around tau=100. In the left graph, three dashed vertical lines mark three steps in the function, two blue lines mark vertical lines without steps. In the right graph, an arrow from t=60 points to the survival probability of around S(tau)=0.98."}
...
The formulation in (@eq-akritas) can be turned into a baseline predictive model by treating estimation of $\hatF_X$ on training data as the learning algorithm, and then using (@eq-akritas) as the prediction algorithm (@sec-ml-models). \index{baseline model}
...
$h_0,H_0,S_0$ are referred to as the baseline hazard, cumulative hazard, and survival function respectively.\index{baseline hazard}\index{baseline survival function}
...
Note how the baseline hazard $h_0$ cancels out in the likelihood contribution and removes the dependency on the exact event time.
...
and the feature-dependent, cause-specific cumulative hazard (@eq-ph-cum) as,\index{cause-specific cumulative hazard function}

$$
H_{q}(\tau \mid \xx) = H_{Bres;q}(\tau)\exp(\xx^\trans \bsbeta_q).
$$ {#eq-cause-specific-cumu-hazard-cox}

A final prediction of the cumulative incidence function\index{cumulative incidence function} follows by (@eq-cif) where the cause-specific hazard function\index{cause-specific hazard function} is the derivative (or a discrete approximation) of (@eq-cause-specific-cumu-hazard-cox) and the all-cause survival probability\index{all-cause survival function} follows by (@eq-all-cause-surv-prob): 
$$
\begin{aligned}
&F_{Bres;q}(\tau \mid \xx) = \\
...
Moreover, even when the subdistribution PH assumption is misspecified, the estimated subdistribution hazard ratio still provides a useful summary of a covariate's effect on the CIF [@Grambauer2010].
...
The dog's survival curve is _accelerated_ by a factor of 5, as shown in the bottom, red curve in @fig-p3c13-dogs.
...
This is shown by the middle, blue curve in @fig-p3c13-dogs.
...
Increasing the hazard ratio to approximately $395$ brings the PH survival curve much closer to the AFT curve, matching its median survival (dashed blue line in @fig-p3c13-dogs).
...
![Comparing human (black) and dog lifespans, with the dog modeled by an AFT model (red, a five-fold acceleration of time) and PH models (blue). The solid blue curve uses a hazard ratio of $5$ (the same numerical factor as the AFT acceleration); the dashed blue curve uses the much larger hazard ratio ($\approx 395$) needed to match the AFT dog's median survival, yet still cannot reproduce its shape. Human lifespan modeled with Gompertz(shape=0.09, rate=0.00005).](Figures/classical/fig-p3c13-dogs.png){#fig-p3c13-dogs width=70% fig-align="center" fig-alt="Survival time tau is on the x-axis (0 to 80) and survival probability S(tau) on the y-axis (0 to 1). All curves start at S(tau)=1. The black curve (human) stays near 1 until about tau=40 then declines gently to about 0.48 at tau=80. The solid blue curve (dog, PH with hazard ratio 5) stays near 1 until about tau=35 then declines, passing 0.5 around tau=60. The red curve (dog, AFT) stays near 1 until about tau=10 then drops steeply between tau=12 and tau=20 to near 0. The dashed blue curve (dog, PH with hazard ratio 395) lies much closer to the red AFT curve, crossing 0.5 near tau=16, but with a more gradual shape: it begins to decline around tau=5 and reaches near 0 only around tau=35."}
...
2. The baseline risk now clearly depends on both the linear predictor as well as time.
3. An increase in a covariate results in a multiplicative effect on survival time, rather than the multiplicative effect on hazard seen in a PH model. This is often viewed as an advantage of AFT models as survival times may be simpler to interpret than hazard ratios.

This third point is visualized in @fig-p3c13-ph-vs-aft in which the linear predictor is increased by $\log(2)$ for a PH model and decreased by $\log(2)$ for an AFT model.
...
![Comparing changes in the linear predictor between PH (left) and AFT (right) models. A linear predictor increase of $\log(2)$ multiplies $h(\tau)$ by $\exp(\log(2)) = 2$ in a PH model. A linear predictor decrease of $\log(2)$ in the AFT results in multiplying $\tau$ within the baseline survival by $2$, hence for any $\tau$, the AFT model reaches $S(\tau)$ in half the time as the baseline.](Figures/classical/fig-p3c13-ph-vs-aft.png){#fig-p3c13-ph-vs-aft width=90% fig-align="center" fig-alt="Two line graphs. Left has 't' on x-axis and 'y' 'h(t)' on y-axis. A blue line (PH model) runs in parallel, vertically above, a black line (baseline). The right graph has 'S(t)' on the y-axis and shows a red line (AFT model) running in parallel but to the left of the baseline. The graph demonstrates how PH acts multiplicatively on the hazard but the AFT acts on the survival function."}
...
For example, @fig-p3c13-loglogistic-hazard visualizes the hazard function of the log-logistic distribution, which can be non-monotonic and allows the baseline risk of an event to increase before decreasing.
...
Although the Royston-Parmar model is not a core model in the same sense as the Cox proportional hazards or accelerated failure time models, it naturally belongs here as it is a straightforward combination of core models and methods (@sec-classical-improving).
...
To fit the model, the number and position of knots are theoretically tunable, although Royston and Parmar advised against tuning and suggest often only one internal knot is required and the placement of the knot does not make a significant difference to performance [@Royston2002]. \index{hyperparameter optimization}
...
## Core methods {#sec-classical-improving}

A number of model-agnostic algorithms have been created to improve a model's predictive ability.
Such extensions of the above models can lead to substantial performance improvements.
These are referred to as 'core methods'.
Like the core models, they are pervasive in the literature and are often found as components within more complex alternatives.
...
Many basis systems can be used, including natural cubic splines, B-splines and the closely related penalized B-splines (P-splines) [@Eilers1996], and thin-plate regression splines [@Wood2003]; simpler choices such as step functions or polynomial bases are also possible (see @Wood2017 for a comprehensive treatment of spline-based modeling).\index{splines}\index{penalization}
...
As $\xi \to 0$, the estimate approaches the unpenalized fit, whereas a large $\xi$ forces each $f_j$ towards a flatter function.
...
In many high-dimensional predictive modeling problems, only a small subset of variables in datasets tend to be relevant for correctly predicting the outcome.
...
Lasso and ridge are both shrinkage methods, which are used to reduce model variance and overfitting, especially in the context of multicollinearity.
...
Deciding between lasso and ridge can also be automated by using elastic net [@Simon2011; @Zou2005], which is a convex combination of $\ell^1$ and $\ell^2$ penalties such that $\bsbeta$ is estimated as: \index{Cox proportional hazards!elastic net}
...
Care is needed when fitting ensembles to avoid data leakage.
In particular, stacking should use predictions from base models that were not trained on the same observations used to fit the meta-model.
...
The average over predictions could be uniform, weighted with weights selected through expert knowledge, or weighted with weights optimized as hyperparameters.
...
Three Cox base models are fit on $\calD_1$, each using a different feature set so that the base models are distinct, and linear predictors are predicted for all $m$ observations in $\calD_2$:
...
* Core statistical models broadly fall into three categories: non-parametric estimators, semi-parametric models that include a non-parametric baseline estimate, and fully-parametric models.
...
* @Collett2014 and @Kalbfleisch1980 both provide comprehensive reading for core statistical models. The former is slightly less technical and covers extensions to multiple settings.
