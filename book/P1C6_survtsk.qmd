...
<!-- TODO (Raphael): Careful with the absolute risk below. Its a new term and has not been introduced in contrast to "relative risk" before. The statement "distribution = absolut risk" is too simplistic. Absolut risk = cumulative at a specific time point, thus F(t), S(t) = absolut risk at t, but h(t) is relative risk at t, but h(t) is a distribution defining quantitiy. -->
...
<!-- TODO (Raphael): survival times are not generally difficult to interpret — they only become so when extrapolation beyond the observation window is involved. The current wording also contradicts the Limitations callout, which says survival time predictions "appear intuitive". Reconcile these two statements. -->
...
@fig-survtsk-overview illustrates the different information provided by the different prediction types based on a Weibull accelerated failure time model (@sec-surv-models-param) . 
The prognostic index is omitted as it would visually look the same as the relative risk prediction type.
Top-left panel shows the tabular survival data using the six patients from the `tumor` data set (@tbl-surv-data-tumor).
The top-right panel shows predicted survival times, for each observation this is a single scalar value representing the estimated event time (here, the predicted expected survival time after operation $\hat{E}[T \mid \xx]$ with its 95% confidence interval).
...
Finally, the bottom-right panel shows survival distribution predictions, where each observation's predicted survival function $\hat{S}(\tau \mid \xx)$ is shown over time.
...
<!-- TODO (Raphael): I wouldn't say so. If I fit a parametric model (or piece-wise exponential), I will make continuous predictions. For plotting of course you choose a discrete grid, but the predictions themselves are continuous. -->
...
<!-- TODO (Raphael): the switch between $t$ and $\tau$ is a bit confusing here — both are used as concrete time-axis values in the same example. Consider using a single symbol consistently. -->
...
<!-- TODO (Raphael): we have to be careful here. why should it be possible to estimate sojourn times but not event times, where the latter is a special case of the former. -->
...
However, this rule is often unusable in survival datasets as censoring can lead to estimated survival distributions that are _improper_ (@sec-distributions-continuous).
...
<!-- TODO (Raphael): the PI definition is currently tied to a *linear* predictor (β'x), but a prognostic index is more general — any scalar function η(x) that summarizes risk. Linear predictors are the classical case (Cox / parametric models), but ML survival models (DeepSurv, gradient boosting, RSF mortality scores) produce nonlinear η(x) that play the same role. Suggestion: broaden the definition to a generic scalar score and drop "linear" in front of "predictor" here. the sentence about g(x)=x -> g(bsbeta) also doesnt parse for me. g(x) doesn't return a scalar x if x is multi-variate. the prognostic index = g(x) which is x^T\beta if g = linear combination of x -->
