---
abstract: "This chapter introduces the different survival problems and formalises them as survival tasks. There are four prediction types in survival analysis: relative risks - predicting the risk of an event compared to others in the sample, survival times - predicting the time until an event happens, prognostic index - predicting a linear predictor to assess outcomes based on risk factors, and survival distributions - predicting the probability of an event taking place over time. These reduce to three formal survival tasks: deterministic (survival time), ranking (risks and prognostic index), and probabilistic (distribution). This separation of tasks helps create a taxonomy of survival models and losses that is used throughout the book." 
---
...
Recall from @sec-ml-tasks that a machine learning task is the specification of a mathematical problem that is solved by a given algorithm.
A task, $f: \calX \rightarrow \calY$ has three components, a description of the input data $\calX$, a description of the expected output (target) $\calY$, and a description of the estimation function to be learnt $f$.
The previous two chapters described handling of the input data and Part III will describe the various different algorithms that can be used for $f$.
This chapter will tackle the final part of the survival task definition, which is describing the codomain $\calY$.
...
In analogy to regression tasks, survival distribution predictions are _probabilistic_ (prediction of probability over time) and survival time predictions are _deterministic_ (single point prediction).
Relative risk and prognostic index predictions do not directly predict when or if the event of interest will occur, but instead provide different interpretations that relate to the risk of the event compared to others in the same sample.
The relative risks and time-to-event prediction types can both be derived from a survival distribution prediction, and the prognostic index is often used as an interim step before making a probabilistic prediction (some examples of this in @sec-classical); so all of these prediction types are closely related.

Despite their close nature, it is vital to clearly separate which prediction problem you are working with, as they are not directly compatible with one another.
...
Practically, the complexity of interpreting survival time and prognostic index predictions mean they are not generally used in practice for predictive models.
Instead, it is more common to use probability distribution predictions when there is a need to estimate the event over time (e.g., "$\tau$-year survival probabilities"), whereas relative risk predictions are more common for resource allocation and risk comparison.

@fig-survtsk-overview conveys the different information provided by the first three prediction types; as will be seen shortly, the prognostic index is very closely related to relative risk predictions.
An example of tabular survival data is in the top-left panel, the remaining three show the different predictions that could be made for each of the five observations.
The top-right panel shows predicted survival times, for each observation this is a single scalar value representing the estimate time the event will take place.
The bottom-left panel are relative risk predictions, each subject's risk is centered around some baseline observation (one in which all covariates are zero) and the direction and magnitude of their risk scores can be compared.
Finally, the bottom-right panel shows survival distribution predictions, where each observations survival probability is estimated over time.

![Illustration of prediction types. Tabular survival data (top-left) can be used to make three distinct prediction types, each conveying different information. Predicted survival times (top-right) provide a single number estimating the time until an event takes place. Predicted relative risk scores (bottom-left) compare the risk of event between subjects within the same sample. Predicted survival distribution (bottom-right) estimate the probability of the event taking place over time.](Figures/survtsk/predict_types.png){#fig-survtsk-overview width=100% fig-alt="Graphical illustration of prediction types. Top-left panel is an example of survival data in a table. Top-right panel is horizontal bars for each subject in the data ranging from 0 to approximately their real survival time. Bottom-left panel is vertical bars ranging from -4 to +8, indicating different risk profiles for each subject. Bottom-right panel is step functions, one for each subject, illustrating individual survival function probabilities decreasing over time."}

Whilst one prediction type is necessary to define a model's output, it is often the case that an algorithm might use another prediction type as an interim prediction, this will be seen throughout Part III.
Under some assumptions, prediction types can also be converted between each other, this will be discussed in the rest of this chapter.
...
In the competing risks setting this would instead be $g: \calX \rightarrow \calS^q$ when predicting $q$ cause-specific distributions or $g: \calX \rightarrow [0,1]^{q\times q}$ for more general multi-state models.
...
Predicting relative risks is defined as predicting a continuous rank for an individual's relative risk of experiencing the event.
...
A potential confusion that should be avoided is conflating a relative risk prediction with a prediction of absolute risk.
Relative risks means the risks are only comparable to other observations within the same sample.
...
Estimation and interpretation of risks in the competing risks setting follows analogously.
Though one must be cautious whether predicting and interpreting risks for each of the $q$ causes, $g: \calX \rightarrow \Reals^q$, or a single all-cause risk.
Risks can be derived directly or using one of the transformation methods listed below.
For multi-state models, predictions of risks is not clearly defined.
...
However, evaluation of these predictions is tricky and generally ill-advised.
...
In the competing risks setting, 'survival time' is ill-defined as it is ambiguous whether this refers to the time until a specific event or until any event takes place.
For multi-state models, there is no single notion of a 'survival time'
One could estimate the sojourn time, the expected time spent in a given state, using the estimated transition probabilities.
...
* For more dynamic survival tasks that update predictions over time see @VanHouwelingen2007;
* For more academic discussion about different ways to transform linear predictors to survival distributions and their relationships, see @KalbfleischPrentice1973.
