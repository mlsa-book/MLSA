...
Given covariates, $\XX \in \Reals^{n \times p}$, and a vector of model coefficients, $\bsbeta \in \Reals^p$, the linear predictor is defined by $\bseta:= \XX^\trans\bsbeta \in \Reals^n$.
