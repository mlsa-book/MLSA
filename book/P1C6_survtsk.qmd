...
2. Ensuring meaningful results: for example the Cox PH model (@sec-classical) applies the transformation $g(\bseta) = \exp(\bseta)$, which guarantees positivity of hazards as well as making the covariates multiplicative;
...
However, there are cases where there may be a clear relationship between the two (an example for accelerated failure time models given in @sec-surv-models-param).
...
This will be seen in detail in future chapters (particularly in @sec-boost and @sec-nnet) but in general it is very common for models to assume data follows either proportional hazards or accelerated failure time (@sec-classical).
In this case models are formed of two components:

1. A 'simple' model that can estimate the baseline hazard, $\hatbh$, or baseline survival function, $\hatbS$. Discussed further in @sec-classical, these are group-wise estimates that apply to the entire sample.
