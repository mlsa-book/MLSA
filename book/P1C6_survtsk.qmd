...
Let $(\XX,\tt,\bsdelta)$ and $\calD$ be as defined in @sec-surv-set-math.
...
Given covariates, $\XX \in \Reals^{n \times p}$, and a vector of model coefficients, $\bsbeta \in \Reals^p$, the linear predictor is defined by $\boldsymbol{\eta}:= \XX^\trans\bsbeta \in \Reals^n$.
...
Let $\calS \subseteq \Distr(\calT)$ be a convex set of distributions on $\calT$ and let $\calR \subseteq \Reals$.
Then,
...
Any other survival prediction type falls within one of the tasks in the box above, for example predicting log-survival time is the deterministic task and predicting prognostic index or linear predictor is the ranking task.
