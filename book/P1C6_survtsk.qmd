...
One _can_ compute $\RMST(6)$ to provide an interpretable lower bound on the mean survival time\index{mean survival time}: $\RMST(6) \leq \EE[Y]$.
This results in a meaningful statement such as: "the average survival time is at least $\RMST(6)$" (@fig-survtsk-rmst).
...
In contrast, over the six-year interval, group $i$ is expected to remain event-free for approximately $0.75$ years ($9$ months) longer than group $j$.
Computing the RMST at these two thresholds provides interpretable summaries: the treatment offers no short-term benefit within three years, and may even be associated with short-term harm, but confers a meaningful advantage over a longer six-year horizon. 
...
In this illustrative example, group $j$'s survival probability drops sharply near the end of the displayed follow-up; this extreme behavior is included only to make the effect of the truncation point visually clear.
Because the RMST at $\tau=6$ only integrates the survival curve up to six years, survival differences after this point do not affect the summary.
...
where $\hatg:\calX \rightarrow \Reals$ maps covariates to a scalar model output, and $\psi$ is a transformation chosen so that the resulting quantity has the desired scale, sign, and interpretation.
...
For more flexible machine learning models, $\hatg$ may be non-linear and need not have an explicit linear-predictor form.
