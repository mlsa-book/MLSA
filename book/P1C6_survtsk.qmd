...
<!-- FIXME (RAPHAEL): the two list items below are both numbered "1." — should this enumeration be "1." and "2."? (the text says "two broad types of conclusion"). -->
...
1. Conclusions comparing risk groups:
...
<!-- FIXME (RAPHAEL): the $\hatH$ values below do not match the $\hatS$ values under $\hatH=-\log\hatS$ (the definition just above). E.g. $-\log(0.8)=0.22$ (not 0.10), $-\log(0.4)=0.92$ (not 0.40), $-\log(0.15)=1.90$ (not 0.82); for j: $-\log(0.6)=0.51$, $-\log(0.4)=0.92$, $-\log(0.35)=1.05$. Correct values: $\hatH_i=(0,0.22,0.92,1.90)$, $\hatH_j=(0,0.51,0.92,1.05)$, with sums 3.04 and 2.48 and ratio ~1.23 (so the "~1.2x higher" conclusion still holds, but the H values and the sums 1.32/1.08 below need updating). -->
...
There is no method to know if the event would have occurred at $\tau=6$ or $\tau=600$; all that can be known is that the event did not occur before $\tau=5$.
...
Moving from a survival time to a distribution prediction\index{survival distribution} is uncommon: given the availability of models that directly predict distributions and the difficulty of evaluating survival time predictions, there is little reason to do so.
It is nonetheless _possible_, as in regression settings, to construct a distribution around a central estimate $\hatt$ by assuming a parametric form --- for example $\operatorname{TruncatedNormal}(\hatt, \sigma, a=0, b=\infty)$, with $\sigma$ an assumed or estimated standard deviation --- but such constructions rely on strong assumptions and are rarely used in practice.
...
<!-- TODO (RAPHAEL): this RMST example/figure is not ideal: (1) the survival curve dropping to zero is unrealistic; (2) framing it as a comparison of two individual patients is suboptimal. Better to use a realistic example with two groups (e.g. treatment yes/no), perhaps from the tumor data (complications?). Also: use theme_bw() for the figure. -->
...
<!-- FIXME (RAPHAEL): I still don't get why this has to be a linear model. Why can't $\eta$ be produced by any ML model, i.e. $\eta = g(x)$, rather than restricting to the linear predictor $\XX\bsbeta$? -->
The term is primarily useful when used specifically in reference to simple linear models.
In these cases, a prognostic index is any scalar function $\psi$ applied to the _linear predictor_\index{linear predictor},  $\bseta := \XX\bsbeta$ where $\XX \in \Reals^{n \times p}$ are covariates and $\bsbeta \in \Reals^p$ are model coefficients.
...
For example, models introduced in @sec-svm and @sec-boost directly predict risk scores that cannot be expressed as transformations of covariates\index{gradient boosting machines}\index{support vector machines}.
...
where $S_0(\tau)$ is known as the baseline survival function\index{baseline survival function}, $\eta_i$ is the individual's linear predictor\index{linear predictor} and $\exp(\eta_i)$ serves as a prognostic index\index{prognostic index}.

<!-- FIXME (RAPHAEL): to my previous point, this is exactly the case where $\eta_i$ could be replaced by a boosting or neural-network predictor (i.e. $\eta_i = g(\xx_i)$) before the Breslow estimator is applied — so it need not be a linear predictor. -->
...
<!-- FIXME (RAPHAEL): why do we need the Van Houwelingen dynamic-prediction reference here? Dynamic prediction isn't discussed in this chapter — does it belong in the further reading for the survival task? -->
