...
Predicting a survival distribution means estimating the probability of an individual's event time over the interval from time-point $0$ to $\infty$.
...
In the event-history analysis setting, a single 'survival time' is ill-defined unless the event or transition of interest is explicitly specified, as it may refer either to the time until a particular event or transition, or more generally to the time until any event occurs.
...
In these cases, a prognostic index is simply any scalar function $g$ applied to the _linear predictor_,  $\bseta := \XX\bsbeta$ where $\XX \in \Reals^{n \times p}$ are covariates and $\bsbeta \in \Reals^p$ are model coefficients.
