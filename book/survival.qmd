...
Here we briefly introduce the Kaplan-Meier (KM) Estimator for the survival function (@eq-surv-haz), which is a popular baseline model when evaluating the predictive performance of more complex methods. In machine learning terms it can be viewed as a "featureless" learner analogon for survival analysis.

The calculation is simple, based on the quantities introduced in the previous section:

$$\hat{S}_{KM}(\tau) = \prod_{k:t_{(k)} \leq \tau}\left(1-\frac{d_{t_{(k)}}}{n_{t_{(k)}}}\right),$$ {#eq-km}
which is a step-function at the  obseved ordered event times $t_{(k)}, k=1,\ldots,m$ with $\hat{S}_{KM}(\tau) = 1\ \forall \tau < t_{(1)}$.

Sometimes the KM Estimator is also used to estimate the distribution of censoring times, for example to calculate inverse probability of censoring weights (IPCW).
We will denote the KM Estimator applied to the observed censoring times $(t_i, 1-\delta_i)$ as $\hat{G}_{KM}$.
...
Sometimes, right-censoring is also subdivided into Type-I, Type-II and random censoring.
...
Random censoring occurs when censoring times *randomly* follow an unknown distribution and we observe $(T_i=\min(Y_i,C_i), \II(Y_i\leq C_i))$.
...
As example, consider a study from the 18th century (@brostrom.influence.1987), when childhood and maternal mortality were relatively high.
The goal of the study was to quantive the effect of a mother's death on the survival of the infant.
Since each death was reported to the authorities, an infant was added to the study if and when their mother died, then, two other infants, whose mothers were alive, were matched into the study based on their age and other relevant features.
Thus, 3 infants always had identical featurs except for the status of the mother (alive or dead).
Notably, because of the study design, infants who died before their mothers could never enter into the study. A mother's death is thus refered to as left-truncation event and the infant's age at time of inclusion into the study is refered to as left-truncation time.

More formally, let $T^L_i$ the subject-specific left-truncation time.
Then we only observe subjects with $Y_i > T^L_i$.
Subjects with $Y_i < T^L_i$ never enter the data.

This is also illustrated in @fig-left-truncation: Staying with the example above, infant 1 dies at $T_1$, while the mother would die at some later timepoint $T_1^L$, which means this subject would never enter the study. The mother of infant 2 dies at $T_2^L$, at which point the infant is included in the study and experiences an event at $T_2$. Similarly, infant 3 enters the study at $T_3^L$ and is censored at 365 days, since the event occurs at some time after the sutdy end ($T_3>365$).
