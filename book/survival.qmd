...
\ref{tab:surv_data_rats} exemplifies an observed survival dataset with a modified version of the \code{rats} dataset  [@pkgsurvival].
...
: \code{rats}  [@pkgsurvival] time-to-event dataset with added hypothetical columns ($Y,C$). Rows are individual observations, $X$ columns are features, $T$ is observed time-to-event, $\Delta$ is the censoring indicator, and $(Y,C)$ are hypothetical (here arbitrary values dependent on $(T,\Delta)$) true survival and censoring times. {#tbl-surv-data-rats}
