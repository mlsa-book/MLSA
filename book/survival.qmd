...
In their broadest and most basic definitions, survival analysis is the study of temporal data from a given origin until the occurrence of one or more events or `end-points'~\cite{Collett2014}, and machine learning is the study of models and algorithms that learn from data in order to make predictions or find patterns~\cite{Hastie2001}. Reducing either field to these definitions is ill-advised.

This chapter collects terminology utilised in survival analysis \see{sec:surv_set} and machine learning \see{sec:surv_setml} in order that this thesis can cleanly discuss `machine learning survival analysis' \see{sec:surv_setmltask}. Once the mathematical setting is set up, the thesis scope is fully presented in \cref{sec:surv_scope}. Whilst the content of this chapter is not novel with respect to either survival analysis or machine learning separately, this does appear to be the first formulation of the survival analysis machine learning `task'~\cite{Kiraly2021}.
...
Some of the biggest challenges in survival analysis stem from an unclear definition of a `survival analysis prediction' and different (sometimes conflicting) common notations. This thesis attempts to make discussions around survival analysis clearer and more precise by first describing the mathematical setting for survival analysis in \cref{sec:surv_set_math} and only then defining the prediction types to consider in \cref{sec:surv_set_types}.
...
\begin{itemize}
\item $X \ \tvi \ \calX \subseteq \Reals^p, p \in \PNaturals$ be the generative random variable  representing the data \emph{features}/\emph{covariates}/\emph{independent variables}.
\item $Y \ \tvi \ \calT \subseteq \NNReals$ be the (unobservable) \emph{true survival time}.
\item $C \ \tvi \ \calT \subseteq \NNReals$ be the (unobservable) \emph{true censoring time}.
\end{itemize}
...
\begin{itemize}
\item $T := \min\{Y,C\}$ be the \emph{observed outcome time}.
\item $\Delta := \II(Y = T) = \II(Y \leq C)$ be the \emph{survival indicator} (also known as the \emph{censoring} or \emph{event} indicator).\footnote{Indicators are usually named to reflect a positive condition in the function (in this case the event when $Y = T$), but counter to this convention the `censoring indicator' is possibly the most common term.}
\end{itemize}

Together $(T,\Delta)$ is referred to as the \emph{survival outcome} or \emph{survival tuple} and they form the dependent variables. The survival outcome provides a concise mechanism for representing the time of the \emph{observed} outcome and indicating which outcome (death or censoring) took place.

Now the full generative template for survival analysis is given by \\ $(X, \Delta, C, Y, T) \ \tvi \ \calX \times \bset \times \calT \times \calT \times \calT$ and with $(X_i, \Delta_i, C_i, Y_i, T_i)$ jointly i.i.d. A \emph{survival dataset} is defined by $\calD = \{(X_1,T_1,\Delta_1),...,(X_n,T_n,\Delta_n)\}$ where $(X_i,T_i,\Delta_i) \iid (X,T,\Delta)$ and $X_i$ is a $p$-vector, $X_i = (X_{i;1},...,X_{i;p})$. Though unobservable, the true outcome times are defined by $(Y_1,C_1),...,(Y_n,C_n)$ where $(Y_i,C_i) \iid (Y,C)$.

\Cref{tab:surv_data_abs} exemplifies a random survival dataset with $n$ observations (rows) and $p$ features.

\begin{table}[H]
\caption[Theoretical time-to-event dataset]{Theoretical time-to-event dataset. $(Y,C)$ are `hypothetical' as they can never be directly observed. Rows are individual observations, $X$ columns are features, $T$ is observed time-to-event, $\Delta$ is the censoring indicator, and $(Y,C)$ are hypothetical true survival and censoring times.}\label{tab:surv_data_abs}
\centering
\begin{tabular}{|c|c|c||c|c|||c|c|}
\toprule
\multicolumn{3}{|c||}{$X$} & $T$ & $\Delta$ & $Y$ & $C$ \\
\hline
$X_{11}$ & $\cdots$ & $X_{1p}$ & $T_1$ & $\Delta_1$ & $Y_1$ & $C_1$ \\
$\vdots$ & $\ddots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ \\
$X_{n1}$ & $\cdots$ & $X_{np}$ & $T_n$ & $\Delta_n$ & $Y_n$ & $C_n$ \\ \bottomrule
\end{tabular}
\end{table}

\Cref{tab:surv_data_rats} exemplifies an observed survival dataset with a modified version of the \code{rats} dataset~\cite{pkgsurvival}.

\begin{table}[H]
\caption[\code{rats} time-to-event dataset]{\code{rats}~\cite{pkgsurvival} time-to-event dataset with added hypothetical columns ($Y,C$). Rows are individual observations, $X$ columns are features, $T$ is observed time-to-event, $\Delta$ is the censoring indicator, and $(Y,C)$ are hypothetical (here arbitrary values dependent on $(T,\Delta)$) true survival and censoring times.}\label{tab:surv_data_rats}
\centering
\begin{tabular}{|c|c|c||c|c|||c|c|}
\toprule
\multicolumn{3}{|c||}{$X$} & $T$ & $\Delta$ & $Y$ & $C$ \\
\hline
\textbf{litter} $(X_{.;1})$ & \textbf{rx} $(X_{.;2})$ & \textbf{sexF} $(X_{.;3})$ & \textbf{time} & \textbf{status} & \textbf{survTime} & \textbf{censTime} \\ \hline
1 & 1 & 1 & 101 & 0 & 105 & 101  \\
1 & 0 & 1 & 49 & 1 & 49 & 55\\
1 & 0 & 1 & 104 & 0 & 200 & 104 \\
2 & 1 & 0 & 91 & 0 & 92 & 91 \\
2 & 0 & 0 & 104 & 1 & 104 & 104 \\
2 & 0 & 0 & 102 & 1 & 102 & 120 \\ \bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[i)]
\item The \emph{set of unique} or \emph{distinct time-points} refers to the set of time-points in which at least one observation dies or is censored, $\calU_O := \{T_i\}_{i \in \{1,...,n\}}$.
\item The \emph{set of unique death times} refers to the set of unique time-points in which death (and not censoring) occurred, $\calU_D := \{T_i : \Delta_i = 1\}_{i \in \{1,...,n\}}$.
\item The \emph{risk set} at a given time-point, $\tau$, is the set of subjects who are known to be alive (not dead or censored) just before that time, $\calR_\tau := \{i: T_i \geq \tau\}$ where $i$ is a unique row/subject in the data.
\item The \emph{number of observations alive} at $\tau$ is the cardinality of the risk set, $|\calR_\tau|$, and is denoted by $n_\tau := \sum_i \II(T_i \geq \tau)$.
\item The \emph{number of observations who die} at $\tau$ is denoted by $d_\tau := \sum_i \II(T_i = \tau, \Delta_i = 1)$.
\item The Kaplan-Meier estimate of the average survival function of the training data \emph{survival distribution} is the Kaplan-Meier estimator \see{sec:surv_models_uncond} fit \see{sec:surv_setml_meth} on training data $(T_i, \Delta_i)$ and is denoted by $\KMS$.
\item The Kaplan-Meier estimate of the average survival function of the training data \emph{censoring distribution} is the Kaplan-Meier estimator fit on training data $(T_i, 1 - \Delta_i)$ and is denoted by $\KMG$.
\end{enumerate}
...
\noindent \paragraph{Censoring `Location'}
...
Left-censoring is a rarer form of censoring and occurs when the event happens at some unknown time before the study start, $Y < \tau_l$. Interval-censoring occurs when the event takes place in some interval within the study period, but the exact time of event is unknown. \Cref{fig:survset_censor} shows a graphical representation of right-censoring.

\begin{figure}
\centering
\includegraphics[width=14cm,height=8cm]{c2_set/censoring}
\caption[Dead and censored subjects over time]{Dead and censored subjects (y-axis) over time (x-axis). Black diamonds indicate true death times and white circles indicate censoring times. Vertical line is the study end time. Subjects 1 and 2 die in the study time. Subject 3 is censored in the study and (unknown) dies within the study time. Subject 4 is censored in the study and (unknown) dies after the study. Subject 5 dies after the end of the study.}
\label{fig:survset_censor}
\end{figure}

\paragraph{Censoring `Dependence'}
Censoring is often defined as \emph{uninformative} if $Y \indep C$ and \emph{informative} otherwise however these definitions can be misleading as the term `uninformative' appears to be imply that censoring is independent of both $X$ and $Y$, and not just $Y$. Instead the following more precise definitions are used in this report.

\begin{definition}
\label{def:cens}
Let $(X,T,\Delta,Y,C)$ be defined as above, then
\begin{enumerate}[i)]
\item If $C \indep X$, censoring is \emph{feature-independent}, otherwise censoring is \emph{feature-dependent}.
\item If $C \indep Y$, then censoring is \emph{event-independent}, otherwise censoring is \emph{event-dependent}.
\item If $(C \indep Y) | X$, censoring is conditionally independent of the event given covariates, or \emph{conditionally event-independent}.
\item If $C \indep (X,Y)$ censoring is \emph{uninformative}, otherwise censoring is \emph{informative}.
\end{enumerate}
\end{definition}
...
\begin{enumerate}[i)]
\item Let $p$ be the proportion of censored observations in the data, then $p \in (0,1)$. This open interval prevents the case when $p = 0$, which is simply a regression problem \see{sec:surv_ml_regr}, or the case when $p = 1$, in which no useful models exist (as the event never occurs).
...
\item There is only one event of interest, an observation that does not experience this event is censored. This eliminates the `competing risk' setting in which multiple events of interest can be modelled.
\item The event can happen at most once. For example the event could be death or initial diagnosis of a disease however cannot be recurrent such as seizure. In the case where the event could theoretically happen multiple times, only the time to one (usually the first) occurrence of the event is modelled.
\item The event is guaranteed to happen at least once. This is an assumption implicitly made by all survival models as predictions are for the time until the true event, $Y$, and not the observed outcome, $T$.
\end{enumerate}
...
\begin{lsti}
\item a survival dataset, $\calD$, is split \see{sec:surv_setml_meth} for training, $\dtrain$, and testing, $\dtest$;
\item a survival model is fit on $\dtrain$;
\atem the model predicts some representation of the unknown true survival time, $Y$, given $\dtest$.
\end{lsti}

The process of `fitting' is model-dependent, and can range from simple maximum likelihood estimation of model coefficients, to complex algorithms.  The model fitting process is discussed in more abstract detail in \cref{sec:surv_setml} and then concrete algorithms are discussed in \cref{chap:review}. The different survival problems are separated by `prediction types' or `prediction problems', these can also be thought of as predictions of different `representations' of $Y$. Four prediction types are discussed in this paper, these may be the only possible survival prediction types and are certainly the most common as identified in chapters \ref{chap:review} and \ref{chap:eval}.  They are predicting:

\begin{enumerate}[i)]
\item The \emph{relative risk} of an individual experiencing an event -- A single continuous ranking.
\item The \emph{time until an event} occurs -- A single continuous value.
\item The \emph{prognostic index} for a model -- A single continuous value.
\item An individual's \emph{survival distribution} -- A probability distribution.
\end{enumerate}

The first three of these are referred to as \emph{deterministic} problems as they predict a single value whereas the fourth is \emph{probabilistic} and returns a full survival distribution. Definitions of these are expanded on below.
\\\\
...
\begin{enumerate}[i)]
\item Conclusions comparing patients. e.g. $i$ is at the least risk; the risk of $j$ is only slightly higher than that of $i$ but the risk of $k$ is considerably higher than $j$; the corresponding ranks for $i,j,k,$ are $1,2,3$.
\item Conclusions comparing risk groups. e.g. thresholding risks at $1.0$ places $i$ and $j$ in a `low-risk' group and $k$ in a `high-risk' group
\end{enumerate}
So whilst many important conclusions can be drawn from these predictions, the values themselves have no meaning when not compared to other individuals. Interpretation of these rankings has historically been conflicting in implementation, with some software having the interpretation `higher ranking implies higher risk' whereas others may indicate `higher ranking implies lower risk' \see{sec:tools_mlr3proba_api_learn}. In this thesis, a higher ranking will always imply a higher risk of event (as in the example above).
