...
Here we briefly introduce the Kaplan-Meier (KM) Estimator for the survival function (@eq-surv-haz), which is a popular baseline model when evaluating the predictive performance of more complex methods. In machine learning terms it can be viewed as a "featureless" learner for survival analysis.

Using the quantities just introduced, the calculation becomes simple:
