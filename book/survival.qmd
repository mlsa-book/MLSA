...
@fig-km-tumor shows the estimated survival probablity obtained by applying the Kaplan-Meier estimator to the `tumor` data. By definition, the survival function starts at $S(t)=1$ at $t=0$ and monotonically decreases towards $S(t)=0$ at $t\rightarrow \infty$. As in practice, the follow-up time is limited, many subjects have often not experienced the event of interest at the end of the follow-up. For non-parametric methods like the KM, therefore, the survival function usually does not reach $S(t)=0$. We then speak of an *improper* distribution in such cases. For the same reason, the expected value of the distribution, that is the expected survival time, can not be calculated. Sometimes the median survival time is used instead, defined as the time at which the survival function reaches $S(t) = 0.5$. In this example, the median survival time is approximately 1500 days, as indicated by the dashed lines. This means that 50% of the subjects are expected to have died within 1500 days after operation.

Note, however, that the median survival time is not always defined. For example, consider @fig-km-tumor-age-bin. Here, the median surival time for subjects aged 50 or higher is approximately 1400 days, while the median survival time for subjects below 50 years of age doesn't exist, as the survival function is always above the 0.5 threshold, that is more than 50% of the subjects in the younger age group were still alive at the end of the follow-up.
...
- left-truncation: $f(Y_i = t_i|Y_i \geq t_i^L, \bstheta) = \frac{f_Y(t_i|\bstheta)}{S_Y(t_i^L|\bstheta)}$
- right-truncation: $f(Y_i = t_i|Y_i \leq t_i^R, \bstheta) = \frac{f_Y(t_i|\bstheta)}{F_Y(t_i^R|\bstheta)}$
