...
In this section, we introduce different functions that represent a distribution of a random variable and particularly the ones that are particularly important in survival analysis.

For now, assume a continuous, positive, random variable $Y$ t.v.i. $\NNReals$. A standard representation of the distribution of $Y$ is given by the probability density function (pdf), $f_Y: \NNReals \rightarrow \NNReals$, and cumulative distribution function (cdf), $F_Y: \NNReals \rightarrow [0,1]; (\tau) \mapsto P(Y \leq \tau)$.

For reasons that will become apparent later and partly for historical reasons, in survival analysis, it is more common to describe the distribution of event times $Y$ via the so called *survival function* and *hazard rate* (or *hazard function*).

The survival function is defined as
$$
S_Y: \NNReals \rightarrow [0,1]; \quad
(\tau) \mapsto P(Y \geq \tau) = \int^\infty_\tau f_Y(u) \ du
$$
and so $S_Y(\tau) = 1-F_Y(\tau)$.
This function is known as the survival function as it can be interpreted as the probability that a given individual survives until some point $\tau \geq 0$.
...
$$
The hazard function is interpreted as the instantaneous risk of death given that the observation has survived up until that point; note this is not a probability as $h_Y$ can be greater than one.
...
However, in the presence of censoring $C$, it is impossible to fully observe both $Y$.
...
A *survival dataset* is defined by $\calD = \{(X_1,T_1,\Delta_1),...,(X_n,T_n,\Delta_n)\}$ where $(X_i,T_i,\Delta_i) \iid (X,T,\Delta)$ and $X_i$ is a $p$-vector, $X_i = (X_{i;1},...,X_{i;p})$.
Though unobservable, the true outcome times are defined by $(Y_1,C_1),...,(Y_n,C_n)$ where $(Y_i,C_i) \iid (Y,C)$.
...
: `rats`  [@pkgsurvival] time-to-event dataset with added hypothetical columns ($Y,C$).
Rows are individual observations, $X$ columns are features, $T$ is observed time-to-event, $\Delta$ is the censoring indicator, and $(Y,C)$ are hypothetical (here arbitrary values dependent on $(T,\Delta)$) true survival and censoring times. {#tbl-surv-data-rats}
...
Finally the following terms are used frequently throughout this report. Let $(T_i, \Delta_i) \iid (T,\Delta), i = 1,...,n$, be random survival outcomes.
Then,
...
The predominating approach to competing-risks analysis is the so called *cause-specific hazards* approach, where we define $h_{0e}$ the hazard for the transition from initial state 0 to state $e =1,\ldots,K$:
...
$$
intuitively the integrants are given as the probability of not experiencing *any* event at time $u$ ($S(u)$) multiplied with the instantaneous risk of experiencing an event of type $e$ ($h_{oe}(u)$).
...
$$
and so $S_W(\tau) = 1-F_W(\tau)$. This function is known as the survival function as it can be interpreted as the probability that a given individual survives beyond some time point $\tau \geq 0$.
