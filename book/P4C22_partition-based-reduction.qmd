...
Define $x_{i1} = \II(\text{complications}_i = \text{"yes"})$.
...
However, using all unique event times can lead to very large transformed datasets: since each of $n$ subjects can contribute up to $m$ rows (where $m$ is the number of unique event times), the augmented dataset has $\tilde{n} = \sum_{i=1}^n J_i$ rows, which is $O(nm)$ in the worst case.
