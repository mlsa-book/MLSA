...
For each subject $i$, we record their interval-specific event indicator
<!--  -->
$$
...
Now consider the partitioning of the follow-up (@eq-cut-points) and assume we are only interested in whether the event occurred within an interval rather than the exact event time.
...
$$
S_{\tilde{Y}}(j) = \prod_{k \leq j}(1 - h_{\tilde{Y}}(k)) \qquad\text{and}\qquad P(\tilde{Y} = j) = S_{\tilde{Y}}(j-1)\, h_{\tilde{Y}}(j).
$$
...
\end{aligned}$$ {#eq-discrete-likelihood}
<!--  -->

For a random variable $Z \sim \text{Bernoulli}(\pi)$ with $\pi = P(Z = 1)$, the likelihood contribution is $P(Z = z) = \pi^z (1-\pi)^{1-z}$, which is exactly the form of each factor in (@eq-discrete-likelihood) under the identification $\pi = h_{\tilde{Y}}(j)$.
...
<!--  -->
$$
\pi_j = P(\Delta_{ij} = 1 \mid t_j) = P(Y_i \in [a_{j-1}, a_j) \mid Y_i \geq a_{j-1}) = h_{\tilde{Y}}(j).
...
$$
\Delta_{ij}| \xx_{i}, t_j \stackrel{iid}{\sim} \text{Bernoulli}(\pi_{ij}),
$$

where $\pi_{ij} = P(\Delta_{ij} = 1| \xx_{i}, t_j) = h_{\tilde{Y}}(j|\xx_{i})$ is the discrete hazard rate for interval $j$ given features $\xx_{i}$ and is estimated by applying binary classifiers to
...
Now assume that the interval-specific event indicators $\delta_{ij}$ are realizations of random variables $\Delta_{ij} \stackrel{iid}\sim \text{Poisson}(\mu_{ij} : = h_j t_{ij})$ and recall that $Z\sim \text{Poisson}(\mu)$ implies $P(Z = z) = (\mu^z \exp(-\mu))/z!$.
Thus, the likelihood contribution of subject $i$ can be written as 
<!--  -->
$$\begin{aligned}
\calL_{\text{Poisson},i} 
& = \prod_{j=1}^{J_i} P(\Delta_{ij} = \delta_{ij}) \\
...
<!--  -->
$$\log(E[\Delta_{ij}]) = \log(h_{ij}) + \log(t_{ij}),$${#eq-pem-model}
<!--  -->
...
- When $t_j$ is treated as a factor variable, the specific choice of $t_j$ (midpoint, endpoint, etc.) doesn't matter at all. Each interval receives its own parameter (e.g., an interval-specific intercept $\beta_{0j}$), as in the examples in @sec-discrete-time-reduction and @sec-piecewise-constant-hazards. This is the classical PEM approach and provides maximal flexibility, but assumes that the hazard in each interval is estimated independently of neighboring intervals. Without penalization, this can lead to implausible hazard estimates, especially when some intervals contain few events. The number of baseline hazard parameters equals $J$, which becomes prohibitive when $J$ is large (e.g., when using all unique event times as cut points).
