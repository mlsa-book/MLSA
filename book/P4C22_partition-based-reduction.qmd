...
<!--  -->
Note that $\delta_{ij}=0$ for all intervals except the last interval of subject $i$, where $\delta_{iJ_i} = \delta_i$. Thus the total number of events is the same in both the original and transformed data, and given the interval boundaries $\mathcal{A}$, $\delta_{ij}$ is a deterministic function of the original data.

We further define the time that subject $i$ was at risk for the event in interval $j$
<!--  -->
$$
t_{ij} = \begin{cases}
...
$$ {#eq-time-at-risk}
<!--  -->
which later enters as offset in some of the partition based reductions, 
as well as some representation of the time in interval $j$, for example the interval midpoint
<!--  -->
$$
t_j = \frac{a_{j-1} + a_j}{2}.
$$ {#eq-interval-endpoint}
<!--  -->
Choosing $t_j$ on the time scale rather than the bare interval index $j$ allows the same framework to handle both interval-specific intercepts (categorical $t_j$) and smooth functions of time (continuous $t_j$); see @sec-choice-of-interval-boundaries for further discussion.

The transformation is completed by recording the subject's (potentially time-dependent) features
<!--  -->
$$
\xx_{ij}
...
Note that $t_j$ is the only feature that is shared across all subjects within an interval. 
It is later used to estimate the (baseline) hazard. 
In many practical implementations it is simply included as another column of $\xx_{ij}$, though we keep it explicit here to emphasise its role.
...
<!--  -->
where $\delta_{ij}$ are the targets and $t_j$ enters the estimation as a feature and is needed in order to estimate different hazards/probabilities in different intervals.
...
To illustrate the discrete-time reduction approach, once again consider the tumor dataset introduced in @tbl-surv-data-tumor. The follow-up time is partitioned into $J = 100$ equidistant intervals and the data are transformed according to the procedure described in @sec-data-transformation.
...
$$
Combining these and using assumption (@eq-constant-hazard) gives
<!--  -->
$$\begin{aligned}
\calL_{i}
...
<!--  -->
where the third and fourth equalities follow from the hazard being a step function (@eq-constant-hazard) and the definition of time at risk (@eq-time-at-risk), and the last equality uses the property $\delta_{iJ_i} = \delta_i$ with $\delta_{ij}=0$ for $j<J_i$ from (@eq-interval-indicator).
...
<!--  -->
$$\log(\mu_{ij}) = \log(E[\Delta_{ij}]) = \underbrace{\beta_{0j} + \beta_{1j}x_{i1}}_{\log(h_{ij})} + \log(t_{ij}),$${#eq-pem-model-interaction}
<!--  -->
