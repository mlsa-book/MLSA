...
When should PEM be preferred over the discrete-time approaches above?
Two practical reasons stand out.
First, PEM preserves the exact within-interval event time through the offset $\log(t_{ij})$, so no information is lost by discretising; the fitted model returns continuous-time hazards, from which survival probabilities at any $\tau$ are obtained without interpolation.
Second, PEM combines naturally with smooth functions of time (e.g.\ penalised splines or tree-based smoothers; see @sec-choice-of-interval-boundaries), which makes the approach scalable to very fine partitions without the proliferation of interval-specific parameters seen in the discrete-time interaction model.
The trade-off is conceptual overhead --- PEM requires a Poisson regression with offset, which fewer software stacks support directly compared to vanilla binary classification --- and the fact that fitted hazards are continuous rates rather than per-interval probabilities, which some readers find less intuitive than $h_{\tilde{Y}}(j)$.
