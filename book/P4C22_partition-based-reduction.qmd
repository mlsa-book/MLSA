...
See @sec-choice-of-interval-boundaries for specific choices of the interval and how they effect modelling.
<!-- TODO: ANDI I DELETED THE ABOVE BECAUSE IT WAS HARD TO UNDERSTAND BY ITSELF AND WITHOUT JUMPING FORWARD -->
...
<!--  -->
The last equality holds case-by-case: for $\delta_i = 1$ the right-hand side reduces to $\prod_{j<J_i}(1-h_{\tilde{Y}}(j))\, h_{\tilde{Y}}(J_i)$ (matching the first bracket), and for $\delta_i = 0$ to $\prod_{j=1}^{J_i}(1-h_{\tilde{Y}}(j))$ (matching the second bracket).
<!-- FIXME: WHAT DO YOU MEAN 'MATCHING THE FIRST/SECOND BRACKET'? WHICH BRACKETS? WHAT'S MATCHING? -->
...
To summarize, by fitting a classification model with $\delta_{ij}$ as targets and $(\xx_i, t_j)$ as covariates, the predictions $\hat{\delta}_{ij}$ can be interpreted as estimates of $\hat{h}_{\tilde{Y}}(j|\xx_{i})$.
<!-- FIXME: ANDI IS THIS CORRECT? -->
...
<!--  -->
to the transformed dataset, where $j$ denotes the interval index, $\beta_{0j}$ are the interval-specific intercepts (technically we include the interval index $j$ as a reference coded categorical feature in the model) and $\beta_1$ is the common effect of complications on the discrete hazard rate (across all intervals).
<!-- FIXME: ANDI THIS IS CONFUSING BECAUSE IT SOUNDS LIKE 'J' IS SEPARATELY INCLUDED SOMEHOW, CAN WE SAFELY DELETE THE LONGER BIT IN PARANTHENSES? -->

The estimated survival probabilities are obtained by calculating

$$
\hat{S}_{\tilde{Y}}(j|x_{1}) = \prod_{k=1}^j (1-\hat{h}_{\tilde{Y}}(k|x_{1})),
$$

for each interval $j$ and complication group, where $\hat{h}_{\tilde{Y}}(k|x_{1})$ are the predicted discrete hazards from the logistic regression model.
<!-- FIXME: I THINK THERE'S SOMETHING MISSING HERE AS YOU'VE TALKED ABOUT HAT{S} AND HAT{h} BUT YOU HAVEN'T SAID HOW THIS RELATES TO THE LOGIT PI FUNCTION. HOW DOES THE LOGIT RESULT IN THE PREDICTED DISCRETE HAZARDS? MAYBE WORTH SHOWING THE RESPONSE/LINK?  -->
...
<!-- FIXME: THIS CONFUSED ME BECAUSE THIS SOUND LIKE THE GENERAL DISCRETE-TIME APPROACH -->
...
Predictions of the survival function at any continuous time $\tau$ follow by integrating the piecewise-constant hazard up to $\tau$,
<!-- FIXME: A LITTLE CONFUSING YOU SAY INTEGRATING BUT THEN SHOW SUMMATION -->
<!--  -->
$$
\hat S(\tau \mid \xx) = \exp\!\left(-\left[\sum_{j: a_j \leq \tau} \hat h_j(\xx)\,(a_j - a_{j-1}) \;+\; \hat h_{J_\tau}(\xx)\,(\tau - a_{J_\tau-1})\right]\right),
$$ {#eq-pem-survival}
<!--  -->
where $J_\tau$ is the index of the interval that contains $\tau$. The first sum accumulates the hazard over completed intervals before $\tau$, and the trailing term adds the partial-interval contribution from $a_{J_\tau-1}$ up to $\tau$.
...
<!-- TODO: ANDI CAN YOU ADD ANYTHING THAT EXPLAINS WHY SOMEONE SHOULD USE PEM OVER ANY OF THE ABOVE METHODS? PROS/CONS -->
