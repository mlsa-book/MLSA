...
1. **For** $b = 1,...,B$:
2. $D_b \gets \text{ Randomly sample, with replacement, } \dtrain \text{ with probability } \phi$
...
Commonly, $\phi = 1$, which means the resulting dataset will be the same size as the original dataset, however as it is a bootstrapped sample with replacement, the resulting data, on average, only contains around 63.2\% of the original observations being included at least once in each bootstrap sample [@Efron1997].
