...
\begin{algorithmic}[1]
\State Compute $(j^*, c^*)$ as the optimisers of $SR$ (e.g. \cref{eq:dt_min}) to create the initial leaf and branches.
...
\State Grow a decision tree, $\hatg_b$, on $D_b$ with \cref{alg:dt_fit}.
\EndFor
...
Prediction from a random forest follows by making predictions from the individual trees and aggregating the results by some function $\sigma$ \see{alg:rsf_pred}; $\sigma$ is usually the sample mean for regression,
...
Intuitively these results follow as the number of deaths in a leaf is distributed according to $\Hyper(n^A_\tau,n_\tau,d_\tau)$. The same statistic results if $L^B$ is instead considered. \Cref{alg:dt_fit} follows for fitting decision trees with the log-rank splitting rule, $SR$, to be maximised.
...
\end{equation}
which is repeated until some stopping criterion is reached. The same cumulative hazard is estimated for all nodes however $\hat{\theta}_m$ varies across nodes. This method lends itself naturally to a composition to a full distribution \see{sec:car_pipelines_distr} as it assumes a PH form and separately estimates the cumulative hazard and relative risk \see{sec:surv_ml_models_ranfor_nov}, though no implementation of this composition could be found.
...
As discussed above, Ishwaran's Relative Risk Forest makes a relative risk prediction in each terminal node \see{sec:surv_ml_models_ranfor_nodes} by fitting
\begin{equation}
