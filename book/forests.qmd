...
\label{sec-surv-ml-models-ranfor}
...
\textbf{Output} Fitted decision tree, $\hatg$.}\label{alg-dt-fit}
\begin{algorithmic}[1]
...
Despite being more interpretable than other machine learning methods, decision trees usually have poor predictive performance, high variance and are not robust to changes in the data. As such, \emph{random forests} are preferred to improve prediction accuracy and decrease variance. Random forests utilise bootstrap aggregation, or \emph{bagging}  [@Breiman1996a], to aggregate many decision trees. A pseudo fitting algorithm is given in  \ref{alg:rsf_fit}.

\begin{algorithm}
\caption{Fitting a random forest. \\
\textbf{Input} Training data, $\dtrain$. Total number of trees, $B \in \PNaturals$. \\
\textbf{Output} Fitted random forest, $\hatg$.}\label{alg-rsf-fit}
\begin{algorithmic}[1]
\For{$b = 1,...,B$}
...
\textbf{Output} Prediction, $\hatY \sim \calY$.}\label{alg-rsf-pred}
\begin{algorithmic}[1]
\For{$b = 1,...,B$}
...
\label{sec-surv-ml-models-ranfor-nodes}
...
\label{sec-surv-ml-models-ranfor-nov}
