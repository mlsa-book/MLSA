...
The survival prediction problems identified in \ref{sec:surv_set_types} are now formalised as machine learning tasks.
...
These closely related tasks are discussed in more detail in \ref{sec:car_redux}, with a particular focus on how the more popular regression setting can be used to solve survival tasks. In \ref{chap:review} the models are first introduced in a regression setting and then the adaptations to survival are discussed, which is natural when considering that historically machine learning survival models have been developed by adapting regression models.