...
## Learners and models {#sec-ml-learners}
...
* A predicted functional is considered 'good' if the *expected generalization error*, $\EE[L(Y^*, \hatg(X^*))]$, is low, where $(X^*, Y^*) \sim (X,Y)$ is independent of the training data $\calD_0$, and $L$ is some loss that is chosen according to the domain of interest (regression, classification, survival).

___

## Training and predicting {#sec-ml-train}

In practice, unless a model is being deployed for ongoing use, learners are not usually trained on all available data, $\calD$.
Instead, data is split into *training data*, $\dtrain$ and *testing data*, $\dtest$, which are comprised of two non-overlapping
