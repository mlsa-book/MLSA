...
Beta coefficients in a linear regression model, $y = \alpha + x\beta$, are model parameters.
Model *hyperparameters* control *how* the algorithm is run, for example determining if the intercept, $\alpha$, should be included in a linear regression model is an example of setting a hyperparameter.
...
Estimating the generalization error requires data splitting, which is discussed next.

## Training and predicting {#sec-ml-train}

If a model is being deployed for ongoing predictions of newly collected data, then a learner may be trained on all available data, $\calD$.
In practice, in scientific researcher and model development, learners are usually trained on a subset of the data and they are tested on the remainder.
This separation of $\calD$ is known as *data splitting*, which creates *training data*, $\dtrain \subset \calD$, and *testing data*, $\dtest = \calD \setminus \dtrain$.
To understand why data splitting is required, consider

Data splitting is required to prevent overoptimism of the training error (@Hastie2013), which occurs when the model's performance
...
* Data splitting, holdout resampling
...
## Evaluation and model comparison {#sec-ml-eval}
