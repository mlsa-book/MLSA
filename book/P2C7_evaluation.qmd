...
By following established statistical methods for evaluation, and ensuring that robust resampling methods are used [@Hastie2013], evaluation provides a method for estimating the 'generalization error', which is the expected model performance on new datasets.
...
These measures could also be categorized into how they evaluate predictions.
...
Measures of predictive performance evaluate individual predictions and usually take the sample mean of these to estimate the generalization error.

In the next few chapters we categorize measures by the type of survival prediction they evaluate, which is a more natural taxonomy for selecting measures, but we use the above categories when introducing each measure.
...
A model is usually said to 'outperform' another if it has a lower generalization error.
...
Where possible, (repeated) k-fold cross-validation (kCV) should be used for more robust estimation of the generalization error and for model comparison.
In kCV, the data is partitioned into $k$ folds (often $k$ is $5$ or $10$), which are non-overlapping subsets.
A model is trained on $k-1$ folds and evaluated on the $k$th fold, this process is repeated until each of the $k$ folds has acted as the test set exactly once, the computed loss from each iteration is averaged into the final loss, which provides a good estimate of the generalization error.

## Baselines

Interpreting measures is more complex in the survival domain compared to classification and regression as these measures often include weighting schemes that prevent raw metrics being understood on their own.
For this reason it is always advisable to compare models in robust benchmark experiments, or, at the very least, include 'baselines' to for model comparison.
Here a baseline refers to a simple model, such that performing better than the baseline represents a model is performing well.
...
For the rest of this part of the book we will introduce different survival measures, discuss their advantages and disadvantages.
...
@sec-conclusions-faq-eval includes recommendations for choosing measures as well as common problems around advanced topics including interpretability, bias/fairness, and measure selection.
