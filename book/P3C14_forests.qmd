...
\end{split}
$$
as the two leaves resulting from partitioning feature $j$ at cutoff $c_j$. To simplify notation, let $L^l, L^r$ denote $L^l(j,c_j)$ and $L^r(j,c_j)$ respectively. A split is then determined by the arguments $(j^*, c^*_{j^*})$ minimising the residual sum of squares across both leaves [@Hastie2013],
$$
...
<!-- TODO: section title a bit misleading. The hypotheis test is never performed in the sence that p-value is calcualted and used for splitting. And commonly hypothesis test based splitting = Coditional inference forest. Here we just split by highest log-rank statistic. -->
...
The log-rank test then tests $H_0: h^l = h^r$, i.e. equal hazard functions in both leaves, with test statistic [@Segal1988],
$$
...
In a competing risks setting, Gray's test [@Gray1988] can be used instead of the log-rank test, as it compares cumulative incidence functions rather than cause-specific hazards; see @sec-surv-ranfor-cr for details.
...
In particular, the likelihood could be constructed for different types of censoring and truncation as discussed in @sec-surv-estimation-param. However, in order to use them in combination with the *likelihood-ratio* statistic for splitting, the likelihood must fit the underlying model assumptions.
For example a parametric form could be assumed and a likelihood proposed by @LeBlanc1992 may be calculated to test model fit. Alternatively, the likelihood-ratio can be calculated using the Cox PH partial likelihood, but this assumes proportional hazards and is only applicable to right-censored data.
While potentially useful in principle, these methods are implemented in very few off-shelf software packages, thus empirical comparisons to other splitting rules are lacking.
...
Given new data $\xx^*$ falling into terminal node $L_b(\xx^*)$, the relative risk prediction is the sum of the predicted cumulative hazard $\hatH_b(\cdot|\xx^*)$ (@eq-surv-nelson) evaluated at the observed outcome time of each observation $i \in L_b(\xx^*)$:
...
<!-- TODO rapha: isn't the below exactly the averaging of mixtures we warn about above? -->
$$
