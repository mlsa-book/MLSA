...
Before moving on, we note that the discrete-time cumulative hazard $H^d_{\tilde{Y}}(\tau)$ is defined two ways in the literature.
...
An alternative convention defines $H^d_{\tilde{Y}}(\tau) := -\log S^d_{\tilde{Y}}(\tau) = -\sum_{k=1}^{\tau}\log\bigl(1 - h^d_{\tilde{Y}}(k)\bigr)$ to preserve $S = \exp(-H)$ exactly.
The two coincide in the small-hazard limit, where $-\log(1 - h^d(k)) \approx h^d(k)$, and both converge to the continuous-time integral $H(\tau) = \int_0^\tau h(u)\,\du$ as the discretisation is refined; for a unified product-integral treatment, see @aalen2008survivalevent [§3.2.4].
...
In practice, $\tau_u$ is often subject-dependent, $\tau_{u,i}$: with *staggered entry* (subjects enter the study at different calendar times) and a fixed calendar end-date, each subject's administrative censoring time is the difference between study closure and their entry.
Conditional on the observed entry time, $\tau_{u,i}$ is deterministic and known, so the mechanism is still classified as Type-I (sometimes *generalized Type-I*); treated unconditionally — entry as a random recruitment process — $\tau_{u,i}$ inherits that randomness and behaves as random censoring.
...
Whether interval-censoring must be handled explicitly depends on the modelling time scale: if the chosen resolution is coarser than the typical interval width (e.g. annual rather than monthly), interval-censored observations effectively become exactly-observed events at that scale.
Otherwise interval-censoring requires dedicated estimators (see @sec-surv-estimation).
...
When the parameter dependence matters explicitly (e.g. inside a likelihood), we write the cdf, density, and survival function evaluated at $\tau$ as $F_Y(\tau\mid\bstheta)$, $f_Y(\tau\mid\bstheta)$, and $S_Y(\tau\mid\bstheta)$.
...
The corresponding estimation principle is *maximum likelihood estimation (MLE)*: parameters are chosen to maximise the probability of the observed data under the model,

$$
\hat{\bstheta} = \argmax_{\bstheta} \calL(\bstheta).
$$

In machine learning, models are typically trained by *minimising a loss function*, and the two views coincide: the negative log-likelihood $-\log \calL(\bstheta)$ is the loss of an MLE-trained probabilistic model.
Most survival-specific losses introduced later in this book are (weighted or censored) variants of a log-likelihood, so reading any of them as "training-time MLE under a particular censoring/truncation pattern" is usually a productive mental model.
...
Closed-form maximisation is not available; the standard tool is Turnbull's iterative *self-consistency algorithm* [@Turnbull1976], a special case of the EM algorithm in which the unobserved exact event times are the latent data. The intuition is that an observation $i$ should only contribute to the estimated probability of an event at a candidate time $\tau$ if its interval $(L_i, R_i]$ could plausibly have contained that event — exactly the condition $\tau \in (L_i, R_i]$.

**Defining the time grid.** Interval-censored data does not provide observed event times. Instead, we construct a grid of *candidate* event times from the data's interval endpoints. Pool all observed left and right endpoints,
$$
\{0\} \cup \{L_1, R_1, \ldots, L_n, R_n\},
$$
sort, drop duplicates, and call the resulting ordered values $0 = \tau_0 < \tau_1 < \cdots < \tau_m$. The candidate event times $\tau_k, k = 1,\ldots,m$ are the right endpoints of the partition intervals $(\tau_{k-1}, \tau_k]$. The NPMLE is a step function on this grid; only intervals $(\tau_{k-1}, \tau_k]$ contained in some observation interval $(L_i, R_i]$ can carry positive mass (the *innermost intervals* of @Peto1973 and @Turnbull1976), and the algorithm below assigns zero mass to the rest automatically, so the pooled grid above can be used directly without first identifying the innermost intervals. Note that the $\tau_k$ are *constructed* grid points and are a different object from the observed ordered event times $t_{(k)}$ (@eq-unique-ordered-event-times) used for the right-censored Kaplan-Meier estimator in @sec-surv-km.
