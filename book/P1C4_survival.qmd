...
![Right-censoring for four subjects. Solid lines mark observed periods; dashed lines mark hypothetical (unknown in reality) post-censoring continuations. Filled black circles mark observed events, filled black diamonds mark true (unobserved) event times after censoring, and open circles mark censoring times. Subject 1's event is observed during the study. Subject 2 is censored during the study and would have died during the study (unobserved). Subject 3 is censored early in the study and dies after study end (unobserved). Subject 4 is administratively censored at study end and dies later (unobserved).](Figures/survival/censoring.png){#fig-survset-censor fig-alt="Time on the x-axis (study start to study end), four subjects on the y-axis. Solid black segments show observed periods, dashed segments show hypothetical post-censoring continuations; filled black circles mark observed events, filled black diamonds mark unobserved true events, open circles mark censoring."}
...
![Illustration of left-truncation for three infants. For each subject, the upper bar shows the left-truncation time $t_i^L$ (age at mother's death) and the lower bar the time-to-event $t_i$ (age at own death or censoring). Solid lines mark in-data subjects ($t_i^L < t_i$); dashed lines mark the invisible subject 1 ($t_1 < t_1^L$, infant dies before mother). Filled black circles, filled black diamonds, and open circles mark observed events, unobserved events, and right-censoring respectively.](Figures/survival/left-truncation.png){#fig-left-truncation fig-alt="Three subjects on the y-axis, each with two stacked horizontal bars: the upper bar shows the left-truncation time and the lower bar the time-to-event. Subject 1's bars are both dashed (never enters the data); subjects 2 and 3's bars are solid, ending in a filled black circle and an open circle respectively."}
...
![Illustration of right-truncation for three subjects observed retrospectively from a disease registry. For each subject, the upper bar shows the right-truncation time $t_i^r$ (time from infection to the database query) and the lower bar the time-to-event $t_i$ (incubation period). Subjects 1 and 3 have $t_i < t_i^r$ (event before query) and are included in the sample; subject 2 has $t_2 > t_2^r$ (event after query), so they are absent from the sample and both bars are drawn dashed.](Figures/survival/right-truncation.png){#fig-right-truncation fig-alt="Three subjects on the y-axis, each with two stacked horizontal bars on a 0-30 day axis (time since infection). Subjects 1 and 3 have solid bars ending in a filled black circle (observed event) within the truncation window; subject 2 has both bars dashed with the time-to-event extending past the truncation time and ending in a filled black diamond (unobserved event)."}
