...
$$
\KMG(t) = \prod_{\ell=1}^g \left(1-\frac{d^C_{t_{(\ell)}}}{n_{t_{(\ell)}}}\right), 
$$ {#eq-km-censoring}

where $t_{(\ell)},l=1,\ldots,g$ are the unique, ordered censoring times.
In this book, $\KMG$ specifically refers to the Kaplan-Meier estimator fit on the observed censoring times $(t_i, 1-\delta_i)$ from some training data.
