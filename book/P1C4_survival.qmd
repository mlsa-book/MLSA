...
it is therefore vital for practitioners to be able identify and specify the survival problem present in their data to ensure models are correctly fit and evaluated.
...
<!--  -->
$$
S_{\tilde{Y}}(\tau) = P(\tilde{Y} > \tau) = \prod_{k\leq\tau} s_{\tilde{Y}}(\tau) = \prod_{k\leq\tau}(1-h_{\tilde{Y}}(\tau)),
...
$$ {#eq-discrete-prob}
<!--  -->
When applied to continuous time $Y$, the follow-up is divided in $J$ disjunct intervals $[a_0,a_{1}), \ldots, [a_{j-1},a_j), \ldots, [a_{J-1},a_{J}]$, such that
<!--  -->
$$
...
Before moving on, note that the discrete-time cumulative hazard $H^d_{\tilde{Y}}(\tau)$ may be defined in one of two ways in the literature.
...
$$
H^d_{\tilde{Y}}(\tau) := -\log S^d_{\tilde{Y}}(\tau) = -\sum_{k=1}^{\tau}\log\bigl(1 - h^d_{\tilde{Y}}(k)\bigr),
$$

which preserves $S = \exp(-H)$ exactly.

The two coincide in the small-hazard limit, where $-\log(1 - h^d(k)) \approx h^d(k)$, and both converge to the continuous-time integral $H(\tau) = \int_0^\tau h(u)\,\du$ as the discretization is refined; for a unified product-integral treatment, see @aalen2008survivalevent [§3.2.4].
...
A *survival dataset* is an $n \times p$, real-valued matrix defined by $\calD = ((\xx_1, \ t_1, \ \delta_1) \cdots (\xx_n,t_n,\delta_n))^\trans$, where $(t_i,\delta_i)$ are realizations of the respective random variables  $(T_i, \Delta_i)$ and $\xx_i$ is a $p$-dimensional vector, $\xx_i = (x_{i,1} \ x_{i,2} \cdots x_{i,p})^\trans$ of features.
...
If a predictive model is defined at the level of calendar years, then event is exactly assigned to year $2$, effectively ignoring the within-year uncertainty which requires interval censoring.
...
Also note that in case of left- and interval-censoring the event is known to have occurred, while for right-censoring the event did not occur during time under observation.
...
For right-censored observations the true event time lies somewhere in $(t_i, \infty]$; as discussed in @sec-distributions-continuous, this implicitly assumes a proper $Y$, with the improper case handled by cure models.
...
More formally, let $t^\ell_i$ the subject-specific left-truncation time.
...
![Illustration of left-truncation for three infants. For each subject, the upper steelblue bar shows the truncation time $t_i^\ell$ (age at mother's death) and the lower black bar the time-to-event $t_i$ (age at own death or censoring). Solid lines mark in-sample subjects ($t_i^\ell < t_i$); dotted lines mark the invisible subject 1 ($t_1 < t_1^\ell$, infant dies before mother). The truncation bar ends in a filled steelblue circle for in-sample subjects and a filled steelblue diamond for the unobserved subject. The vertical dashed line marks the study end at age 365 days. Filled black circles, filled black diamonds, and open circles mark observed events, unobserved events, and right-censoring respectively.](Figures/survival/left-truncation.png){#fig-left-truncation fig-alt="Three subjects on the y-axis, each with two stacked horizontal bars on an age axis 0-365 days: the upper steelblue bar shows the truncation time and the lower black bar the time-to-event. Subject 1's bars are both dotted (never enters the sample) and the truncation bar ends in a filled steelblue diamond; subjects 2 and 3's bars are solid with their truncation bars ending in a filled steelblue circle, while the time-to-event bars end at a filled black circle (observed event at ~200 days) and an open circle (censoring at study end) respectively. A vertical dashed grey line marks the study end at 365 days."}
...
![Illustration of right-truncation for three subjects observed retrospectively from a disease registry. The x-axis is calendar time and subjects are recruited at staggered times (subject 1 at $t = 0$, subject 3 at $t = 5$, subject 2 at $t = 10$). For each subject, the upper steelblue bar runs from recruitment to the database query date (truncation time, $t_i^r$) and the lower black bar runs from recruitment to the event ($t_i$). Subjects 1 and 3 have $t_i < t_i^r$ (event before query) and are included in the registry (solid bars, truncation bar ending in a filled steelblue circle). Subject 2 has $t_2 > t_2^r$ (event after query), so they are absent from the registry — both bars are drawn dotted, the truncation bar ends in a filled steelblue diamond, and the unobserved event marker (filled black diamond) lies beyond the query line. The vertical dashed line marks the database-query date.](Figures/survival/right-truncation.png){#fig-right-truncation fig-alt="Three subjects on the y-axis, calendar time 0-32 days on the x-axis. Each subject has two stacked horizontal bars starting at their recruitment time: an upper steelblue bar (truncation time, ending at the database-query date) and a lower black bar (time-to-event). Subjects 1 and 3 have solid bars with the truncation bars ending in filled steelblue circles and observed-event circles before the query; subject 2 has dotted bars with the truncation bar ending in a filled steelblue diamond and the time-to-event extending past the query line and ending in an unobserved-event black diamond. A vertical dashed grey line marks the database-query date."}
...
Equivalently, training a model on right-censored data by minimising the negative log of @eq-right-censoring-likelihood is the MLE view of the same objective.
...
* left-truncation:
  ...
  * event: $f(Y_i = t_i|Y_i \geq t_i^\ell, \bstheta) = \frac{f_Y(t_i|\bstheta)}{S_Y(t_i^\ell|\bstheta)}$
  * left-censoring: $P(Y_i < t_i|Y_i \geq t_i^\ell, \bstheta) = \frac{S_Y(t_i|\bstheta)}{S_Y(t_i^\ell|\bstheta)}$
  * interval-censoring: $P(l_i < Y_i \leq r_i|Y_i \geq t_i^\ell,\bstheta) = \frac{S_Y(l_i|\bstheta) - S_Y(r_i|\bstheta)}{S_Y(t_i^\ell|\bstheta)}$
...
Define $\tilde{Y} = k \Leftrightarrow Y \in [t_{(k)},t_{(k+1)})$ as the discrete time representation of $Y$ (with $t_{(m+1)}$ being some time point larger then the last event time).
...
The variance of the Kaplan-Meier estimator is commonly estimated with the consistent Greenwood estimator\index{Kaplan-Meier estimator!Greenwood} [@Greenwood1926]:

<!-- FIXME (ANDREAS): DOUBLE CHECK BELOW CORRECT -->
$$
\widehat{\mathrm{Var}}(S_{KM}(\tau)) = S_{KM}^2(\tau) \sum_{k:t_{(k)} \leq \tau} \frac{d_{t_{(k)}}}{n_{t_{(k)}}(n_{t_{(k)}} - d_{t_{(k)}})}.
$$ {#eq-greenwood}
...
For interval-censored data, the Kaplan-Meier estimator is replaced by the nonparametric maximum likelihood estimator (NPMLE) [@Zhigang2010; @Sun2006]. The NPMLE maximises the likelihood derived in @sec-surv-estimation-param under interval-censoring without assuming a parametric form for $S_Y$.

Closed-form maximisation is not available; the standard tool is Turnbull's iterative *self-consistency algorithm* [@Turnbull1976], a special case of the expectation-maximization algorithm in which the unobserved exact event times are latent data.
...
$$ {#eq-turnbull-w}
recording whether the $k$-th grid interval could have contained subject $i$'s event. Initialise $\hatS^{0}(\tau_k), k = 0, \ldots, m$ as described above.
<!-- FIXME: I DON'T THINK YOU CAN SAY 'AS DESCRIBED ABOVE' WITHIN A CALLOUT -->
2. **For** $j = 1, 2, \ldots$ **until convergence:**
3. \ \ \ \ \ For $k = 1, \ldots, m$, compute the current probability mass on the $k$-th grid interval $(\tau_{k-1}, \tau_k]$:
$$p_{\tau_k}^{(j)} = \hatS^{j-1}(\tau_{k-1}) - \hatS^{j-1}(\tau_k).$$ {#eq-turnbull-p}
4. \ \ \ \ \ For $k = 1, \ldots, m$, compute the expected number of events in $(\tau_{k-1}, \tau_k]$ under the current estimate:
$$d_{\tau_k}^{(j)} = \sum_{i=1}^{n} \frac{w_{ik}\, p_{\tau_k}^{(j)}}{\sum_{l=1}^{m} w_{il}\, p_{\tau_l}^{(j)}}.$$ {#eq-turnbull-d}
5. \ \ \ \ \ Compute the corresponding number "still at risk just before $\tau_k$":
$$n_{\tau_k}^{(j)} = \sum_{l=k}^{m} d_{\tau_l}^{(j)}.$$ {#eq-turnbull-n}
6. \ \ \ \ \ Plug $d_{\tau_k}^{(j)}$ and $n_{\tau_k}^{(j)}$ into the Kaplan-Meier formula (@eq-km) to obtain $\hatS^{j}$.
7. \ \ \ \ \ **If** $\max_{k}\bigl|\hatS^{j}(\tau_k) - \hatS^{j-1}(\tau_k)\bigr| < \epsilon$, **stop**.
8. **Return** $\hatS^{j}$.
...
Reading the matrix: subject 1's event could fall in any of the first three grid intervals (all contained in $(0, 5]$); the third grid interval $(4, 5]$ is the only one in which any of the three subjects' events could fall.
...
$$
L_{NL}(\xx_i, \delta_i, t_i) = \frac{-\delta_i \log(\hat{f}(t_i|\xx_i))}{\KMG(t_i)}.
$$
...
| $t_i$ | $\delta_i$ | $\hat{G}(t_i)$ | $1 / \hat{G}(t_i)$ | $-\delta_i \log(\hat{f}(t_i \mid \mathbf{x}_i))$ | $L_{NL}(\mathbf{x}_i, \delta_i, t_i)$ |
|---|---|---|---|---|---|
