...
## Independent and dependent censoring {.unnumbered .unlisted}
...
Event-dependent censoring, when $C \nindep Y$, is a tricky case that, if not handled appropriately can easily lead to poor model development.
...
Many of the losses in Part II are (weighted or censored) variants of a log-likelihood, reading these as "training-time MLE under a particular censoring/truncation pattern" can be a productive mental model.
<!-- TODO: WHAT DO YOU MEAN 'TRAINING-TIME MLE' -->
...
<!-- FIXME: THIS FEELS MORE COMPLEX AND MATH HEAVY THAN THE REST OF THE BOOK; I'M NOT CONVINCED IT'S NEEDED IN THIS DETAIL -->
...
**Defining the time grid.** Interval-censored data does not provide observed event times. Instead, one constructs a grid of *candidate* event times from the data's interval endpoints.
The left and right endpoints are then pooled,
$$
\{0\} \cup \{L_1, R_1, \ldots, L_n, R_n\},
$$

sorted, and duplicates dropped.
The resulting ordered values are $0 = \tau_0 < \tau_1 < \cdots < \tau_m$.
The candidate event times $\tau_k, k = 1,\ldots,m$ are the right endpoints of the partition intervals $(\tau_{k-1}, \tau_k]$.
The NPMLE is a step function on this grid; only intervals $(\tau_{k-1}, \tau_k]$ contained in some observation interval $(L_i, R_i]$ can carry positive mass (the *innermost intervals* of @Peto1973 and @Turnbull1976), and the algorithm below assigns zero mass to the rest automatically, so the pooled grid above can be used directly without first identifying the innermost intervals.
Note that the $\tau_k$ are *constructed* grid points and are a different object from the observed ordered event times $t_{(k)}$ (@eq-unique-ordered-event-times) used for the right-censored Kaplan-Meier estimator in @sec-surv-km.

For each subject $i$ and grid index $k$, define the indicator
$$
w_{ik} = \mathbb{I}\bigl((\tau_{k-1}, \tau_k] \subseteq (L_i, R_i]\bigr),
$$ {#eq-turnbull-w}
which records whether the event for subject $i$ could have occurred in the $k$-th grid interval. Right-censored observations have $R_i = \infty$, so $w_{ik} = 1$ for all $k$ with $\tau_{k-1} \geq L_i$.

**Worked example.** Consider three subjects with intervals $(0, 5]$, $(2, 8]$ and $(4, 10]$. Pooling the endpoints gives $\{0, 2, 4, 5, 8, 10\}$, hence $\tau_0 = 0,\ \tau_1 = 2,\ \tau_2 = 4,\ \tau_3 = 5,\ \tau_4 = 8,\ \tau_5 = 10$ (so $m = 5$). The indicator matrix is
...
Reading the matrix: subject 1's event could fall in any of the first three grid intervals (which are all contained in $(0, 5]$); the third grid interval $(4, 5]$ is the only one in which any of the three subjects' events could fall.

**Initial guess.** Following @Turnbull1974 and @Giolo2004, a convenient starting estimate $\hatS^{0}$ is the Kaplan-Meier estimator (@eq-km) computed by treating each $R_i$ as an event time if $R_i < \infty$ and as right-censored otherwise; this *right-endpoint Kaplan-Meier* coincides with the usual Kaplan-Meier when no observation is interval- or left-censored. A flat starting value, $\hatS^{0}(\tau_k) = 1 - k/m$, also works and is the choice in some software [e.g. @pkgicenReg]; the iterates converge to the same limit.
...
1. Construct the grid $\tau_0 < \cdots < \tau_m$ from the pooled endpoints, the indicators $w_{ik}$ from @eq-turnbull-w, and an initial estimate $\hatS^{0}(\tau_k), k = 0, \ldots, m$ as described above.
...
A close variant of this algorithm handles purely left- or doubly-censored data; see @Klein2003 [§5.2] for a fully worked example on the time-to-first-marijuana-use data set of @Turnbull1974.
...
* @rinneHazardRateTheory2014 for a comprehensive discussion of the hazard rate.
