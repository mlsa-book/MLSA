...
No let $\tilde{Y}$ be a discrete, positive random variable that represents discrete or discretized time, taking values in $\mathbb{N}_{>0}$; and $\tau\in \mathbb{N}_{>0}$ some time point in discrete time.
...
Note that interval-censoring is often introduced by coarsening the data or rounding. This however can be ignored if the interval length scale is negligible compared to the time scale of the event of interest. For example, time-until death after surgery is often reported as a date rather than exact date-time. The resulting survival time is thus given in days an technically interval-censored. However, since the time until death is often months or years, such data are often analyzed using standard techniques
...
<!-- TODO (RS) NOT SURE I UNDERSTAND THIS FIRST PART OF THE SECTION -->
...
A common principle for such techniques is to partition the follow-up into intervals or to define specific time-points during the follow-up and to estimate the continuous or discrete time hazards (@eq-discrete-hazard) for each interval/time-point.
...
The Kaplan-Meier estimator introduced earlier (@sec-surv-km) is an example for this principle.
...
For interval-censoring, the Kaplan-Meier estimator is replaced by the nonparametric maximum likelihood estimator (NPMLE) [@Zhigang2010], which estimates the likelihood function under the assumption of left- or interval-censoring data as in @sec-surv-estimation-param.
...
Let $\tbi, k = 1,...,m$ be the ordered times at which to evaluate the NPMLE.

1. Make an initial guess for $\hatS^0(\tbi), k = 1,...,m$.

Where $\hatS^0$ is the initial guess and $\hatS^j$ is the estimated survival function at iteration $j$.

2. **While** $\quad \lvert \hatS^j(t_{(k)}) - \hatS^{j-1}(t_{(k)}) \rvert > \epsilon, \forall k = 1,...,m \quad$ **Do:**

a. Compute the probability of an event at each time-point:

$$
p_{t_{(k)}} = \hatS^{j-1}(t_{(k-1)}) - \hatS^{j-1}(t_{(k)}), \quad k = 1,...,m
$$
...
In the presence of left-truncation, the Kaplan-Meier is calculated using the left-truncated risk-set definition [@McGough2021], which excludes individuals from the risk set until their left-truncation time has occurred (@eq-risk-lt).
The number of individuals at risk at $\tau$ is then:

$$
n^L_\tau = \sum_i \II(t_i^L \leq \tau \leq t_i)
$$

where $t_i, t_i^L$ are the outcome time and left truncation time of observation $i$ respectively.
This equation can be substituted into the Kaplan-Meier formula (@eq-km), with $d_{t_{(k)}}$ calculated in the usual manner.
