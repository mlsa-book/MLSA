...
Note that interval-censoring is often introduced by coarsening the data or rounding.
This however can be ignored if the interval length scale is negligible compared to the time scale of the event of interest. 
For example, time-until death after surgery is often reported as a date rather than exact date-time.
The resulting survival time is thus given in days and technically interval-censored.
However, since the time until death is often months or years, such data are often analyzed using standard techniques
