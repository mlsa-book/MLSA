...
Consider a survey participants are asked "how old were you when you used a smart phone for the first time?".
...
In the phone survey example, the participants might specify any age range between $0$ and $t_i$; some examples are given in (@tbl-censoring-phones).
...
In such cases, these non-independent censoring events should actually be modelled as competing events\index{competing risks} (@sec-eha).
...
As an example, if studying the survival time of pregnant patients in hospital, dropping out of the study due to pregnancy is clearly dependent on how many weeks pregnant the patient is when the study starts (for the sake of argument assume no early or late pregnancy due to illness).
Type I and type II\index{censoring!type I}\index{censoring!type II} censoring (@sec-types-of-censoring-right) are usually conditionally event-independent because the censoring mechanism is determined by study design or stopping rules rather than the underlying event process.
However, patients with a more favorable prognosis at study entry are often more likely to remain under observation for longer and therefore have a higher probability of censoring, illustrating that censoring may depend on covariates while still remaining conditionally event-independent.
