...
These are commonly referred to as *time-varying*\index{time-varying covariates} or *time-dependent covariates*\index{time-dependent covariates|see{time-varying covariates}}.
...
For example, suppose a treatment is only offered after three years of survival.
Knowing that a patient received the treatment therefore guarantees that they survived long enough to receive it.
If patients are incorrectly assigned to the treatment group at baseline ($t=0$) rather than when treatment actually begins ($t=3$), the model learns an artificial survival advantage for treated patients, as they must have survived the initial untreated period.
...
For a comprehensive treatment see @vanHouwelingen2011.
...
In particular, left truncations plays an important role when modeling recurrent events\index{recurrent events} or multi-state data\index{multi-state models} (@sec-multi-state).
While censored observations have incomplete information about the time-to-event, they are still part of the dataset.
Whereas truncation leads to observations not entering the dataset, or at least not at time $0$.
...
2. if the subject _does_ enter the study, $t_i \ge t^\ell_i$, then they are only observed from $\tau = t_i^\ell$ rather than $\tau = 0$ (known as *delayed entry*)\index{delayed entry}\index{selection bias}.

Methods to handle these cases are discussed in @sec-surv-estimation-param and @sec-non-param-lt; failing to account for left-truncation results in biased estimators.
...
In this example, left-truncation biases the sample towards healthier infants, as frail infants die earlier and thus on average before their mothers.
...
<!-- FIXME (ANDREAS): SHOULDN'T THIS BE "SURVIVAL PROBABILITIES WERE OVERESTIMATED"? I THINK UNDER/OVERESTIMATED IS A BIT CONFUSING ANYWAY. COULD WE INSTEAD SAY SOMETHING CLEARER LIKE "LARGER" -->
...
$$
G_{KM}(\tau) = \prod_{k:t_{(k)} \leq \tau}\left(1-\frac{d^C_{t_{(k)}}}{n_{t_{(k)}}}\right),
$$ {#eq-km-censoring}

where again $t_{(k)}, k=1,\ldots,m$, are the ordered event times.
...
where $S_C(\tau \mid \xx_i)$ is the survival function of the censoring time, that is, the probability that the observation is uncensored up until $\tau$.
...
Under the simplifying assumption that censoring is independent of the covariates, the censoring distribution is usually estimated with the Kaplan-Meier estimator (@sec-surv-km-censoring).
...
The illustrative loss excludes censored observations as their true event times are unknown.
...
| $t_i$ | $\delta_i$ | $\hat{G}(t_i)$ | $1 / \hat{G}(t_i)$ | $-\delta_i \log(\hatf(t_i \mid \xx_i))$ | $L_{NL}(\xx_i, \delta_i, t_i)$ |
|---|---|---|---|---|---|
