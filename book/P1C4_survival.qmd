...
The exact event time is unknown but it *is* known that the event happened after the observed censoring time, hence *right*-censoring (imagine a timeline from left to right as in @fig-p1c4-censoring).
...
Dotted lines in @fig-p1c4-km-tumor indicate the median survival\index{median survival}, defined as the time at which the survival function reaches $S(\tau) = 0.5$. 
