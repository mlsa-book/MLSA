...
![Dead and censored subjects (y-axis) over time (x-axis). Black diamonds indicate true death times and white circles indicate censoring times. Diamonds at the end of dashed lines are hypothetical (unknown in reality). Vertical line is the study end time. Subject 1 dies during the study. Subject 2 is censored during the study and (unknown) dies during the study. Subject 3 is censored early in the study and (unknown) dies after the study. Subject 4 is administratively censored at study end and (unknown) dies later.](Figures/survival/censoring.png){#fig-survset-censor fig-alt="Time on the x-axis, four subjects on the y-axis. Solid black lines show observed periods, dashed black lines show hypothetical post-censoring continuations, filled diamonds mark true death times and white circles mark censoring times; a vertical line marks the study end."}
...
This is illustrated in @fig-left-truncation for three observed infants.
Each infant's left-truncation period (steelblue) runs from birth to the mother's death at $t_i^L$, after which the infant is included in the study and is followed (black) until either the event of interest or the end of the study at age $\tau^* = 365$ days.
Infants 1 and 2 die during the study at ages $t_1$ and $t_2$ respectively; infant 3 is alive at study end and is therefore right-censored.
Crucially, infants whose own death precedes their mother's are absent from the dataset entirely (point (i) above) and are not shown.

![Illustration of left-truncation data for three infants who entered the study after their mother's death. The left-truncation period (steelblue) runs from birth to study entry at $t_i^L$; the follow-up period (black) runs from entry to event (filled diamond) or right-censoring at study end (white circle). Infants whose own death preceded their mother's never enter the dataset.](Figures/survival/left-truncation.png){#fig-left-truncation fig-alt="Schematic illustration of left-truncation: three subjects on the y-axis, with each subject's timeline split into a steelblue truncation period from age 0 to study entry, followed by a black follow-up period ending at an event (diamond) or censoring (circle)."}
...
![Illustration of right-truncation based on registry data. For subjects 1 and 3 the event occurs before the database is queried (filled diamond on a black observed segment), so they are included in the sample. For subject 2 the event occurs after the query (filled diamond on the steelblue truncation segment, beyond the dashed query line) and the subject is therefore excluded from the sample.](Figures/survival/right-truncation.png){#fig-right-truncation fig-alt="Calendar time on the x-axis, three subjects on the y-axis. Subjects 1 and 3 have black observed segments ending in a filled diamond before the database-query line; subject 2 has a black segment up to the query line followed by a steelblue right-truncation segment ending in a filled diamond beyond the query."}
