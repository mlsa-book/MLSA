...
This problem can also arise when a covariate is recorded as fixed at baseline but is defined using information from after baseline.
In the example above, this bias could easily arise from a retrospective dataset that records a patient received treatment by assigning the covariate value `treatment = 1`.
Methods for addressing immortal time bias are conceptually similar to methods for handling left truncation: observations with an immortal time period must be modeled so that they do not contribute to the risk set (@eq-risk-set) during that period.
For more on immortal time bias, see @Suissa2008 and @Levesque2010.
