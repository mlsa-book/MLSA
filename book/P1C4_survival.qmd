...
Almost all software implementations rely on continuous-time theory, and continuous-time methods are therefore typically applied even when the underlying data are technically discrete.
...
CLearly if an observation is censored at $\tau = 0$, it provides no information about the survival process and can be reasonably removed.
The appropriate handling of events at $\tau = 0$ depends on the time scale and application.
For example, on a scale of days, an observation with $t_i = 0$ may indicate that no meaningful prediction can be made prior to the event.
In contrast, for coarser time scales such as years, assigning observations to $t_i = 1$ ensures they are still encoded as high-risk.
...
<!--  -->
$$
S^d_{\tilde{Y}}(\tau) = P(\tilde{Y} > \tau) = \prod_{k\leq\tau} s^d_{\tilde{Y}}(\tau) = \prod_{k\leq\tau}(1-h^d_{\tilde{Y}}(\tau)),
