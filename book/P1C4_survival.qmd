...
The complement of @eq-discrete-hazard, the probability to survive beyond $\tau$ given survival until $\tau$, is defined as,
...
The third case is referred to as left-censoring, the event occurred some time before the interview.
The fourth case is an example of interval-censoring, the event occurred within some age span but the exact age is unknown.
...
Then only with $y_i > t^\ell_i$ are observed; subjects with $y_i < t^\ell_i$ never enter the data.
...
For example, for right-censored data ($\delta_i = 0$), it is only known that the event occurred after the censoring time $t_i$.
...
In the presence of left-truncation, $n_{t_{(k)}}^\ell \leq  n_{t_{(k)}}$ and therefore $S(t_{(k)})^\ell \leq S(t_{(k)})$.
...
The definition of the risk set\index{risk set} (@eq-risk-set) is thus the denominator $n_{\tau}$ in @eq-disc-haz-non-param remains the same, but the numerator $d_{\tau}$ is now the number of censoring events at $\tau$, which is denoted by, 
