...
*Survival analysis*\index{survival analysis} is concerned with data where the outcome is the time until an event takes place (a 'time-to-event').
...
In survival analysis terminology this is referred to as *right-censoring*, one of several censoring mechanisms introduced below.\index{censoring!right}
...
For example, a problem that aims to predict the number of rounds of chemotherapy before remission would record the outcome as a discrete-time value\index{discrete-time survival analysis}.
...
Moreover, discrete-time methods are often applied to continuous-time data and vice versa (see @sec-partition-based-reductions for discrete-time methods applied to continuous data)\index{reductions}.
...
A standard representation of the distribution of $Y$ is given by the probability density function\index{probability density function}, $f_Y: \NNReals \rightarrow \NNReals$, and cumulative distribution function\index{cumulative distribution function}, $F_Y: \NNReals \rightarrow [0,1]; F_Y(\tau) = P(Y \leq \tau)$.
...
By definition, $S_Y(0)=1$ and $S(\tau)\rightarrow 0$ for $\tau \rightarrow \infty$ (handling when this is not the case is discussed at the end of this section).
...
The formal definitions above take $Y$ to be a *proper* random variable\index{proper distribution}, i.e. $S_Y(\tau) \to 0$ as $\tau \to \infty$.
However, many real-world events are *improper* at the population level\index{improper distribution}, $S_Y(\infty) = \pi_0 > 0$.
...
Now let $\tilde{Y}$ be a discrete, positive random variable that represents discrete or discretized time, taking values in $\PNaturals$; and $\tau\in \PNaturals$ some time point in discrete time\index{discrete-time survival analysis}.
...
A useful quantity to define that is frequently used in the literature is the set of unique, ordered event times\index{unique event times}.
For example, given survival data, $(t_1=6, \delta_1=0)$, $(t_2=1, \delta_2=1)$, $(t_3=10,\delta_3=1)$, $(t_4=5,\delta_4=0)$, $(t_5 =10,\delta_5=1)$ the unique ordered event times are $\{t_{(1)},  t_{(2)}\} = \{1,10\}$ as the censored observations are removed, as are duplicated times.
Formally, let $m\leq n$ be the number of unique, observed event times.
Then the *unique, ordered event times* are defined simply as,
...
Let $(t_i, \delta_i) \iid (T,\Delta), i = 1,...,n$, be observed survival outcomes, then,
...
Multiple types of censoring and/or truncation\index{truncation} (@sec-truncation) can occur in any given dataset and it is vital to identify which types are present in order to correctly select and specify models and measures for the data.

### Right-censoring

Right-censoring is the most commonly assumed form of censoring in survival data\index{censoring!right}.
...
Left-censoring\index{censoring!left} occurs when the event is known to have happened at some unknown time before observation time. 
Interval-censoring\index{censoring!interval} occurs when the event is known to have happened within some time span, but not the exact time.
...
* If $C \indep X$, censoring is *feature-independent*, otherwise censoring is *feature-dependent*.\index{censoring!feature-dependent}\index{censoring!dependent}
* If $C \indep Y$, censoring is *event-independent*, otherwise censoring is *event-dependent*\index{censoring!event-dependent}.
* If $C \indep Y | X$, censoring is conditionally independent of the event given covariates, or *conditionally event-independent*.\index{censoring!conditionally independent}
...
In such cases, these non-independent censoring events should actually be modelled as alternative events of interest using a competing risks\index{competing risks} framework (@sec-eha).
...
Left-truncation\index{truncation!left} often arises when inclusion into a study is conditional on the occurrence of another event, and plays an important role when modeling recurrent events\index{recurrent events} or multi-state data\index{multi-state models} (@sec-eha), thus the concept is introduced in more detail.

Importantly, left-truncation has two defining consequences for the observed data: (i) subjects whose event occurs *before* study entry never enter the dataset, so the sample is systematically biased toward longer event times; and (ii) subjects who *do* enter the dataset are followed from their subject-specific entry time $t_i^\ell$ rather than from $t = 0$ (*delayed entry*)\index{delayed entry}\index{selection bias}.
...
Right-truncation\index{truncation!right} often occurs in retrospective sampling based on registry data, when data is queried for cases reported by a certain cut-off time (see for example @vakulenko-lagun.inverse.2020).
...
While specific details about estimation will be given in Parts III and IV, there are general concepts that can apply across a wide range of models.
...
Let $\mathcal{O}, \mathcal{RC}, \mathcal{LC}, \mathcal{IC}$ non-overlapping subsets of the observed data $\mathcal{D}$ for subjects with observed event times, right-censoring, left-censoring and interval-censoring, respectively\index{censoring!right}\index{censoring!left}\index{censoring!interval}.
...
Note that in practice, datasets will often not contain all types of censoring or truncation, in which case @eq-objective-function will contain only a subset of the product terms.
This has been illustrated in @eq-right-censoring-likelihood under absence of truncation and $\mathcal{LC}=\mathcal{IC}=\emptyset$.
...
The *Kaplan-Meier (KM) estimator*\index{Kaplan-Meier estimator} [@Kaplan1958] is the standard non-parametric estimator of the survival function (@eq-surv-prob) under fully independent right-censoring.
...
The survival probability\index{survival function} and the definition of the Kaplan-Meier estimator (@eq-km) then follow from Equation (@eq-discrete-surv-prob):
...
Dotted lines indicate the median survival\index{median survival}, defined as the time at which the survival function reaches $S(\tau) = 0.5$. 
...
For example, @fig-km-tumor-age-bin shows $\KMS$ separately for subjects aged 50 years or older and subjects younger than 50 years, respectively.
Dashed lines again illustrate median survival times.
...
As the Kaplan-Meier estimator (@sec-surv-km), the Nelson-Aalen\index{Nelson-Aalen estimator} estimator (@Nelson1972,@Aalen1978) is based on the empirical discrete-time hazards estimator (@eq-disc-haz-non-param).
...
Kaplan–Meier\index{Kaplan-Meier} and Nelson–Aalen are consistent for survival and cumulative hazard respectively under independent right-censoring.
...
The Kaplan-Meier\index{Kaplan-Meier} (@eq-km) and Nelson-Aalen\index{Nelson-Aalen} (@eq-nelson-aalen) estimators can still be used with $n_{t_{(k)}}$ replaced by $n^\ell_{t_{(k)}}$ and $d_{t_{(k)}}$ calculated in the usual manner.
...
For interval-censored\index{censoring!interval} data, the Kaplan-Meier\index{Kaplan-Meier estimator} estimator is replaced by the nonparametric maximum likelihood estimator (NPMLE)\index{nonparametric maximum likelihood estimator} [@Zhigang2010; @Sun2006]. The NPMLE maximizes the likelihood derived in @sec-surv-estimation-param under interval-censoring without assuming a parametric form for $S_Y$.

Closed-form maximization is not available; the standard tool is Turnbull's\index{Turnbull estimator} iterative *self-consistency algorithm* [@Turnbull1976], a special case of the expectation-maximization algorithm in which the unobserved exact event times are latent data.
...
6. \ \ \ \ \ **If** $\max_{k}\bigl|\hatS^{j}(\tau_k) - \hatS^{j-1}(\tau_k)\bigr| < \epsilon$, **stop**.
7. **Return** $\hatS^{j}$.
...
The techniques discussed in section @sec-surv-estimation can also be used to estimate the distribution of censoring times $(t_i, 1-\delta_i)$\index{censoring distribution}. 
This is often done in order to calculate the inverse probability of censoring weights\index{inverse probability of censoring weights} (@sec-surv-ipcw) used in the evaluation of survival models (@sec-cindex) and in order to account for (covariate-)dependent censoring (@willems2018correctingdependent).


### The Kaplan-Meier estimator for the censoring distribution {#sec-surv-km-censoring}

The Kaplan-Meier\index{Kaplan-Meier estimator} estimator can be used to estimate the distribution of censoring times.
The definition of the risk set\index{risk set} (@eq-risk-set) is thus the denominator $n_{\tau}$ in @eq-disc-haz-non-param remains the same, but the numerator $d_{\tau}$ is now the number of censoring events at $\tau$, which we denote as 
...
IPC weighting is most commonly used to adjust for bias introduced by right-censoring\index{censoring!right} by upweighting observations that remain under observation at later time points.
...
For uncensored observations, the negative log-likelihood of the predicted probability density function is evaluated at the true event time.
