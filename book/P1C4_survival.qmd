...
To continue an example from the introduction, when modelling the time between relapses from multiple sclerosis, not every patient will experience a relapse.
...
$$
<!-- FIXME (ANDREAS): I ADDED THE EXTRA LINE BECAUSE THE RELATIONSHIP WASN'T IMMEDIATELY OBVIOUS TO ME -->
...
* $n_{\tau = 1217} = \vert \calR_{1217} \vert = 3$
...
Thus the likelihood contribution for such data points is $\Pr(Y_i > t_i) = S_Y(t_i)$, whereas for observed event times ($\delta_i=1$) the likelihood contribution is $f_Y(t_i)$ as before.
...
$$
\calL(\bstheta) \propto \prod_{i \in \mathcal{O}}f_Y(t_i \mid \bstheta) \prod_{i \in \mathcal{RC}}S_Y(t_i \mid \bstheta) \prod_{i \in \mathcal{LC}}(1-S_Y(t_i \mid \bstheta))\prod_{i \in \mathcal{IC}}(S_Y(l_i \mid \bstheta)-S_Y(r_i \mid \bstheta))
...
$$
h_{\tilde{Y}}(k) = P\left(Y\in [t_{(k)},t_{(k+1)}) \mid Y \geq t_{(k)}\right)=\frac{d_{t_{(k)}}}{n_{t_{(k)}}}.
...
$$
L_{NL}(\xx_i, \delta_i, t_i) = \frac{-\delta_i \log(\hatg(t_i \mid \xx_i))}{\KMG(t_i)}.
$$
