...
First, the hazard is a **rate per unit time**: its numerical value depends on the scale on which $\tau$ is measured.
A hazard of $0.01$ on a daily scale equals $3.65$ on an annual scale for the same underlying process. 
Second, hazards — not survival or density functions — are the natural modelling target in survival analysis because censoring affects them locally: at a given $\tau$, only subjects still at risk contribute, so the hazard can be estimated from locally available information even when $S(\tau)$ is not directly observed at every $\tau$.
...
A *survival dataset* is a $n \times p$, real-valued matrix defined by $\calD = ((\xx_1, t_1, \delta_1), \ldots, (\xx_n, t_n, \delta_n))^\trans$, where $(t_i,\delta_i)$ are realizations of the respective random variables  $(T_i, \Delta_i)$ and $\xx_i$ is a $p$-dimensional vector, $\xx_i = (x_{i,1}, x_{i,2}, \ldots, x_{i,p})^\trans$ of features.
...
As the name suggests, these weights are defined as the inverse of the probability of remaining uncensored.
