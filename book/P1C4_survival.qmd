...
When the parameter dependence matters explicitly (for example, inside a likelihood), one denotes the cumulative distribution function, density, and survival function evaluated at $\tau$ as $F_Y(\tau\mid\bstheta)$, $f_Y(\tau\mid\bstheta)$, and $S_Y(\tau\mid\bstheta)$.
