...
For example, some approaches treat the event indicator as a target for a classification task or directly use the observed time as a target for a regression task whilst ignoring the censoring status [@Schwarzer2000].
...
This yields a dataset which can be passed to the previously learned regression or classification model to generate predictions.
...
The second reduction transforms the problem into a regression task (@sec-pseudo-value-reduction).
Like the classification approach, it can estimate survival probabilities at fixed times, but may also be used to estimate many other important quantities including the restricted mean survival time, and is also applicable to event history analysis.
...
In contrast, the partition-based reductions in @sec-partition-based-reductions aim to estimate the entire event time distribution rather than a handful of time points.
Finally, @sec-reductions-eha introduces reductions that allow event history analysis to be expressed in terms of single-event tasks, which allows many of the models in Part III to be applied to competing risks and multi-state settings.
