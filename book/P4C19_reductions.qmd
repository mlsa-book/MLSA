...
According to @beygelzimerLearningReductionsThat2016, a reduction is ''a complex problem decomposed into simpler subproblems so that a solution to the subproblems gives a solution to the complex problem''.
While the techniques discussed here also introduce some additional overhead, for example the need to perform a specific type of data pre-processing, we argue that they can still be viewed as reductions, as they transform several survival tasks to more standard regression or classification tasks.
...
- there is a temporal delay between the development of a new machine learning method for classification or regression and its adaptation to survival analysis. 
...
- do not make (strong) assumptions about the underlying distribution of event times, and thus have the same advantages as survival-specific semi-parametric methods (@sec-classical-nonpar, @sec-classical-cox),
...
In particular we will consider IPC-weighted classification (@sec-icpw-reduction), pseudo-value based regression (@sec-pseudo-value-reduction), discrete-time reduction (@sec-discrete-time-reduction) and piecewise exponential models (@sec-piecewise-constant-hazards). 