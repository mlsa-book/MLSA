...
Survival analysis reductions cover a lot of possibilities, including transforming the event-history setting to single-event and vice versa; converting between survival, regression, and classification; and transforming the time horizon between continuous and discrete time.
...
The reduction methods in this book are based on clear theory that focuses on how the resulting predictions are unbiased estimates for time-to-event predictions, which means appropriately dealing with (and not ignoring) censoring and/or truncation.
Furthermore, the reductions in the next chapters do not make (strong) assumptions about the underlying distribution of event times, and thus have the same advantages as non-parametric (@sec-classical-nonpar) and semi-parametric methods (@sec-classical-cox),
