...
This applies to right-censored data only but is particularly useful when there are several time-points of interest, the dataset is very large, or computational resources are limited.
The second reduction transforms the problem into a regression task (@sec-pseudo-value-reduction).
Like the classification approach, it can estimate survival probabilities at fixed times, but may also be used to estimate many other important quantities across event-history analysis including the RMST and CIF.
However this flexibility comes at a computationally cost, especially when many time points are considered or the dataset is large.
...
Finally, @sec-reductions-eha introduces reductions that allow event history analysis to expressed in terms of single-event tasks, which allows many of the models in Part III to be applied to competing risks and multi-state settings.
