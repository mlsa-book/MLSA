...
The process of fitting is model-dependent, and can range from non-parametric methods and maximum likelihood estimation of model coefficients to machine learning approaches.
The model fitting process is discussed abstractly in @sec-surv-setml and concrete algorithms are discussed in Part III of the book.
...
* The observed data used for model training (observed times $T$) is different from the outcome of interest (event times $Y$).
...
* With the exception of the time-to-event prediction, all other prediction types do not predict $\EE(Y)$, which is often of interest, but some other (related) quantity.
...
Given covariates, $\XX \in \Reals^{n \times p}$, and a vector of model coefficients, $\bbeta \in \Reals^p$, the predictor is defined by $\boldsymbol{\eta}:= g(\XX) \in \Reals^n$, where $g$ is a function of the covariates (for example $\XX\bbeta$ in linear models).
The prognostic index is a term often used to refer to some transformation, $\phi$, on the linear predictor, $\phi(\eta)$, (which could simply be the identity function $(\phi(\eta) = \eta)$).
