...
In this section we consider the different prediction types that might be of interest the survival analysis context and introduce the notion of a survival task more formally.
...
The model fitting process is discussed on a high-level in @sec-surv-setml and concrete algorithms are discussed in Part III of the book.
...
Interpretation of these rankings depends on the model class (for example, PH and AFT models have opposite interpretation, @sec-models-classical) and its parametrization or implementation in specific software.
...
Given covariates, $\XX \in \Reals^{n \times p}$, the predictor is defined by $\boldsymbol{\eta}:= g(\XX) \in \Reals^n$, where $g$ is a function of the covariates. In case of linear models that estimate a vector of model coefficients, $\bbeta \in \Reals^p$, this would be given by $\boldsymbol{\eta}=\XX\bbeta$.
The prognostic index is a term often used to refer to some transformation, $\phi$, on the predictor, $\phi(\eta)$, (which could simply be the identity function, $\phi(\eta) = \eta$.
...
As mentioned above, all prediction types can (in theory) be derived from a survival distribution prediction.
For example, a time-to-event prediction can be obtained via $E(Y|\xx) = \int_0^\infty \hat{S}(t)$.
For non-parametric methods, however, the estimated cdf is often improper in the presense of censoring and thus integration requires extrapolation of the cdf.
...
## Survival Task {#sec-surv-setmltask}

To summarize, a survival task, Let $\calX$ the feature space such that $\xx\ t.v.i. \calX \subseteq \Reals^p$ as before, $Y\ t.v.i. \calT \subseteq \NNReals$,
and $\calS \subseteq \distrT$ a convex set of distributions on $\calT$ and let $\calR \subseteq \Reals$.
Then,
...
Any other survival prediction type falls within one of these tasks for example predicting survival time is the deterministic task and predicting prognostic index or linear predictor is the ranking task.
