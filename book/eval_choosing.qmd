...
Experiments may be performed to investigate patterns in observed data, to make predictions for new data, to compare the performance of multiple models ('benchmark experiments'), or some combination of these.
...
Finally, there are survival adaptations to the well-known AIC [@Liang2008] and BIC [@VolinskyRaftery2000] however as these are generally only applicable to low-complexity models, they are out of scope for this book and hence have not been discussed.
...
To reduce this risk, this book recommends always reporting both Harrell's C and Uno's C when evaluating discrimination from ranking predictions as doing so will increase confidence in drawing general conclusions about a model's overall discriminatory ability.
When evaluating discrimination from distribution predictions, Antolini's C can be used.
