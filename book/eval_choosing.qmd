...
After reading this part of the book, survival analysis may appear more complex than regression and classification settings, which, in contrast, have fewer (common) measures to choose from.
...
In contrast, there are no standard rules for selecting measures in survival analysis.
...
Experiments may be performed to investigate patterns in observed data, to make predictions for new data, or to compare the performance of multiple models ('benchmark experiments').
...
Before data can be investigated, any model that is trained on the data must first be demonstrated to be a good fit.
In-sample measures may be appropriate when testing if a model is a good fit to the data, whilst these have not been discussed in this book, there are survival adaptations to the well-known AIC [@Liang2008] and BIC [@VolinskyRaftery2000].
...
There is also the risk of C-hacking [@Sonabend2022] when the ranking prediction is composed from a distribution prediction (@sec-car) or when multiple C-indices are reported.
To reduce this risk, this book recommends always reporting Harrell's C and Uno's C when evaluating discrimination from ranking predictions, or to use Antolini's C when reporting discrimination from distribution predictions.
Before these metrics are computed, a pre-defined threshold should be established to determine if the deployed model is optimal, a natural threshold would be 0.5 as anything above this is better than a baseline model.
