...
# Extensions and Practical Considerations {#sec-eval-choose}

{{< include _wip_minor.qmd >}}

Given the plethora of possible survival measures to choose from, evaluating models may seem daunting.
Moreover, measures have primarily only been introduced for the single-event setting, and there are a number of ways these could be extended to competing risks.
Therefore, this final chapter in this Part of the book focuses on practical considerations for how to extend measures to competing risks and how to select measures for a given task.
...
To recap, given $q$ possible events then the cause-specific hazard for an event is defined as $h_{e}$:
...
$$
L^A_{RCLL} = \sum^q_{e=1} \frac{1}{n} \sum^n_{i=1} L^e_{RCLL;i}(\hatS_{i;e}, t_i, \delta_i)
$$ {#eq-surv-rcll-cr}
...
Note that dividing @eq-surv-rcll-cr by $q$ to find the average performance is not useful in practice.
As already stated, the numerical value of a loss rule does not have any inherent meaning beyond comparison to another model's performance within the same experiment, hence dividing all losses by the same constant (i.e., $q$) does not change the interpretation.
