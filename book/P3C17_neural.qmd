...
where $\hh^{(1)} = (h_1^{(1)} \ h_2^{(1)} \cdots h_4^{(1)})^\trans$, the activation, $a^{(1)}$, is applied element-wise, and
...
For a deterministic regression problem, the output layer consists of a single unit constructed in the same way as the hidden units, except the activation functions are replaced by an optional _response function_ (@sec-nnet-activations), $r$, giving in (@fig-ffnn-arch): \index{neural networks!response function}
...
As ReLU is piecewise linear, a network composed entirely of ReLU activations represents a piecewise-linear function with finitely many breakpoints.\index{neural networks!activation function!ReLU} <!-- TODO (ANDREAS; MINOR): SAME CHANGE AS THE OTHER RELU NOTE-->
...
As with boosting (@sec-boost), $M$ is usually chosen to be large and training is terminated by early stopping.\index{early stopping}
...
Using the notation in Algorithm 3, let $\calU(\bstheta, \bsgamma, \alpha)$ be an update rule with parameters $\bstheta$, gradients, $\bsgamma$, and learning rate $\alpha$.\index{gradient descent}
...
In contrast to bootstrapping  (@sec-ranfor), mini-batches are created by shuffling and partitioning the data into approximately $\lceil n / B \rceil$ batches of size $B$, ensuring every observation is included in exactly one batch.\index{neural networks!hyperparameters!mini-batch size}
...
Plain gradient descent (including its stochastic and mini-batch variants) is the simplest choice for the optimizer, $\calU$.\index{neural networks!optimizer}
...
As with gradient boosting machines, the number of epochs is also usually not tuned but instead a sufficiently large number is chosen and then early stopping is used to terminate training once a held-out validation loss stops improving.\index{gradient boosting machines}
...
Using the Weibull distribution as an example\index{Weibull distribution}, a network predicts two positive scalars: the scale $\lambda(\xx_i \mid \bstheta) > 0$ and shape $k(\xx_i \mid \bstheta) > 0$, with positivity again ensured by softplus or exponential response functions (@sec-nnet-activations).
...
where $\calR_{\tbk}$ is the risk set at the $k$th ordered event time, $\tbk$, for $m \leq n$ unique event times.
