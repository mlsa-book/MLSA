...
For a deterministic regression problem, the output layer consists of a single unit constructed in the same way as the hidden units, except the activation functions are replaced by an optional _response function_ \index{neural networks!response function}(@sec-nnet-activations), $r$. 
...
Finally, we use 
$$g(\xx \mid \bstheta) = y$$
as the conventional short-cut to define the neural network model as a function that maps inputs $\xx$ to outputs $y$, given parameters $\bstheta = \{\mathbf{W}^{(l)}, \mathbf{b}^{(l)}\}_{l=1}^{L+1}$.
