...
Let $\psi(\tau)$ denote a univariate (featureless), unbiased estimator of the quantity of interest, such as the Kaplan-Meier estimator, estimated as normal from all observations.
Let $\psi^{-i}(\tau)$ be the same estimator but fit by omitting the $i$-th observation from the dataset.
...
The choice of $h(\cdot)$ has consequences for both how the model is estimated and how covariate effects are interpreted; this is discussed in @sec-pseudo-value-response-function.
The following sections then provide step-by-step examples for two specific quantities of interest: survival probabilities (@sec-pseudo-value-S) and the restricted mean survival time (@sec-pseudo-value-rmst).
...
The response function $h(\cdot)$ is a modeling choice with two distinct but related consequences: how the model is estimated, and how covariate effects are interpreted.

### Post-hoc vs. integrated application
...
For example, when $h(\cdot)$ is the sigmoid, a model trained with squared error on pseudo-values $\tilde{\theta}_i \in \Reals$ will not necessarily minimize the error of the final probabilities $h(\hat{f}_\tau(\xx)) \in [0,1]$.

The alternative is to *integrate* $h(\cdot)$ into the fitting process by modifying the loss function.
...
For RMST targets ($\tilde{\theta}_i \geq 0$), the log link $h(f) = \exp(f)$ is a natural choice that guarantees positive fitted values and yields coefficients interpretable as log-RMST ratios [@andersen2004regressionanalysis; @Sachs2022].

In practice, post-hoc transformation is often sufficient when the pseudo-values are well-behaved (not too close to the boundary of the target space), and may be preferred for simplicity.
Integrated application is recommended when calibration is critical or when the pseudo-values vary widely.
...
@tbl-pseudo-link-interpretation summarizes the most common choices for survival probability targets and how they are interpreted, assuming a linear model for the features with effects  $\hat{\bsbeta}_\tau$. However, same principles apply when using interpretable machine learning methods for black-box models instead [@Molnar2019; @Langbein2025].

| Link $h^{-1}(\psi)$ | **Response** $h(f(\xx))$ | Interpretation of $\beta_j$ |
|---|---|---|
| Identity: $h^{-1}(\psi) = \psi$ | $h(f(\xx)) = f(\xx)$ | Absolute difference in $\psi(\tau|\xx)$ per unit increase in $x_j$ |
| Logit: $h^{-1}(\psi) = \log\frac{\psi}{1-\psi}$ | $\frac{1}{1+\exp(-f(\xx))}$ | Log-odds ratio for the event at $\tau$; $e^{\beta_j}$ is an odds ratio |
| Complementary log-log: $h^{-1}(\psi) = \log(-\log(\psi))$ | $1-\exp(-\exp(f(\xx)))$ | Log-hazard ratio; $e^{\beta_j}$ is a hazard ratio, equivalent to Cox PH at $\tau$ |
| Log: $h^{-1}(\psi) = \log(\psi)$ | $\exp(f(\xx))$ | Log-survival ratio; $e^{\beta_j}$ is a survival ratio |

: Common response functions for pseudo-value regression on survival probability targets ($\psi = S(\tau|\xx)$) and their interpretation of covariate effects. {#tbl-pseudo-link-interpretation}

The complementary log-log (cloglog) link deserves particular attention.
For survival probability $S(\tau|\xx)$, applying the cloglog link gives
$$
\log(-\log(S(\tau|\xx))) = f_\tau(\xx).
$$
If $\hat{f}_\tau(\xx) = \log(H_0(\tau)) + \xx^\top\bsbeta$, this is exactly the Cox proportional hazards model (equation @eq-ph in @sec-classical-cox), with $e^{\beta_j}$ interpretable as a hazard ratio.
Fitting the cloglog pseudo-value model separately for multiple time points $\tau$ thus offers a diagnostic for the proportional hazards assumption: if $\hat{\bsbeta}_\tau$ varies substantially across $\tau$, proportionality is in doubt [@Andersen2010].
...
: Illustration of pseudo-value calculation for the first 4 subjects from the tumor dataset at time point $\tau = 1000$ days. The table shows the overall Kaplan-Meier estimate $\hat{S}_{KM}(\tau)$ calculated for all $776$ subjects, the leave-one-out Kaplan-Meier estimate $\hat{S}_{KM}^{-i}(\tau)$ (obtained by omitting subject $i$), and the calculated pseudo-value $\tilde{\theta}_i(\tau)$. The features age and complications are not used for the calculation of the pseudo-values but are included in the regression step. {#tbl-pseudo-value-calculation}
...
$$
\hat{\tilde{\theta}}(\tau|\xx^*) = \hat{S}(\tau|\xx^*) = \left(1 + e^{-((\xx^*)^\top\hat{\bsbeta}_{\tau})}\right)^{-1}.
$$ 
...
$$
\hat{\tilde{\theta}}(\tau) = \hat{S}(\tau|\xx_i) = \hat{\beta}_{\tau,0} + \hat{\beta}_{\tau,1} x_{i,1},\quad x_{i,1} = \II(\text{complications}_i = \text{`yes'}),
...
<!--  -->
$$
\hat{\tilde{\theta}}(\tau) = \widehat{\text{RMST}}(\tau|\xx_i) = \hat{\beta}_{\tau,0} + \hat{\beta}_{\tau,1} x_{i,1},\quad x_{i,1} = \II(\text{complications}_i = \text{`yes'}),
$$
<!--  -->

and again assume $h(f(\xx)) = f(\xx)$ for simplicity (see @sec-pseudo-value-response-function for a discussion of alternative response functions for RMST targets).
...
$${#eq-pseudo-value-cif}
<!--  -->
where $F_{AJ,e}^{-i}(\tau)$ is the Aalen-Johansen estimator of the CIF for event type $e$ obtained by omitting the $i$-th observation from the dataset.
