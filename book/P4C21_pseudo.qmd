...
Going forward, we will simplify this notation and directly write 
$$\hat{\psi}(\tau \mid \xx_i) = g(\xx_i\mid\hat{\bstheta}_\tau)$$
when referring to the fitted model.

Sometimes, rather than learning $g(\cdot)$ directly, the model learns a *predictor function* $\eta(\xx_i\mid\bstheta_\tau)$, combined with a transformation via a known *response function* $r(\cdot)$, such that 

$$
g(\xx_i\mid\bstheta_\tau) = r(\eta(\xx_i\mid\bstheta_\tau)).
$$ {#eq-pseudo-value-response-function}

The response function maps the predictor $\eta$ to the target space (similarly to what was discussed for neural networks in @sec-nnet-activations).
...
This section details the use of pseudo-values for survival distribution predictions at selected time points.\index{reduction!pseudo-value regression!survival probabilities}
...
A linear model is then fit to these pseudo-values, shown in @tbl-pseudo-value-calculation, giving
...
At first glance this may seem surprising, as the pseudo-values are computed from a single, *unstratified* Kaplan-Meier estimator, yet the predictions closely match the *stratified* estimates.
...
Nonetheless, all of the estimated survival probabilities here lie within the valid range.
