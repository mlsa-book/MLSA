...
Let $\psi(\tau|\mathbf{x})$ denote some quantity of interest, for example the survival probability $S(\tau | \mathbf{x})$ or the restricted mean survival time, $\text{RMST}(\tau | \mathbf{x})$ (@sec-pseudo-value-rmst).
Assume we have an unbiased, univariate estimator $\psi(\tau)$ for the quantity of interest.
Then pseudo-values are defined as
