...
Computing all pseudo-values, $\dvec{\theta}{n}$, requires $n+1$ evaluations of the univariate estimator ($n$ evaluations of $\hat\psi^{-i}(\tau)$ and one evaluation of $\hat\psi(\tau)$) for each of the $m$ time points of interest, resulting in $m(n+1)$ evaluations in total.
Although this may seem computationally demanding, univariate estimators are typically very fast to compute, and efficient approximations have been proposed in the literature [@parnerRegressionModelsCensored2023; @bouaziz2023fastapproximations].
...
First, by construction, the empirical mean of the pseudo-values equals the global marginal estimate,
...
$$
\EE[\theta_i(\tau) \mid \xx_i] \;\approx\; \psi(\tau \mid \xx_i).
$$ {#eq-pseudo-value-expectation-1}

This approximate unbiasedness motivates regressing $\theta_i$ on $\xx_i$ in this reduction.
The relationship between features $\xx_i$ and pseudo-values $\theta_i(\tau)$ can then be learned through regression, with a separate model fitted for each time point $\tau$,

$$
r(f_\tau(\xx_i)) \approx \EE[\theta_i(\tau)\mid\xx_i],
$$ {#eq-pseudo-value-expectation-2}

where $f_\tau(\xx_i)$ is a function of the features learned by the model; for example, $f_\tau(\xx_i) = \xx_i^\top\bsbeta_{\tau}$ for linear regression.\index{linear regression}
The subscript $\tau$ indicates that a different function is estimated for each time point.
The function $r(\cdot)$ is a known *response function*\index{response function} that maps the predictions to the target space.
Its inverse $r^{-1}(\cdot)$ is the *link function*, which are common in generalized linear models.\index{link function}
...
The choice of $r$, and consequences for estimation and interpretation, are discussed in @sec-pseudo-value-response-function.

It follows from (@eq-pseudo-value-expectation-1) and (@eq-pseudo-value-expectation-2) that the fitted regression models can be used to make predictions of the quantity of interest conditional on covariates from new data $\xx_*$,

$$
\hat\theta(\tau\mid\xx_*) = \hat{\psi}(\tau\mid\xx_*) = r(\hat{f}_{\tau}(\xx_*)).
$$
...
$$
\theta_i(\tau) = n\KMS(\tau) - (n-1)\KMS^{-i}(\tau).
...
| complications | age | $i$| $t_i$| $\delta_i$| $\KMS(\tau)$| $\KMS^{-i}(\tau)$| $\theta_i(\tau)$|
|:-------|-------:|-------:|-------:|-------:|-------:|-------:|-------:|
...
: Illustration of pseudo-value calculation for the first 4 subjects from the tumor dataset at time point $\tau = 1000$ days. The table shows the overall Kaplan-Meier estimate $\KMS(\tau)$ calculated for all $776$ subjects, the leave-one-out Kaplan-Meier estimate $\KMS^{-i}(\tau)$ (obtained by omitting subject $i$), and the calculated pseudo-value $\theta_i(\tau)$. The features age and complications are not used for the calculation of the pseudo-values but are included in the regression step. {#tbl-pseudo-value-calculation}
...
$$
\theta_i(\tau) = n \widehat{\RMST}(\tau) - (n-1)\widehat{\RMST}^{-i}(\tau),
...
$$
\hat{\theta}(\tau) = \widehat{\RMST}(\tau \mid \xx_i) = \hat{\beta}_{\tau,0} + \hat{\beta}_{\tau,1} x_{i,1},\quad x_{i,1} = \II(\text{complications}_i = \text{`yes'}),
$$
