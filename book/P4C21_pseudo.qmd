...
Formally, let $\psi(\tau|\xx)$ denote the (population) quantity of interest, for example the survival probability $S(\tau | \xx)$ or the restricted mean survival time, $\text{RMST}(\tau | \xx)$ and $\psi(\tau)$ denote the marginal (univariate) version of the quantity of interest.
...
Therefore, the true quantity of interest is $\psi(\tau|\xx_i) = S(\tau|\xx_i)$, for which a natural univariate estimator is the Kaplan-Meier method, $\psi(\tau) = S_{KM}(\tau)$.
...
<!--  -->
$$
\hat{\theta}_i(\tau) = \hat{S}(\tau|\xx_i) = \xx_i^\top\hat{\bsbeta}_{\tau}.
...
$$
\hat{\theta}(\tau) = \hat{S}(\tau|\xx_i) = \hat{\beta}_{\tau,0} + \hat{\beta}_{\tau,1} x_{i,1},\quad x_{i,1} = \II(\text{complications}_i = \text{`yes'}),
...
$$
\begin{aligned}
\hat{S}(\tau|\xx_i) = \hat{\beta}_{\tau,0} + \hat{\beta}_{\tau,1} x_{i,1} + \hat{\beta}_{\tau,2} x_{i,2},\\ \quad x_{i,1} = \II(\text{complications}_i = \text{`yes'}), x_{i,2} = \text{age}_i,
\end{aligned}
$$
...
<!--  -->
$$
\hat{\theta}(\tau) = \widehat{\text{RMST}}(\tau|\xx_i) = \hat{\beta}_{\tau,0} + \hat{\beta}_{\tau,1} x_{i,1},\quad x_{i,1} = \II(\text{complications}_i = \text{`yes'}),
$$
<!--  -->
...
$$
\widehat{\text{RMST}}(\tau|\xx_i) = \hat{\beta}_{\tau,0} + \hat{\beta}_{\tau,1} x_{i,1} + \hat{\beta}_{\tau,2} x_{i,2},\quad x_{i,1} = \II(\text{complications}_i = \text{`yes'}), x_{i,2} = \text{age}_i,
$$
...
: Common response functions for pseudo-value regression on estimated survival probability targets ($\hat{\psi} = \hat{S}(\tau|\xx)$) and the associated interpretation of their corresponding covariate effects. {#tbl-pseudo-link-interpretation}

The complementary log-log link deserves particular attention.
For survival probability $\hat{S}(\tau|\xx)$, applying the complementary log-log link gives
$$
\log(-\log(\hat{S}(\tau|\xx))) = \hat{f}_{\tau}(\xx).
$$
...
An unbiased estimator for the CIF is the Aalen-Johansen estimator (@sec-aalen-johansen). 
...
In multi-state settings (@sec-multi-state), pseudo-values are often used in order to estimate state occupation probabilities conditional on features. These represent the probability of being in state $e$ at time $\tau$. Non-parametric estimators for state occupation probabilities can be obtained via the Aalen-Johansen estimator for multi-state models (@sec-ms-aalen-johansen). The pseudo-values for state occupation probabilities are then defined as
