...
$$
r(g_\tau(\xx_i)) \approx \EE[\theta_i(\tau)\mid\xx_i],
$$ {#eq-pseudo-value-expectation-2}

where $g_\tau(\xx_i)$ is a function of the features learned by the model; for example, $g_\tau(\xx_i) = \xx_i^\trans\bsbeta_{\tau}$ for linear regression.\index{linear regression}
...
$$
\hat\theta_*(\tau) = \hat{\psi}(\tau\mid\xx_*) = r(\hatg_{\tau}(\xx_*)).
$$
...
This section details the use of pseudo-values for survival distribution predictions.\index{reduction!pseudo-value regression!survival probabilities}
...
The true quantity of interest is $\psi(\tau \mid \xx_i) = S(\tau \mid \xx_i)$, for which a natural univariate estimator is the Kaplan-Meier estimator, $\psi(\tau) = S_{KM}(\tau)$.
...
A linear model can then be fit to the pseudo-values obtained in @tbl-pseudo-value-calculation to obtain predictions for new data,

$$
\hat{\theta}_i(\tau) = \hat{S}(\tau \mid \xx_i) = \xx_i^\trans\hat{\bsbeta}_{\tau}.
...
for time points $\tau \in \{1000, 2000, 3000\}$ days and using the identity function as the response. 

The results are shown in @fig-pseudo-lm, where the black dots indicate the survival probability predictions from (@eq-pseudo-value-S-lm-complications) and the solid curves are Kaplan-Meier estimates of the survival probabilities stratified by the 'complication' variable.
Despite not stratifying the Kaplan-Meier estimator used to calculate the pseudo-values, the pseudo-value predictions are still very close to the stratified Kaplan-Meier estimates.
The separation between the curves suggests that the proportional hazards assumption may not hold for this data.
However, the pseudo-value approach can accommodate such departures as the regression model estimates $\hat{\beta}_{\tau,0}$ and $\hat{\beta}_{\tau,1}$ separately for each $\tau$.
...
As a final illustration, consider adding age to the model,
...
and again the response is the identity function.
...
Therefore, even as the number of covariates increase, the pseudo-value approach remains a highly interpretable reduction.

As pseudo-values directly estimate survival probabilities, they can be evaluated using any of the metrics in Part II that can handle distribution predictions.
...
The respective estimates are $\hat{\beta}_{1000,1} \approx -230$ and $\hat{\beta}_{1000,2} \approx -3$, such that, everything else being equal, the expected RMST is reduced by around 3 days per year of age.
...
The examples in @sec-pseudo-value-S and @sec-pseudo-value-rmst assumed the identity response, $r(x) = g(x)$, for simplicity.\index{response function}
...
For example, when $r(\cdot)$ is the sigmoid, a model trained with squared error on pseudo-values $\theta_i \in \Reals$, will not necessarily minimize the error of the final probabilities $r(\hatg_\tau(\xx)) \in [0,1]$.
...
$$
\ell(\theta_i, g_\tau(\xx_i)) = -\left[\theta_i \log r(g_\tau(\xx_i)) + (1 - \theta_i) \log(1 - r(g_\tau(\xx_i)))\right],
$$

where $r(\cdot)$ is the sigmoid function and $g_\tau(\xx_i)$ is the model output on the logit scale (@tbl-pseudo-link-interpretation).
...
Despite the availability of alternative response choices, it transpires that the identity function, or post-hoc transformations, are usually sufficient when the pseudo-values are not too close to the boundary of the target space (such as near $0$ or $1$ when estimating probabilities), and may therefore be preferred for simplicity.
This was seen in @fig-pseudo-lm, where even using the identity response, the predictions still remained within the required $[0,1]$ range.
...
@tbl-pseudo-link-interpretation summarizes the most common choices when estimating survival probabilities, along with their interpretations after the response function is applied; all assume a linear fitted model $\hatg_{\tau}$, and estimated coefficients $\hat{\bsbeta}_\tau$.
<!-- TODO (ANDREAS; MINOR): REMOVED GUMBEL BELOW BECAUSE THEN WE HAVE TO GET INTO SURVIVAL VS EVENT PROBS AND IT DOESN'T FIT WITH DERIVATION BELOW IT -->

| Link $r^{-1}(\hat{\psi})$ | **Response** $r(\hatg_{\tau}(\xx))$ | Interpretation of $\hat{\beta}_{\tau;j}$ |
|---|---|---|
| Identity<br>$\hat{\psi}$ | Identity<br>$\hatg_{\tau}(\xx)$ | Absolute difference in $\hat{\psi}(\tau\mid\xx)$ per unit increase in $x_j$ |
| Logit<br>$\log\frac{\hat{\psi}}{1-\hat{\psi}}$ | Sigmoid<br>$\left[1+\exp(-\hatg_{\tau}(\xx))\right]^{-1}$ | Log-odds ratio for the event at $\tau$; $e^{\hat{\beta}_{\tau;j}}$ is an odds ratio |
| Complementary log-log<br>$\log(-\log(\hat{\psi}))$ | Complementary log-log inverse<br>$\exp(-\exp(\hatg_{\tau}(\xx)))$ | Log-hazard ratio; $e^{\hat{\beta}_{\tau;j}}$ is a hazard ratio, equivalent to Cox PH at $\tau$ |
| Log<br>$\log(\hat{\psi})$ | Exponential<br>$\exp(\hatg_{\tau}(\xx))$ | Log-survival ratio; $e^{\hat{\beta}_{\tau;j}}$ is a survival ratio |
...
A consistent estimator for the CIF is the Aalen-Johansen estimator (@sec-aalen-johanson) and pseudo-values for the CIF are thus given by:

$$
\theta_{i,q}(\tau) = n\hat{F}_{AJ,q}(\tau) - (n-1)\hat{F}_{AJ,q}^{-i}(\tau),
$${#eq-pseudo-value-cif}
...
Non-parametric estimators for state occupation probabilities can be obtained via the Aalen-Johansen estimator for multi-state models (@sec-ms-aalen-johanson).
The pseudo-values for state occupation probabilities are then defined as:

$$
\theta_{i,q}(\tau) = n\hat{P}_{AJ,q}(\tau) - (n-1)\hat{P}_{AJ,q}^{-i}(\tau),
