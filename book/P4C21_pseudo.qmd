...
Pseudo-value-based approaches are relatively new to survival analysis [see @Andersen2010 for an overview] but have quickly gained traction due to their flexibility and ease of application. 
...
Furthermore, pseudo-value analysis is most commonly used only when a few time points are of interest so $m$ is small.
...
<!--  -->
$$
E[\theta_i(\tau)\mid\xx_i] = h(f_{\tau}(\xx_i)),
...
$$
\hat\theta(\tau\mid\xx^*) = \hat{\psi}(\tau\mid\xx^*) = h(\hat{f}_{\tau}(\xx^*)).
$$

The following sections introduce concrete examples assuming $h(f(\xx)) = f(\xx)$ for illustration: survival probabilities (@sec-pseudo-value-S) and the restricted mean survival time (@sec-pseudo-value-rmst).
The choice of $h$ is then discussed in @sec-pseudo-value-response-function, where its consequences for estimation and the interpretation of covariate effects are spelled out.
...
and again assume $h(f(\xx)) = f(\xx)$ for simplicity.
...
The examples in @sec-pseudo-value-S and @sec-pseudo-value-rmst assumed the identity link, $h(f(\xx)) = f(\xx)$, for simplicity.
In general the response function $h(\cdot)$ in (@eq-pseudo-value-expectation-2) is a modeling choice with two distinct but related consequences: how the model is estimated, and how covariate effects are interpreted.

### Post-hoc vs. embedded applications

The simplest approach is to apply $h(\cdot)$ post-hoc: fit a regression model on the raw pseudo-values (e.g., minimizing squared error) and then transform the predictions.
...
For example, when $h(\cdot)$ is the sigmoid, a model trained with squared error on pseudo-values $\theta_i \in \Reals$ will not necessarily minimize the error of the final probabilities $h(\hat{f}_\tau(\xx)) \in [0,1]$.

The alternative is to embed $h(\cdot)$ into the fitting process by modifying the loss function.
...
$$
\ell(\theta_i, f_\tau(\xx_i)) = -\left[\theta_i \log h(f_\tau(\xx_i)) + (1 - \theta_i) \log(1 - h(f_\tau(\xx_i)))\right],
$$
where $h(\cdot)$ is the sigmoid and $f_\tau(\xx_i)$ is the model output on the logit scale.
...
For RMST targets, which take values in $[0, \tau]$, the log link $h(f) = \exp(f)$ is a natural choice that guarantees positive fitted values and yields coefficients interpretable as log-RMST ratios [@andersen2004regressionanalysis; @Sachs2022].
...
| Link $h^{-1}(\hat{\psi})$ | **Response** $h(\hat{f}_{\tau}(\xx))$ | Interpretation of $\hat{\beta}_{\tau;j}$ |
|---|---|---|
