...
In the competing risks setting, a quantity of particular interest is the cumulative incidence function (CIF) $F_e(\tau)$ (@eq-cif), which represents the probability of experiencing event type $e$ before time $\tau$. A suitable, univariate, unbiased estimator for the CIF is the Aalen-Johansen estimator $F_{AJ,e}(\tau)$ (@sec-aalen-johanson). The pseudo-values for the CIF are thus given by
