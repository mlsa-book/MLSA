...
#| fig-cap: "Weighting functions obtained on the `whas` dataset. x-axis is follow-up time. y-axis is outputs from one of three weighting functions: $\\KMG$, survival function based on the censoring distribution of the `whas` dataset (red), and $\\KMG^{-2}$ (green), $\\KMS$, marginal survival function based on original `whas` dataset (blue), . The vertical gray line at $t = \\tau=1267$ represents the point at which $\\hatG(t)<0.6$."
