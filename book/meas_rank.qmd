...
The category of 'discrimination measures' evaluate how well models separate observations into different risk groups.
...
The next section focuses on time-dependent measures (@sec-eval-crank-disc-auc), which are primarily AUC-type measures that evaluate discrimination over all possible unique time-points and integrate the results for a single metric.
...
Let $\dvec{\hatr}{m}$ be predicted risks, $(\tt, \dd) = ((t_1, \delta_1) \ (t_2, \delta_2) \cdots (t_m, \delta_m))^\trans$ be observed outcomes, let $W$ be some weighting function, and let $\tau$ be a cut-off time.
Then, the time-independent ('ind') *survival concordance index* is defined by,

$$
C_{ind}(\hatrr, \tt, \dd|\tau) = \frac{\sum_{i\neq j} W(t_i)\II(t_i < t_j, \hatr_i > \hatr_j, t_i < \tau)\delta_i}{\sum_{i\neq j}W(t_i)\II(t_i < t_j, t_i < \tau)\delta_i}
$$

The choice of $W$ specifies a particular variation of the c-index (see below).
The use of the cut-off $\tau$ mitigates against decreased sample size (and therefore high variance) over time due to the removal of censored observations (see @fig-ipcw) for illustration).
...
#| fig-cap: "Weighting functions obtained on the `whas` dataset. x-axis is follow-up time. y-axis is outputs from one of three weighting functions: S(t), marginal survival function based on original `whas` dataset (blue), G(t), survival function based on the censoring distribution of the `whas` dataset (red), and 1/G(t)^2 (green). The vertical gray line at t=1267 represents the point at which G(t)<0.6."
...
#| label: tbl-ipcw
#| tbl-cap: Comparing C-index measures (calculated on the `whas` dataset using a Cox model with three-fold cross-validation) with no cut-off (top) and a cut-off when $\hat{G}(t)=0.6$ (bottom). First column is Harrell's C, second is the weighting $1/\hat{G}(t)$, third is Uno's C.
...
$$
\begin{split}
&TPR_U(\hatee, \tt, \dd | \tau, \alpha) =  \frac{\sum^m_{i=1} \delta_i \II(k(\hat{\eta}_i) > \alpha, t_i \leq \tau)[\KMG(t_i)]^{-1}}{\sum^m_{i=1}\delta_i\II(t_i \leq \tau)[\KMG(t_i)]^{-1}}
\end{split}
$$
