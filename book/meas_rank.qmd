...
The next category of measures, 'discrimination measures', evaluate hwo well models separate observations into different risk groups.
...
Concordance measures generally estimate the probability of the predicted risk for one observation being lower than another if the former experience the event first *and* is not censored:

$$P(y_i > y_j | t_i < t_j \cap \delta_i)$$

From this formula it can be seen why these measures are referred to as time *independent*, once observations are paired up according to observed survival times, their discrimination is calculated according to risk aggregated over all time-points.
...
As well as these methods, another alternative C-index has been proposed by G\"onen and Heller (2005) [@Gonen2005] also proposed a C-index measure.
However, the metric is only applicable to the Cox PH model, and is therefore outside the scope of this book which focuses on generalisable measures for widespread model comparison.
...
Harrell's C is the most common C-index [@Collins2014; @Gonen2005; @Rahman2017] but several papers have shown it to be greatly affected by censoring [@Koziol2009; @Pencina2012; @Royston2013; @Uno2011].
...
On the other hand, if a poor choice is selected for $\tau$ (cutting off too late) then IPCW measures can be highly unstable [@Rahman2017], for example the variance of $C_U$ drastically increases with increased censoring [@Schmid2012].
...
As the C-index is highly dependent on censoring within a dataset, C-index values between experiments are not directly comparable, instead comparisons are limited to comparing model rankings, for example conclusions such as "model A performed significantly better than model B with respect to Harrell's C in this experiment".
...
In contrast to the measures described above, Antolini's C [@Antolini2005] provides a time-dependent formula by estimating the survival probability of both observations at the event time for the observation with the smaller outcome time:

$$P(S_i(t_i) < S_j(t_i) | t_i < t_j \cap \delta_i)$$

Note that observations are concordant when $S_i(t_i) < S_j(t_i)$ as at the time $t_i$, observation $i$ has experienced the event and observation $j$ has not, hence the expected survival probability for $S_i(t_i)$ would ideally be 0 but otherwise should be less than $S_j(t_i)$ as $j$ is still 'alive'.
