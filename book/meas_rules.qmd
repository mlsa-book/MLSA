...
$$L_{Brier}(\hatp_i, y_i) = (y_i - \hatp_i(y))^2$$
...
Recall the classification Brier score, $L(\hatp_i, y_i) = (y_i - \hatp_i(y))^2$, this provides a method to compare a evaluate a probability mass function at one time-point.
...
They are also intuitive as observation's are either weighted by $\tau$ when they are still alive and therefore still part of the sample, or by $t_i$ otherwise.
...
$$
L_{RGS}(\hatS_i, t_i, \delta_i|\KMG) = \frac{\delta_i \int_{\calT} (\II(t_i \leq \tau) - \hatF_i(\tau))^2 \ d\tau}{\KMG(t_i)}
$$
...
$$
L_{RISLL}(\hatS_i, t_i, \delta_i|\KMG) = -\frac{\delta_i \int_{\calT} \II(t_i \leq \tau)\log[\hatF_i(\tau)] + \II(t_i > \tau)\log[\hatS_i(\tau)] \ d\tau}{\KMG(t_i)}
$$
...
$$
L_{SDLL}(\hatf_i, t_i, \delta_i|\KMG) = - \frac{\delta \log[\hatf_i(t_i)]}{\KMG(t_i)}
$$
