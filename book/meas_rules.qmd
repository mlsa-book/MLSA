...
$$
L_{RGS}(\hatS_i, t_i, \delta_i|\KMG) = \frac{\delta_i \int_{\PReals} (\II(t_i \leq \tau) - \hatF_i(\tau))^2 \ d\tau}{\KMG(t_i)}
$$
...
$$
L_{RISLL}(\hatS_i, t_i, \delta_i|\KMG) = -\frac{\delta_i \int_{\PReals} \II(t_i \leq \tau)\log[\hatF_i(\tau)] + \II(t_i > \tau)\log[\hatS_i(\tau)] \ d\tau}{\KMG(t_i)}
$$
