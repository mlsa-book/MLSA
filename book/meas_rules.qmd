...
* The majority of scoring rules are *not* strictly proper, however experiments have shown that improper rules still provide useful and trustworthy results [@Sonabend2022b]
...
* Any measure that relies on the density, $f(t)$, is difficult to use in practice. The majority of survival analysis models return discrete distribution predictions due to estimation of the baseline survival with the Kaplan-Meier estimator. Hence, the probability *mass* not *density* function is returned by these models and measures such as RCLL and SDLL can often not be used in practice. Direct estimation of $f(t)$ is usually not impossible and the likelihood is termed 'intractable' [@Rindt2022].
