...
: Comparing proper ($L_P$) and strictly proper ($L_S$) scoring rules across different qualities of predictions. {#tbl-compare-scoring}
...
Two widely used losses for classification are the Brier score [@Brier1950] and log loss [@Good1952], defined respectively by
...
3. $\frac{\hat{F}^2(\tau)}{\KMG(\tau)}$ -- If the observation's outcome (censoring or event) is after $\tau$

<!-- FIXME: BELOW IS TERRIBLY WRITTEN! -->
As we have no information about the true survival status of censored observations beyond their censoring times, it is sensible to not attempt to provide a meaningful score once censored, so their contribution is $0$.
For observations that are known to have experienced the event at $\tau$, we would expect their survival probability to be zero as the event has occurred (and they cannot continue to survive) hence contributing $\hatS^2$ -- the addition of $\KMG(t)$ has the effect of placing more weight on the score at the observed event time if the proportion of censoring is lower at this time, the reason being that when the observations are alive ($t > \tau$) then their contributing the rest of the weighting after this time.
