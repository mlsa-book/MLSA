...
$$
S(\tau) = \prod_{k:t_{(k)} \leq \tau}\left(1-\frac{d_{\tbk}}{\tbk}\right)
$$ {#eq-km-ml-two}
...
$h_0,S_0,H_0$ are referred to as the baseline hazard, survival, and cumulative hazard function respectively.
...
In that case, the PH model is simply $h_{PH}(\tau|\xx_i)= h_0(\tau)$.
...
Therefore, PH models estimate the baseline risk of an event at a given time, and modulate this risk according to individual covariates.
...
So in the case where the covariate differs between subjects by $1$, the hazard ratio increases by a multiple of $\exp(\beta)$.
This yields an interpretable model, in which covariate effects (and hazard ratios) are constant over time and don't depend on $\tau$.
Moving from parameter estimation, we next consider how to actually fit the $\beta$ parameters.
...
The Cox model fits the $\beta$ terms in (@eq-ph) by ignoring the baseline hazard as a 'nuissance parameter' and using the 'partial likelihood' function [@Cox1975]:

$$
\calL(\beta) = \prod_{k=1}^m \left(\frac{\exp(\eta_{\tbk})}{\sum_{j \in \calR_{\tbk}} \exp(\eta_j)}\right)
$$ {#eq-partial}

where $\eta_{\tbk}$ and $\calR_{\tbk}$ is the linear predictor and risk set (@sec-data-rc) at the $k$th ordered event time, $\tbk$, respectively.
Note that the product is taken over uncensored observations only but the sum in the denominator is over all observations in the risk set (including if eventually censored).

The log-likelihood is then:

$$
\call(\beta) = \sum_{k=1}^m \left(\eta_{\tbk} - \log \left(\sum_{j \in \calR_{\tbk}} \exp(\eta_j)\right)\right)
$$

Given these formulae, one can already make predictions in the form of relative risks (@sec-survtsk).
To make survival distribution predictions, the baseline hazard must be estimated.
The Breslow estimator [@Cox1975] provides a way to obtain an estimate of the cumulative baseline hazard, $H_0$, using the covariate effect estimates obtained from the partial likelihood (@eq-partial):

$$
\hat{H}_{Bres}(\tau) = \sum_{k:\tbk \leq \tau} \frac{d_{\tbk}}{\sum_{j \in \calR_{\tbk}} \exp(\hat{\eta}_j)}
$$ {#eq-breslow}
...
$$
\hat{H}_{Bres}(\tau) = \sum_{k:\tbk \leq \tau} \frac{d_{\tbk}}{\sum_{j \in \calR_{\tbk}} 1} = \sum_{\tbk \leq \tau} \frac{d_{\tbk}}{n_{\tbk}} = \hat{H}_{NA}(\tau)
$$
...
One method to resolve this is to extend the model to handle time-varying coefficients and model stratification [@Cox1972], though it is surprisingly difficult to make meaningful and interpretable predictions from a time-varying model [@Reid1994].
Whilst violation of the PH assumption can be a significant problem when interpreting model covariates, the goodness of fit is less important when using the model for predictive purposes.
...
$$
\calL(\bsbeta_e) = \prod_{k=1}^m \Bigg(\frac{\exp(\eta_{\tbke})}{\sum_{j \in \calR_{\tbke}} \exp(\eta_{e;j})}\Bigg)
$$ {#eq-partial-cr}
...
:
$$
\hat{H}_{Bres;e}(\tau) = \sum_{k:\tbke \leq \tau} \frac{d_{\tbke}}{\sum_{j \in \calR_{\tbke}} \exp(\hat{\eta}_{e;j})}
$$ {#eq-breslow-cr}
...
Subsituting (@eq-hazard-sd) into the baseline hazard of the Cox model yields the Fine-Gray subhazard model for the $e$th cause [@Fine1999]:
...
$$
\calL_e(\beta) = \prod_{k=1}^m \left(\frac{\exp(\eta_{\tbke})}{\sum_{j \in \calR_{e;\tbke}} w_{kj}\exp(\eta_j)}\right)
$$ {#eq-fg-likelihood}

Where $\tbke$ is the same as in the cause-specific case.
...
The Fine-Gray model therefore uses inverse probability of censoring weighting (as in many measures in @sec-eval-crank-conc) to compensate for the effect of competing events.
