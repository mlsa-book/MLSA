...
Firstly, on low dimensional data (small number of variables), foundational methods often outperform machine learning models [@Burk2025; @Beaulac2020].
...
As predictive models, these can also be extended to other censoring and truncation types as well as event history analysis more generally from using the estimators defined in @sec-surv-estimation-non-param, @sec-aalen-johansen, and @sec-ms-aalen-johansen.
...
This yields an interpretable model, in which hazard ratios are constant over time and don't depend on $\tau$.
...
We next consider how to fit the $\beta$ parameters semi-parametrically (@sec-ph-semi-parametric) and fully parametrically (@sec-ph-parametric).
...
Information about the event times only contributes to (@eq-partial) through the index of the product and sum, thus preserving rankings, i.e., the product is taken from first to last observed event time.
...
Equation (@eq-partial) is referred to as a *partial likelihood* [@Cox1975] function, as it does not make use of all the observed data.

Equation (@eq-partial) also assumes that there are no ties in the event time, that is, no two subjects have an event at the same time.
...
However, especially in case of time-varying covariates, it is difficult to make meaningful and interpretable predictions (as the values of covariates might not be known at the time of prediction).
...
Moreover, it is unique (and Exponential as a special case) in that it has both the PH and AFT (@sec-aft) property (technically a less known representation of Gompertz also has this property).
...
$$
\begin{aligned}
\mathcal{L}(\bstheta) &= \prod_{i=1}^n h_Y(t_i|\xx_i, \bstheta)^{\delta_i}S_Y(t_i|\xx_i, \bstheta) \\
...
$$
<!--  -->
In order to estimate $\bsbeta_e$, let $\tbke, k=1,\ldots,m(e)$ be the unique, ordered event times at which events of cause $e$ occur and let $i_{e;(k)}$ be the index of the observation that experiences the $k$th event of cause $e$. Then the cause-specific partial likelihood is given by:
<!--  -->
$$
\mathcal{L}_{PL}(\bsbeta_e) = \prod_{k=1}^{m(e)} \Bigg(\frac{\exp(\eta_{e;i_{e;(k)}})}{\sum_{j \in \calR_{\tbke}} \exp(\eta_{e;j})}\Bigg),
$$ {#eq-partial-cr}

This is identical to the single-event partial likelihood in (@eq-partial), with the only difference being that the product and sum are over the unique, ordered event times for cause $e$.
...
<!--  -->
where $\Delta H_{Bres;e}(\tau)$ is the increment of the cause-specific cumulative baseline hazard between successive time points (note the missing sum over different time points compared to @eq-breslow-cr).
...
<!--  -->
$$
h^{SD}_{e}(\tau) = \lim_{\dtau \to 0} \frac{P(\tau \leq Y < \tau + \dtau, E = e\ |\ Y \geq \tau \vee (E \neq e \wedge \Delta=1))}{\dtau}.
$$ {#eq-hazard-sd}
<!--  -->
...
<!--  -->
$$
\mathcal{L}_{PL}^{SD}(\bsbeta) = \prod_{k=1}^m \left(\frac{\exp(\eta_{e;i_{e;(k)}})}{\sum_{j \in \calR_{e;\tbke}} w_{kj}\exp(\eta_{e;j})}\right),
$$ {#eq-fg-likelihood}
<!--  -->
...
2. If $t_j < \tbke$ then $w_{kj} < 1$ as $G_{KM}$ is a monotonically decreasing function and $w_{kj}$ continues to decrease as the distance between $t_j$ and $\tbke$ increases. Thus the contribution from observations that have experienced competing events reduces over time.
...
The cause-specific hazards approach for the estimation of the CIF (@sec-cr-notation and Equations @eq-all-cause-surv-prob-cox, @eq-cif-cox) is often preferred [@Austin2022b] as fitting subdistribution models requires model assumptions to hold for all causes simultaneously, which is not possible (@bonnevilleWhyYouShould2024).
...
The Weibull (and by extension Exponential) distribution is unique in that it can be parameterized as either a proportional hazards or AFT model, which means a Weibull AFT model may be suitable even when the proportional hazards assumption does not hold.
<!-- TODO (Raphael): suggest dropping this sentence. PH and AFT are two equivalent parameterizations of the same Weibull distribution — switching between them does not change the underlying distribution or relax any assumption. Also: PH is a property of covariate effects, not of the baseline distribution. Therefore the "non-monotonic ... allowing non-PH representations" comment in the next sentence is also imprecise — e.g., a Cox model can readily handle non-monotonic baseline hazards, and covariates can act in a PH fashion on a non-monotonic baseline. -->
...
When distributions are well-specified and the PH assumption is violated, AFTs can be a valid alternative to PH models [@Patel2006; @Qi2009; @Zare2015].
...
$$
\begin{aligned}
\mathcal{L}(\bstheta) &= \prod_{i=1}^n h_Y(t_i|\bstheta)^{\delta_i}S_Y(t_i|\bstheta) \\
...
### Non-linear effects {#sec-classical-nonlinear}
...
$$
\hat{\bsbeta} = \argmax \mathcal{L}(\bsbeta); \text{ subject to } \sum |\beta_j| \leq \gamma
$$
...
$$
\hat{\bsbeta} = \argmax \mathcal{L}(\bsbeta); \text{ subject to } \alpha \sum |\beta_j| + (1-\alpha) \sum \beta_j^2 \leq \gamma
$$
...
One particularly popular form of stacking in survival analysis is the 'SuperLearner' method [@vanderLaan2007], which constructs an ensemble by optimally weighting multiple candidate models based on cross-validated predictive performance.
...
* Extensions of the proportional hazards framework include more general hazard-based models that allow covariates to act simultaneously on the hazard and time scales. An example is the general hazards model of [@Chen2001]. While flexible, such models do not appear common in predictive modelling but instead preferred for inference [e.g., @Tong2013; @Rubio2019; @Rubio2022].
