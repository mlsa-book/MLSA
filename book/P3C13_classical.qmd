...
# Foundational Survival Models and Estimators {#sec-models-classical}
...
(@eq-partial) is therefore referred to as a *partial likelihood* [@Cox1975] function, as it does not make use of all the observed data.

Equation @eq-partial also assumes that there are no ties in the event time, that is, no two subjects have an event at the same time. In practice, ties can be common and several methods have been proposed to handle them, namely an exact method [@KalbfleischPrentice1973] (which is computationally expensive), the Breslow approximation [@breslowCovarianceAnalysisCensored1974], and the Efron approximation [@Efron1977]; further details are not discussed here but all three methods are readily available in openly available software.
...
Traditionally, $\hat{\bsbeta}$ is obtained using numerical optimization methods, such as Newton-Raphson or Fisher-Scoring, which require derivation the first and second derivatives of (@eq-lpartial).
...
* @Collett2014 and @KalbfleischPrentice1973 both provide comprehensive reading for foundational statistical models. The former is slightly less technical and covers extensions to multiple settings.
