...
Which is independent of $e$ and therefore identical to (@eq-riskset).
...
$$
\calL_e(\beta) = \prod_{k:t_{e;(k)}} \Bigg(\frac{\exp(\eta_k)}{\sum_{j \in \calR_{t_{e;(k)}}} w_{kj}\exp(\eta_j)}\Bigg)
$$
...
The Fine and Gray model therefore uses inverse probability of censoring weighting (as in many measures @sec-eval-crank-conc) in order to compensate for the effect of competing events.
Because of the way the risk set is defined, the denominator effectively does one of two things:

1. If $t_{e;(k)} \leq t_j$ then $w_{kj} = 1$. Thus, observations who have not experienced any event contribute the most to the likelihood.
2. If $t_{e;(k)} > t_j$ then $w_{kj} < 1$ as $\KMG$ is a monotonically decreasing function.
On the other hand as $\KMG$ is a mdecreasing function, contributions in the demoniator from observations that have experienced competing events ()

 $t_j > t_{e;(k)}$ contributions from observations that have experienced competing events diminish over time as $t_j$ increases.
