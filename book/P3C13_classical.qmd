...
There are two common methods to extend the PH model to the competing risks setting.
The first makes use of the cause-specific hazard to fit a cause-specific model, the second fits a 'subdistribution' hazard.
...
Where $h_{e;0}$ is a cause-specific hazard and $\eta_{e;i}$ is the cause-specific coefficient for observation $i$.
This model is fit by estimating $\hat{\eta}_{e;i}$ with cause-specific partial likelihood and Breslow estimator:
...
where $\tbke$ is the ordered event times for cause $e$.
There is no 'cause-specific risk set' definition, this is identical to the single event setting (see @sec-data-rc):
