...
Finally, most machine learning survival algorithms build on these foundational models as a central component, for example by using nonparametric estimators (@sec-classical-nonpar) and/or assuming a proportional hazards form (@sec-classical-cox).
...
The same idea of using a nonparametric featureless estimator as a predictive baseline follows for other censoring and truncation types (@sec-surv-estimation-non-param) and event history analysis more generally (@sec-aalen-johansen).
...
<!-- FIXME (RAPHAEL): is the $\ell_i$ notation for the likelihood contribution still correct/consistent given the likelihood-definition changes in the survival chapter (@sec-surv-estimation-param)? -->
$$
\ell^{Cox}_{i_{(k)}}
...
This extends the cause-specific hazard (@eq-cause-specific-hazard), which conditions only on remaining event-free ($E(\tau-) = 0$), by additionally retaining in the conditioning set those who have already experienced a competing event (who can never transition to $q$).
...
This mirrors the proportional hazards survival form $S_{PH} = S_0(\tau)^{\exp(\eta_i)}$ (@eq-ph-surv): here $1 - F_q(\tau)$ plays the role of the survival function (in place of the all-cause survival), so the hazard-survival relationship (@eq-surv-haz) gives $1 - F_q(\tau \mid \xx_i) = (1 - F^{SD}_{q;0}(\tau))^{\exp(\eta_i)}$, which rearranges to (@eq-fg-cif).
...
When $Q>1$, if no observation that has experienced a competing event is in the risk set at any cause-$q$ event time, all weights equal $1$ and (@eq-fg-likelihood) coincides with the cause-specific Cox partial likelihood (@eq-partial-cr).
When $Q=1$ (no competing events), the subdistribution risk set (@eq-riskset-sd) reduces to the standard risk set (@eq-risk-set) --- only the first condition can ever hold --- and the subdistribution hazard reduces to the usual hazard, exactly recovering the single-event Cox PH model (@eq-ph).
...
Finally, fitting multiple subdistribution models requires the PH assumption to hold simultaneously across all causes, which is often difficult to justify and may even be impossible: the cause-specific CIFs must sum to at most one, so proportional subdistribution hazards cannot hold for every cause at once [@Grambauer2010; @bonnevilleWhyYouShould2024].
Even when the subdistribution PH assumption is misspecified, however, the estimated subdistribution hazard ratio still provides a useful summary of a covariate's effect on the cause-specific CIF [@Grambauer2010].
...
Nevertheless, under the proportional hazards assumption it is not possible to recover the *shape* of the AFT curve --- the only exception being the Weibull distribution, which can be expressed as both a PH and an AFT model.
...
Firstly, $\exp(-\eta_i)$ is modeled instead of $\exp(\eta_i)$, so the linear predictor relates to risk in the opposite direction to the PH model: a larger $\eta_i$ lengthens survival ($S_{AFT}(\tau \mid \xx_i) > S_{AFT}(\tau \mid \xx_j)$ for $\eta_i > \eta_j$, since $S_0$ is decreasing and $\tau e^{-\eta_i} < \tau e^{-\eta_j}$), whereas in a PH model a larger $\eta_i$ raises the hazard, reducing survival. 
Interpreting the AFT linear predictor as a risk score therefore reverses the book's 'high value, high risk' convention, so its sign should be flipped before it is used for relative-risk prediction.
...
<!-- FIXME (RAPHAEL): here and elsewhere, please use $Y$ (the variable of interest, introduced earlier) rather than $X$ for the random variable. $X$ was introduced as the covariate vector/random variable, and here it is introduced as the survival time, then one line later $x$ switches roles to a feature/covariate --- very confusing for beginners. Suggest $Y \sim \Exp(\lambda)$, $h_Y$, $S_Y$. -->
...
<!-- FIXME (RAPHAEL): here and elsewhere, if you mean a specific feature use $x_p$ (with coefficient $\beta_p$) rather than $x_i$/$\beta_i$ --- the $i$ index is reserved for subject $i$. ($\xx_i$ above, the covariate vector for subject $i$, is correct.) -->
...
When $\tau \rightarrow \infty$, it holds that $S_0(\tau) \rightarrow 0$ and (@eq-po-ratio) $\rightarrow 1$.
...
<!-- TODO (RAPHAEL): why partial likelihood / semiparametric here? The section introduces only the general PO form (in parallel with PH and AFT), and a semiparametric PO model is never set up --- so jumping to "the partial likelihood of a semiparametric proportional odds model" is unmotivated. Coming straight after AFT, a reader would expect the fully parametric treatment. Please clarify or restructure (e.g. state up front that PO is used parametrically, with the semiparametric route only noted in passing). -->
...
where $\bsnu$ are spline coefficients to be fitted by maximum likelihood estimation, $\mathbf{k}$ are the positions of $K$ knots, and $s$ is the restricted cubic spline function in log time defined by, \index{splines}
...
where the $B_{jm}$ are the basis functions and the $\nu_{jm}$ are the corresponding basis coefficients to be estimated.
Many basis systems can be used, including natural cubic splines, B-splines and the closely related penalized B-splines (P-splines) [@Eilers1996], and thin-plate regression splines [@Wood2003]; simpler choices such as step functions or polynomial bases are also possible (see @Wood2017 for a comprehensive treatment). \index{splines}

Because a rich basis can easily overfit, estimation is usually combined with a penalty on the basis coefficients that discourages overly wiggly fits.
Collecting all parameters in $\bstheta$, the functions are then estimated by optimizing a penalized criterion of the general form
...
A common choice penalizes squared differences between adjacent coefficients [@Eilers1996], for a single function $f_j$,

$$
P(\bstheta) = \sum_{m=2}^{M_j} (\nu_{jm} - \nu_{j,m-1})^2,
$$

which shrinks neighbouring coefficients towards one another and thereby penalizes abrupt changes in $f_j$.
As $\xi \to 0$ the estimate approaches the unpenalized fit, whereas a large $\xi$ forces each $f_j$ towards a straight line.
Flexible parametric models like the Royston-Parmar model (@sec-flexible) are additive models that use splines to model the baseline hazard, and are implemented in the $\texttt{flexsurv}$ package [@pkgflexsurv]. Additive Cox models, which instead estimate smooth functions of the covariates directly within the proportional hazards framework, are described in @Hastie1990.

The constructions above make the predictor considerably more flexible, but they still assume each effect is a smooth function of a single covariate, rule out interactions between covariates, and require the predictor to be prespecified --- the modeler must decide in advance which features are modeled via splines and how.
More generally, the predictor can be written as

$$
\eta = g(\xx \mid \bstheta),
$$ {#eq-general-predictor}

where $\bstheta$ collects all parameters of the model and $g$ is a general function of the features that may include non-linear effects, interactions, and non-smooth components.
Machine learning models such as random forests (@sec-ranfor), gradient boosting (@sec-boost), and neural networks (@sec-nnet) are designed to learn such functions directly from data, without the need to prespecify the range of possible effects.
...
Wrappers fit multiple models on subsets of variables and select the best-performing subsets. This is often computationally infeasible in the context of very large datasets.
This section does not consider general algorithms such as principal component analysis, see further reading below for suggested additional material that covers these areas.
...
* Foundational statistical models broadly fall into three categories: nonparametric estimators, semiparametric models that include a non-parametric baseline estimate, and fully-parametric models.
...
* Kernel-based approaches not covered here have been suggested for smooth nonparametric estimates of the baseline hazard (@Gefeller1992; @Muller1994).
