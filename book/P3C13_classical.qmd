...
This estimator is the analogue to the sample mode and sample mean baseline estimators in classification and regression respectively.
...
The formulation in (@eq-akritas) can be turned into a baseline predictive model by treating estimation of $\hatF_X$ on training data as the learning algorithm, and then using (@eq-akritas) as the prediction algorithm (@sec-ml-models).

## Proportional hazards {#sec-classical-cox}

This section begins with an introduction to the proportional hazards concept, introduces estimation with the Cox PH model (@sec-ph-semi-parametric), and then moves to fully parametric proportional hazards models (@sec-ph-parametric), with the Weibull model as a motivating example.
...
Instead of modelling a separate intercept, the _baseline hazard_ represents the hazard when the linear predictor is zero, hence the term _baseline_. 
...
It can be seen from (@eq-ph) that time is only incorporated via the baseline hazard [see @vanHouwelingen2011 for extensions to time-varying covariates].
...
For a single covariate $x$, the _hazard ratio_ is:
...
which depends on $\bsbeta_i$ via $\eta_i=\xx_i^\trans\bsbeta$.
Note how the baseline hazard $h_0$ cancels out in the likelihood contribution and removes the dependency on time.
For the estimation of $\bsbeta$, the baseline hazard can be considered a "nuisance parameter" and the likelihood for the entire data set can be defined as:
...
In practice, ties can be common and several methods have been proposed to handle them, including an exact (but computationally expensive) method [@Kalbfleisch1973], the Breslow approximation [@breslowCovarianceAnalysisCensored1974], and the Efron approximation [@Efron1977]; further details are not discussed here but all three methods are readily accessible in openly available software.
...
The Breslow estimator is identical to the Nelson-Aalen estimator (@eq-nelson-aalen) when there are no covariates, when covariate values are identical, or when all are zero-valued:
...
However, there are some cases where modelling a particular distribution is sensible.
On these occasions, a particular probability distribution of the event times is assumed, with three common choices being the exponential, Gompertz, and Weibull distributions [@Kalbfleisch1980; @Wang2017].
...
Parameters can then be fit using maximum likelihood estimation with respect to all unknown parameters $\bstheta = \{\bsbeta, \gamma, \lambda\}$.
...
The competing risks partial likelihood (@eq-partial-cr) is identical to the single-event partial likelihood in (@eq-partial), with the only difference being that the product and sum are over the unique, ordered event times for cause $q$.

Using the same logic as in the single-event case, the Breslow estimator follows from (@eq-breslow):
...
A final prediction of the cumulative incidence function follows by (@eq-cif) where the cause-specific hazard function is the derivative (or a discrete approximation) of (@eq-cause-specific-cumu-hazard-cox) and the all-cause survival probability follows by (@eq-all-cause-surv-prob): 
...
<!-- TODO (ANDREAS; MAJOR): CHECK BELOW CORRECT AND CONSISTENT WITH NOTATION IN EHA; q_i notation is easier but then conflicts with E(tau) notation below -->
$$
...
@Fine1999 proposed a proportional hazards formulation of the subdistribution hazard (@eq-hazard-sd) as:
...
This implies that the proportional hazards assumption must hold for each subdistribution.
...
Finally, prediction of the subdistribution cumulative hazard follows by updating the Breslow estimator (@eq-breslow) to incorporate the subdistribution risk set definition and applying the same weighting as in (@eq-fg-likelihood) to compensate for competing events: 
...
In such settings, coefficients from a subdistribution model may be more intuitive to interpret than those from cause-specific models as they directly relate covariates to event incidence via (@eq-fg-cif), which is often the quantity of primary interest [@Austin2017b].

## Accelerated failure time {#sec-surv-models-param}

While the proportional hazards models is a powerful model, it often does not represent real-world phenomena well.
...
Similarly, while its theoretically possible to fit cause-specific AFT models for the competing risks setting, this does not appear common in practice.
...
Consider the lifespans of humans and small dogs. Suppose small dogs age five times faster than humans.
...
If instead, one assumes a constant hazard ratio then at age $\tau$ (the PH assumption), the dog's risk is five times the humans (@eq-ph-surv):
...
Firstly, $\exp(-\eta_i)$ is modelled instead of $\exp(\eta_i)$, therefore for values $\eta_i > \eta_j$ it holds that $h_{PH} > h_{PH}$ but $h_{AFT} < h_{AFT}$: larger values of $\eta_i$ increase survival time and thus decrease relative risk.
...
Proportional odds (PO) models [@Bennett1983] are the final of the three major linear model classes in survival analysis.
...
Instead, they are more commonly found as components within neural networks (@sec-nnet) or in flexible parametric models (@sec-flexible).
...
There is no simple closed form expression for the partial likelihood of a semi-parametric proportional odds model.
...
To see how the proportional hazards RP model relates to the Weibull distribution, first integrate and take logs of the Weibull-PH hazard function from (@eq-ph-weibull-haz):
...
The Royston–Parmar model can be fit via maximum likelihood estimation, making estimation of smooth survival functions under this framework straightforward using readily available off-the-shelf software.
...
Consider modelling the time until disease progression with covariates representing age (continuous) and treatment (binary):
...
The functions $f_j$ are often chosen to be natural splines, but step functions, polynomial bases, or any other transformation can be used.
...
In these cases, using all variables for modeling results in worse interpretability, increased computational complexity, and often inferior model performance as model's tend to overfit the training data and generalize poorly to new data.
...
Deciding between lasso and ridge can also be automated by using elastic net [@Simon2011;@Zou2005], which is a convex combination of $\ell^1$ and $\ell^2$ penalties such that $\bsbeta$ is estimated as:

$$
\hat{\bsbeta} = \argmax \mathcal{L}(\bsbeta); \text{ subject to } \alpha \sum \vert \beta_j \vert + (1-\alpha) \sum \beta_j^2 \leq \gamma,
$$

where $\alpha$ is a hyper-parameter to be tuned (@sec-ml-opt).

#### Filter methods {.unlisted .unnumbered}
Filter methods are a two-step process that score features according to some metric and then select either a given number of top-performing features (those with the top scores) or those where the score exceeds some threshold.
...
Model-based boosting fits a sequence of models that iteratively improve predictive performance by correcting errors from previous iterations.
...
These _pseudo-residuals_ are the negative gradient (@sec-ml-gradient-descent) of a chosen loss function evaluated at the previous iteration; this process is revisited to in detail in @sec-boost.
...
\vdots & \ddots & \vdots \\
\hat{\eta}^{(1)}_{m} & \hat{\eta}^{(2)}_{1} & \hat{\eta}^{(3)}_{m} \\
\end{bmatrix},
$$
...
* Foundational statistical models broadly fall into three categories: non-parametric estimators, semi-parametric models that include a baseline estimate, and fully-parametric models.
...
* Unregularized models perform badly on high-dimensional data without some form of pre-processing, however this is relatively simple with modern off-shelf software such as $\texttt{sklearn}$ [@pkgsklearn] and $\texttt{mlr3}$ [@pkgmlr3].
...
* Extensions of the proportional hazards framework include more general hazard-based models that allow covariates to act simultaneously on the hazard and time scales. An example is the general hazards model of [@Chen2001]. While flexible, such models do not appear common in predictive modelling but instead preferred for inference [for example, @Tong2013; @Rubio2019; @Rubio2022].
