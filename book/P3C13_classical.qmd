...
In a predictive setting it can be easy to dismiss 'traditional models' and favour testing of more modern 'machine learning' tools; this would be a mistake.
Firstly, on low dimensional data (small number of variables), traditional methods often outperform machine learning models [@Burk2025; @Beaulac2020].
Secondly, even on high-dimensional data, several papers have demonstrated that augmenting traditional models (@sec-classical-improving) can yield models that outperform machine learning alternatives [@Zhang2021; @Spooner2020].
Finally, the majority of machine learning survival algorithms make use of these traditional models, for example by using non-parametric estimators (@sec-classical-nonpar) and/or assuming a proportional hazards form (@sec-classical-cox), as a central component to construct an algorithm around.
...
Cox's idea was to define the likelihood of the observed data in terms of the probability that subject $i_{(k)}$ experienced the event at $\tbk$ given that someone in the risk set $\calR_{\tbk}$ experienced the event, and to relate this probability to hazards @eq-ph. 
Hence, the likelihood contribution of the $k$th event is given by

$$\begin{align*}
\ell^{Cox}_{i_{(k)}} 
 & =\frac{h_0\left(\tbk\right)\exp\left(\eta_{i_{(k)}}\right)}
{\sum_{j\in \calR_{\tbk}} h_0\left(\tbk\right)\exp\left(\eta_j\right)}=\frac{\exp\left(\eta_{i_{(k)}}\right)}
...
In Equation (@eq-partial), the information of the event times is only used in the index of the product, thus the ranks of the occured events (and the associated covariate values) are preserved, but the baseline hazard $h_o$ and therefore the exact event time information is absent from the function. This likelihood is therefore referred to as *partial likelihood* [@Cox1975] function, as it doesn't use all of the information about the observed data.
The partial likelihood is evaluated only at observed event times, but the sum in the denominator is over all observations in the risk set (including eventually censored).
@eq-partial also assumes that there are no ties in the event times (that is no two subjects have an event at the same time). In practice, ties are quite common and several methods have been proposed to handle them, namely an exact method [@KalbfleischPrentice1973] (which is computationally expensive), Breslow approximation [@breslowCovarianceAnalysisCensored1974], and Efron approximation [@Efron1977].

The log-partial likelihood, usually prefered for optimization, is given by
<!--  -->
$$
\mathcal{l}_{PL}(\bsbeta) = \sum_{k=1}^m \left(\eta_{i_{(k)}} - \log \left(\sum_{j \in \calR_{\tbk}} \exp(\eta_j)\right)\right),
$${#eq-lpartial}
<!--  -->
such that 
<!--  -->
...
Traditionally, $\hat{\bsbeta}$ is obtained using numerical optimization methods, such as Newton-Raphson or Fisher-Scoring, which require derivation the first and second deriviatives of @eq-lpartial.
...
<!--  -->
$$
\calL_{PL}(\bsbeta_e) = \prod_{k=1}^{m(e)} \Bigg(\frac{\exp(\eta_{e;i_{e;(k)}})}{\sum_{j \in \calR_{\tbke}} \exp(\eta_{e;j})}\Bigg),
$$ {#eq-partial-cr}
...
<!--  -->
$$
\calL_{PL}^{SD}(\bsbeta) = \prod_{k=1}^m \left(\frac{\exp(\eta_{e;i_{e;(k)}})}{\sum_{j \in \calR_{e;\tbke}} w_{kj}\exp(\eta_{e;j})}\right),
$$ {#eq-fg-likelihood}
<!--  -->
