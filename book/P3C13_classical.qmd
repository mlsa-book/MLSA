...
Note that in Equation (@eq-partial), the information of the event times is only used in the index of the product, thus the ranks of the occured events (and the associated covariate values) are preserved, but the baseline hazard $h_o$ and therefore the exact event time information is absent from the function. This likelihood is therefore referred to as *partial likelihood* [@Cox1975] function, as it doesn't use all of the information about the observed data.
