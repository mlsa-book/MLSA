...
Cause-specific hazards (@eq-cause-hazards) represent the instantaneous risk of an individual experiencing the cause of interest, given that they have not yet experienced *any* event.
...
In practice, the difference comes from the definition of the risk set.
...
Which is identical to @eq-risk

Whereas the subdistribution risk set is defined as:

$$
\calR_{e;\tau} := \{i: t_i \geq \tau \cup (t_i \leq \tau \cap e_i \neq e)\}
$$


Recall from  the hazard for some event $e$ is defined by:

$$
h_{e}(\tau) = \lim_{\Delta \tau \to 0} \frac{P(\tau \leq Y \leq \tau + \Delta \tau, E = e\ |\ Y \geq \tau)}{\Delta \tau}
$$

Substituting the cause-specific hazard into the semi-parametric PH form (@eq-cox) yields the Fine and Gray model for the subhazard function of the $e$th cause:

$$
h_{FG,e}(\tau|\xx_i)= h_{e_0}(\tau)\exp(\eta_i) 
$$

This model form assumes that the subdistribution hazards are all proportional.
Estimating the $\beta$ coefficients follows from a weighted, cause-specific adaptation of the partial likelihood (@eq-partial).
For the $e$th cause, the partial likelihood is

$$
\calL_e(\beta) = \prod_{k:t_{e;(k)}} \Bigg(\frac{\exp(\eta_k)}{\sum_{j \in \calR_{t_{e;(k)}}} \exp(\eta_j)}\Bigg)
$$

<!-- FIXME: ANDI ARE YOU HAPPY WITH THIS NOTATION? IF SO SHOULD WE ADD TO EHA CHAPTER? -->
Where $t_{e;(k)}$ is the ordered event times for observations that experience event $e$.
A quirk of the competing risk setting is that the risk set includes observations that experienced a competing event

$$
\calR_{e;\tau} := \{i: t_i \geq \tau \cup (t_i \leq \tau \cap e_i \neq e)\}
$$

Estimation of the cause-specific baseline hazard, $h_{e_0}$ 
...
Moving from a PH to AFT framework can be confusing, so to elucidate this further, take the following example adapted from @Klein2003.
