...
This mirrors the proportional hazards survival form $S_{PH} = S_0(\tau)^{\exp(\eta_{q;i})}$ (@eq-ph-surv) but with $1 - F_q(\tau)$ playing the role of the survival function in place of the all-cause survival.
...
$$
\hat{F}_{FG;q}(\tau \mid \xx_i) = 1 - \exp\left(-\hat H^{SD}_{Bres;q}(\tau)\exp(\hat{\eta}_{q;i})\right).
$$
...
This is often computationally infeasible in the context of very large datasets and as such does not generalize well to survival analysis.
