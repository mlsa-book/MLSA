...
In particular, the focus of this chapter is on machine learning concepts including tasks\index{task} (introduced in a survival context in @sec-survtsk), learners\index{learner}, loss functions\index{loss}, resampling, and evaluation\index{model evaluation}, which are essential for understanding how survival analysis problems can be formulated and evaluated using machine learning.
...
Let $f : \calX \rightarrow \calY$ be the regression task of interest with $\calX \subseteq \Reals$ (a single feature) and $\calY \subseteq \Reals$.
...
The simplest method to estimate the generalization error is to use *holdout resampling*\index{resampling}\index{resampling!holdout}, which is the process of partitioning the data into one training dataset and one testing dataset, with the model trained on the former and predictions made for the latter.
