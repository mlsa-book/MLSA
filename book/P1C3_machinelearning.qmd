...
* Unsupervised clustering\index{clustering}: Categorize observations into clusters.
* Reinforcement learning\index{reinforcement learning}: Learn to speak like a clinician depending on client diagnosis.
...
In machine learning literature, deterministic regression is more common than probabilistic and hence the shorthand 'regression' is used to refer to deterministic regression.
...
$$
\hatg(x) = \hat{\beta}_0 + \hat{\beta}_1 x
$$
...
Say that $\gamma = 2$ is set and the learner is then trained by passing $(\xx, \yy)$ to the learning algorithm and thus estimating $\hat{\bstheta}$ and $\hatg$.
...
A general principle used by many learning algorithms to obtain the model parameters $\bstheta$ during training is *gradient descent*.
The idea is simple: given a differentiable loss function $L(\bstheta)$ that measures how well the model predicts the training data, repeatedly take a small step in the direction that decreases the loss until no further improvement is possible.
This principle underlies, among others, gradient boosting (@sec-boost) and neural networks (@sec-nnet), but it is easiest to introduce on a simple linear regression model with only two parameters.

Consider the linear regression model $y = g(x \mid \bstheta) = \beta_0 + \beta_1 x$ with parameter vector $\bstheta = (\beta_0, \beta_1)^\trans$, fitted to $n$ observations $(x_i, y_i)$ by minimising the *mean squared error* loss
$$
L(\bstheta) = \frac{1}{n}\sum_{i=1}^n \bigl(y_i - \beta_0 - \beta_1 x_i\bigr)^2.
$$ {#eq-ml-mse}
For this loss the optimum $\hat{\bstheta} = \argmin_{\bstheta} L(\bstheta)$ has a closed form (the OLS normal equations), so gradient descent is not actually needed here.
We use it only as a transparent setting in which every step of the procedure can be checked against the known optimum.

The key quantity is the *gradient* $\nabla_{\bstheta} L = \bigl(\partial L / \partial \beta_0,\, \partial L / \partial \beta_1\bigr)^\trans$, the vector of partial derivatives of the loss with respect to each parameter.
At any point $\bstheta$, the gradient points in the direction of steepest *increase* of $L$.
To decrease the loss, we therefore take a step in the *opposite* direction,
$$
\bstheta^{t+1} = \bstheta^{t} - \alpha \,\nabla_{\bstheta} L\bigl(\bstheta^{t}\bigr),
$$ {#eq-ml-gd}
where $\alpha > 0$ is the *learning rate* (or step size).
Starting from an arbitrary initial guess $\bstheta^{0}$ and applying @eq-ml-gd repeatedly drives $\bstheta$ towards a minimum of $L$.

For the MSE loss in @eq-ml-mse the partial derivatives are
$$
\begin{aligned}
...
\end{aligned}
$$
so each gradient-descent step uses the current residuals $r_i = y_i - \beta_0^{t} - \beta_1^{t} x_i$ to update the intercept and slope simultaneously.

@fig-gradient-descent illustrates the procedure for a concrete numerical example.
The contours shows the loss surface of $L(\bstheta)$ over the two-dimensional parameter space; lighter colours correspond to smaller loss.
The black cross marks the OLS optimum.
Starting from a deliberately poor initial guess (green square), gradient descent traces the blue path along the loss surface and reaches the optimum (red circle) — at every step the update moves in the direction $-\nabla_{\bstheta} L$, and the step size automatically becomes smaller as the gradient shrinks near the optimum.

![Gradient descent on a two-parameter linear regression problem. Contours and colour show the MSE loss $L(\bstheta)$ from @eq-ml-mse in the parameter space $(\beta_0, \beta_1)$. The black cross marks the closed-form OLS optimum. From the start point (green square), gradient descent follows the blue path of @eq-ml-gd, ending close to the optimum (red circle).](Figures/introduction/gradient-descent.png){#fig-gradient-descent fig-alt="Filled contour plot of the loss over the two-dimensional parameter space (intercept on the horizontal axis, slope on the vertical axis), shaded from yellow at low loss to dark red at high loss. The OLS optimum is marked with a black cross in the upper-left region. A green square in the lower-right marks the starting point; a blue path runs from the green square up and to the left, ending at a red circle that essentially coincides with the OLS cross." width=70%}

The learning rate $\alpha$ controls how far each step moves: too small and convergence is slow, too large and the iterates can overshoot or oscillate.
In simple settings such as this one a single $\alpha$ chosen by hand is enough; in more complex models it becomes a tuning parameter in its own right.
...
More generally, losses compare an observed outcome with a prediction object, which could be a numeric value, a class probability vector, or a predicted distribution.
