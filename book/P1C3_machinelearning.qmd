...
$$
\hatg(x \mid \bstheta) = \hat{\beta}_0 + \hat{\beta}_1 x
$$
...
This principle underlies, among others, gradient boosting\index{boosting} (@sec-boost) and neural networks (@sec-nnet),\index{neural networks} but is simpler to introduce for a simple linear regression model\index{linear regression} with only two parameters.
...
In machine learning models $\alpha$ is typically tuned (@sec-ml-opt). \index{tuning}
...
@fig-gradient-descent illustrates gradient descent for a different example with more observations, comparing the gradient descent procedure to the known closed form optimum.
