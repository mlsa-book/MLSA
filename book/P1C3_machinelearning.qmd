...
The basic machine learning workflow is represented in @fig-p1c3-ml-basic:
...
![Basic machine learning workflow. The available data $\mathcal{D}$ is split into a training and a test set. A learner is trained on the training data, inducing a fitted model $\hat{g}$; the model is applied to the test features $\mathbf{X}_*$ to produce predictions $\hat{\mathbf{y}}_*$, which are compared with the test labels $\mathbf{y}_*$ by a performance measure.](Figures/ml/fig-p1c3-ml-basic.png){#fig-p1c3-ml-basic width=100% fig-align="center" fig-alt="Flow diagram in four stages from left to right. Data splitting: a Data hexagon branches into a Training data hexagon and a Test data hexagon. Training: the training data feeds a Learner box, which produces a Model box. Prediction: an arrow from the model and an arrow from the test Features hexagon meet at a junction that forms the Predictions hexagon. Evaluation: the predictions and the test Labels hexagon feed a Measure box, which outputs a Performance hexagon."}
...
The performance from each fold is averaged into a final performance estimate (@fig-p1c3-cv).
...
![Three-fold cross-validation. The data is partitioned into three folds; across the three iterations each fold is held out once for testing (highlighted) while the remaining two folds are used for training. The same learner is trained on every fold's training data, yielding a fold-specific model $\hat{g}^{(k)}$ that is evaluated on the held-out fold via a performance measure. The three per-fold measures are averaged into a single cross-validation performance estimate.](Figures/ml/fig-p1c3-cv.png){#fig-p1c3-cv width=100% fig-align="center" fig-alt="Diagram with three rows, one per fold. In each row the data is drawn as three hexagons, two training folds and one test fold, with the held-out test fold in a different position in each row. The training folds of every row connect, via a bracket per fold, into a single shared Learner box in the centre. The Learner emits three Models, one per fold row. For each fold the test fold supplies features and labels (dotted lines) that combine with the model output in a Measure box, and the three Measure boxes feed a single CV performance hexagon (mean score) on the right."}
...
*Nested resampling*\index{resampling!nested} reduces the optimistic bias that arises when the same data is used both to tune hyperparameters and to estimate performance [@Simon2007].
It nests two resampling loops.
In the *outer* loop, the data is split by an outer resampling scheme (for example, three-fold CV as in @fig-p1c3-cv) into outer-training and outer-test sets that are used to estimate performance.
Each outer-training set is then resampled again in an *inner* loop (for example, four-fold CV), and this inner resampling is used only for tuning: the candidate hyperparameter configurations are compared on the inner folds, which never include the corresponding outer-test set.
The best configuration found in the inner loop is then used to refit the model on the entire outer-training set, and this refitted model is evaluated once on the held-out outer-test set.
Averaging the outer-test scores over all outer folds yields an approximately unbiased estimate of the performance of the entire modelling pipeline, tuning included.
...
In machine learning models $\alpha$ is typically tuned\index{tuning} (@sec-ml-opt) or adaptive and combined with early stopping\index{early stopping} (@sec-ml-early-stopping).
...
@fig-p1c3-gradient-descent illustrates gradient descent for an example with more observations, comparing the gradient descent procedure to the known closed form optimum.
