...
In particular, the focus of this chapter is on machine learning concepts including tasks (introduced in a survival context in @sec-survtsk), learners, loss functions, resampling, and evaluation, which are essential for understanding how survival analysis problems can be formulated and evaluated using machine learning.
...
Whereas the learner defines the algorithm for training and prediction, the model is the result of training the algorithm on data.
...
Consider the linear regression model $y = g(x \mid \bstheta) = \theta_0 + \theta_1 x$ with parameter vector $\bstheta = (\theta_0, \theta_1)^\trans$, fitted to $n$ observations $(x_i, y_i)$ by minimising the *mean squared error* loss
$$
L(\bstheta) = \frac{1}{n}\sum_{i=1}^n \bigl(y_i - \theta_0 - \theta_1 x_i\bigr)^2.
$$ {#eq-ml-mse}
...
The key quantity is the *gradient* $\nabla_{\bstheta} L = \bigl(\partial L / \partial \theta_0,\, \partial L / \partial \theta_1\bigr)^\trans$, the vector of partial derivatives of the loss with respect to each parameter.
...
\end{aligned}
$$
so each gradient-descent step uses the current residuals $r_i = y_i - \theta_0^{t} - \theta_1^{t} x_i$ to update the intercept and slope simultaneously.
...
A central component of machine learning best practice is rigorous, empirical model evaluation.
Building a complex model does not guarantee strong predictive performance.
Moreover testing performance on a single dataset only provides very limited insight into how  model will generalize to new data.
This issue is especially pronounced in modern pre-trained generative AI models, where there is often an implicit assumption by users that these models will perform well across a wide range of domains without task-specific validation.
Trusting a model's performance requires two elements: firstly, quantifying performance using appropriate and interpretable measures; secondly, clearly specifying what exactly is being assessed and how.
...
Given data, $(\XX^*, \yy^*)$, and a trained model, $\hatf$, the loss is given by $L(\hatf(\XX^*), \yy^*) = L(\hatyy, \yy^*)$.
<!-- TODO (Raphael): the "However ... new, previously unseen data" framing doesn't match the notation here — $(\XX^*, \yy^*)$ already denotes new/unseen observations (cf. line 94 and the prediction-algorithm example, line 127). -->
However, for a model to be _truly_ useful, it should perform well _in general_, meaning it should perform well on new, previously unseen data, rather than just the data encountered during training and development.
A model's _generalization error_ refers to its estimated expected performance on new data.
...
Another benefit of data splitting is to help mitigate model overfitting, which occurs when models learn noise in the training data leading them to perform worse on new data.
This can be avoided using data splitting methods but also feature engineering (manually or using built-in machine learning methods) to remove noisy features, and early stopping which stops training a model once a certain performance has been reached.
<!-- TODO (Raphael): the claim that removing noisy features mitigates overfitting is not quite right — a sufficiently flexible learner can still memorize the training data even on a clean feature set. The actual prevention mechanism is capacity control / regularization. Also data splitting per se does not prevent overfitting, it only lets you detect/quantify it. -->
...
Other resampling mechanisms may also be useful for event history analysis (@sec-eha).
For example, stratifying with respect to different competing risks, or using more complex resampling schemes that can handle non-independent observations for recurrent events data [@Hornung2023].
...
Determining if one model is actually better than another is a surprisingly complex topic [@Benavoli2017; @Demsar2006; @Dietterich1998; @Nadeau2003; @SchulzKuempel2025] and is out of scope for this book, instead any benchmark experiments performed in this book are purely for illustrative reasons and no results are expected to generalize outside of these experiments.
<!-- TODO (Raphael): I don't think we have any benchmark experiments in the book? -> so no need to call it out -->
...
<!-- TODO (Raphael): I suggest to drop the abstract loss definition and just write out the actual one with y*, yhat -->
...
<!-- TODO (Raphael): "for now assume the latter" implies the range case will be revisited, but it never is. Either drop the qualifier (and merge into the next sentence) or actually contrast the two later. -->
