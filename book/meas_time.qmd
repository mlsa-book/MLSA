...
In fact, to our knowledge, there are no specialised 'survival time measures', instead regression measures are used once censored observations have been removed.
...
Note that all of these, assume that the error for an over-prediction ($\hatt > t$) should be equal to an under-prediction ($\hatt < t$).
