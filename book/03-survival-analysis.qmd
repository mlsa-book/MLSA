...
The exact event time is unknown but it is known that, if the event occurs, this will be after the observed censoring time, hence *right*-censoring (imagine a timeline from left to right as in @fig-survset-censor).
...
Dotted lines in @fig-km-tumor indicate the median survival\index{median survival time}, defined as the time at which the survival function reaches $S(\tau) = 0.5$. 
