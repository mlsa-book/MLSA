## Output: book/Figures/reductions/fig-p4c22-discrete-time-glm-interaction.png
## Discrete-time survival reduction with interval x complications interaction
## (logistic regression) vs Kaplan-Meier on the `tumor` data.
...
## Interaction: separate baseline hazard per complications group
...
surv_data <- bind_rows(
  data.frame(complications = unique(tumor_comp$complications),
             time = 0, survival = 1, model = "Discrete Time (GLM, Interaction)"),
  surv_data
) |>
...
p <- ggplot(plot_data, aes(x = time, y = survival,
  ...
  geom_line(data = filter(plot_data, model == "Discrete Time (GLM, Interaction)"),
            linewidth = 1.2) +
  ...
  scale_linetype_manual(
    ...
    labels = c("Kaplan-Meier" = "Kaplan-Meier",
               "Discrete Time (GLM, Interaction)" = "Discrete Time (GLM)")
  ) +
...
save_fig(p, "book/Figures/reductions/fig-p4c22-discrete-time-glm-interaction.png",
         width = 6, height = 6)
