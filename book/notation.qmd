...
| $(\XX, \tt, \dd)$ | Survival data where $\XX \in \Reals^{n \times p}$ is a real-valued matrix of $n$ observations (rows) and $p$ features (columns), $\tt \in \Reals^n$ is a vector of observed outcome times, and $\dd \in \Reals^n$ is a vector of observed outcome indicators. |
| $\bb$|Vector of model coefficients/weights, $\bb \in \Reals^p$. |
