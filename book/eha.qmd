...
From this more general point of view such data is sometimes refered to *event-history data* and its analysis as *event-history analysis* (EHA).
...
- *all-cause hazard*, $h(\tau) = \sum_{e = 1}^{k}h_{e}(\tau)$, which is the hazard for any event occuring
- *all-cause cumulative hazard*: $H(\tau) = \int_{0}^\tau h(u)\ du = \sum_{e=1}^{k}\int_{0}^\tau h_{e}(u)\ du = \sum_{e=1}^k H_{e}(\tau)$, and
- *all-cause survival probability*: $S(\tau) = P(Y > \tau) = \exp(-H(\tau))$

Note that while notationally identical to the survival probability in the single-event case, the all-cause survival probability implies that *none* of the events occurred before $\tau$.
...
A common multi-state model, the illness-death model, is illustrated in @fig-illness-death.
...
@fig-mulit-state depicts a more general multi-state setting with back-transitions.
One can think of transitions between a healthy state (0) to different non-terminal disease progressions (1, 2) with possibilty of improvement and recovery, with possible transition to a terminal state (3).
...
In this representation, we only consider how long the subject was at risk for the $k$th event, not at which time (on the calendar time scale). This representation has the advantage that standard methods for analysis of single-event data can be used for modeling. However, the resulting dependency between multiple data points from one subject (at risk for multiple events at the same time) needs to be taken into account in the modeling step.
