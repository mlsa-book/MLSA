...
In the presence of censoring, however, @eq-binary-target does not define a valid target variable, as observations censored before time $\tau$ ($y_i < \tau \wedge \delta_i = 0$) are neither events nor non-events (as the event possibly occured between $y_i$ and $\tau$). 
Treating those observations as non-events would introduce bias.

One way to deal with this is to calculate adapted IPC weights (@sec-surv-ipcw) for each observation as 
...
where $\hat{G}_{KM}$ is the Kaplan-Meier estimator of the censoring distribution (@eq-km-censoring).
...
$$
\mathcal{L}(\pi) = \frac{1}{n} \sum_{i=1}^n \tilde{w}_i \ell_i(e_i, \pi(\mathbf{x}_i))
...
How exactly @eq-ipcw-loss-weighted looks like, will depend on the choice of learner and objective function. 
...
$$
\mathcal{L}(\pi) = \frac{1}{n} \sum_{i=1}^n \tilde{w}_i (e_i\log(\pi(\mathbf{x}_i)) + (1-e_i)\log(1 - \pi(\mathbf{x}_i))),
...
For some implementations of other popular learners like decision trees or random forests, specifying a suitable, custom objective function like @eq-ipcw-loss-weighted-logistic might not be possible.
However, in most implementations it is also possible to pass weights and instead use the weighted Gini impurity as a splitting criterion (@Vock2016).
