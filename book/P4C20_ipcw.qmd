...
However, in the presence of censoring, (@eq-binary-target) does not define a valid target variable: an observation $i$ censored before $\tau$ is neither an event ($\delta_i=0$), nor a confirmed non-event, as it is unknown whether the event would have occurred before $\tau$ had follow-up continued.
