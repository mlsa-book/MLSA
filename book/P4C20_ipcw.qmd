...
\hat{w}_i(\min(y_i, \tau)) = \frac{1}{\hat{G}_{KM}(\min(y_i, \tau))} & \text{else}
\end{cases}
$$ {#eq-adapted-ipc-weights}
<!--  -->
where $\hat{G}_{KM}$ is the Kaplan-Meier estimate of the censoring distribution (@sec-surv-km-censoring) and $\hat{w}_i(\min(y_i, \tau))$ are the IPC weights (@eq-ipcw-km) introduced in @sec-surv-ipcw.
...
For example, using the log loss (binary cross-entropy) $\ell(e_i, \pi(\mathbf{x}_i)) = e_i\log(\pi(\mathbf{x}_i)) + (1-e_i)\log(1 - \pi(\mathbf{x}_i))$ as loss function, @eq-ipcw-loss-weighted becomes 
...
- Equation (@eq-adapted-ipc-weights) implies that contributions of observations censored before time $\tau$ are multiplied with zero in (@eq-ipcw-loss-weighted) while uncensored observations are upweighted. Algorithmically, it would be more efficient to remove censored observations from the data set before training in order to avoid unnecessary computations of the loss. However, this might interfere with the usual training and evaluation procedures, for example when used in combination with cross-validation.
