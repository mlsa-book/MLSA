...
In continuous time settings, it is rare to encounter exact ties in observed event times, $t_i = t_j$, though the probability of this occurring as the time outcome is increasingly rounded or aggregated to lower precision.
As model complexity increases, ties in predicted risks, $\hat{r}_i = \hat{r}_j$, tend to become less common but again may be more likely in simpler models, for example a Cox PH model (@sec-classical) with few coefficients.
...
Equation @eq-cindex then follows with,
...
$$
C(\hat{\mathbf{h}}^{e^*}, \tt|\tau) = \frac{\sum_{i\neq j} \II(t_i < t_j, \hat{h}^{e^*}_{i}(t_i) > \hat{h}^{e^*}_j(t_i), t_i < \tau, e_i = e^*)}{\sum_{i\neq j}\II(t_i < t_j, t_i < \tau, e_i = e^*)},
$$

where $\hat{\hh}^{e^*} = (\hath^{e^*}_1 \ \hath^{e^*}_2 \cdots \hath^{e^*}_n)^\trans$ are the predicted cause-specific hazard functions for event $e^*$.
...
However, two recent concordance index estimators for left-truncated data have been proposed in the literature and implemented in software; whereas no comparable estimators could be found for right-truncated data [@McGough2021; @pkgsurvival].
...
$$
\operatorname{Control}_{2;i}^{e^*}(t_i, e_i, \tau) := t_i > \tau \vee (t_i \le \tau \wedge e_i \in \{1,...,q\} \setminus \{e^*\}).
$$ 
...
* Concordance measures should rarely be used to evaluate prediction's on their own as there is often a trade-off between discrimination and calibration, which is discussed in the next chapter.
