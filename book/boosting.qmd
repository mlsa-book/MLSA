...
This chapter focuses on tree-based learners, which are primarily used for survival analysis, due the flexibility demonstrated in @sec-surv-ml-models-ranfor.
...
Boosting algorithms use these losses to train the model coefficients, $\bb$, hence at each iteration in the algorithm, $g_m(\xx_i) = \xx_i\bb^{(m)}$, where $\bb^{(m)}$ are the updated coefficients in iteration $m$.

The Cox partial likelihood [@Cox1972; @Cox1975] is given by

$$
L^{PH}(\bb) = \prod^n_{i:\delta_i=1} \frac{\exp(\eta_i)}{\sum^n_{j \in \calR_{t_i}} \exp(\eta_j)}
$$
...
$$
\begin{split}
-l^{AFT}_m(\bb) = -\frac{\partial}{\partial f}\sum^n_{i=1} \delta_i\Big[- \log\hat{\sigma_{m-1}} + \log f_W\Big(\frac{\log(t_i) - \hat{g}_{m-1}(\xx_i)}{\hat{\sigma}_{m-1}}\Big)\Big] + \\
