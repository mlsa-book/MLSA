...
4. \ \ \ \ \ $\rr_{im} \gets -[\frac{\partial L(y_i, g_{m-1}(X_i))}{\partial g_{m-1}(X_i)}], i \in \{i: X_i \in \dtrain^*\}$
5. \ \ \ \ \ Fit a weak learner to $(\XX_i, \rr_{im}), j = 1,..,p$
6. \ \ \ \ \ $j^* \gets \argmin_{j = 1,..,p} \sum_{i \in \{i: X_i \in \dtrain^*\}}
(r_{im} - w_j(X_i))^2$
1. \ \ \ \ \ $g_m \gets g_{m-1} + \nu w_{j^*}$
2. **end For**
3.  **return** $\hatg = g_M$
