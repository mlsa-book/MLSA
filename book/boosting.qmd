...
\label{sec-surv-ml-models-boost}

\subsection{Gradient Boosting Machines for Regression}
\label{sec-surv-ml-models-boost-regr}
...
\textbf{Output} Boosted model, $\hatg$.}\label{alg-surv-gbm}
\begin{algorithmic}[1]
...
\textbf{Output} Prediction, $\hatY \sim \calY$.}\label{alg-surv-gbm-pred}
\begin{algorithmic}[1]
...
Finally for decision stumps (\ref{fig:surv_stump}), a decision tree, $w_j$, is grown (\ref{alg:dt_fit}) on $(X_{;j}, r_m)$ to depth one (equivalently to two terminal nodes) for each of the $j = 1,...,p$ covariates  [@Friedman2001].
...
\label{sec-surv-ml-models-boost-surv}
