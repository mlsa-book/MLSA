...
In simple linear regression, the aim is to estimate the line $y = \beta_0 + x\beta_1$ by estimating the $\beta_0,\beta_1$ coefficients.
...
In this view, the room appears to have dimensions (x=left-right, y=top-bottom) and the divider is a simple line of the form $y = \beta_0 + x\beta_1$.
In reality, this room is actually three dimensional and has a third dimension (z=up-down) and the divider occupies these dimensions as a hyperplane of the form $y = \beta_0 + x\beta_1 + z\beta_2$.

![Visualising a hyperplane by viewing a 3D room in two-dimensions with a wall that is now seen as a simple line. When standing in this room, the wall will clearly exist in three dimensional space.](Figures/svm/hyperplane.webp){#fig-svm-hyper fig-alt="AI generated image of an office cut in half by a dividing wall." width=400}

Similarly to multiple linear regression (MLR), support vector machines (SVMs)  [@CortesVapnik1995] fit a hyperplane, $g$, on given training data, $\XX$.
In contrast to MLR, the hyperplane is not restricted to a linear boundary but instead fits a more flexible hyperplane while simultaneously minimizing model complexity by optimizing a regularized error function that allows a margin of tolerance for 'incorrect' predictions and using kernel functions to transform high-dimensional spaces.
The terms introduced above will be broken down and visualized to aid in exposition in the survival setting later.

In penalized simple linear regression, used to reduce model complexity, the aim is to minimize the error function

$$
\frac{1}{2} \sum^n_{i=1} (g(x) - y)^2 + \frac{\lambda}{2} \|\beta\|^2
$$

for some error function $\lambda$.
In MLR, the goal of fitting the model is to find the hyperplane that represents the best linear relationship between coefficients and outcomes - where an observation lies on the line determines the prediction and the goal is to minimize the average difference between all predictions and true outcomes.
SVMs instead aim to find the hyperplane that minimizes the difference between predictions and the truth for individual observations.
...
Instead, the optimization problem allows incorrect errors for some observations, with the magnitude of error controlled by an $\epsilon>0$ parameter as well as slack parameters, $\xi, \xi^*$:
...
![Visualising a support vector machine with an $\epsilon$-tube and slack parameters $\xi$ and $\xi^*$. Red circles are values within the $\epsilon$-tube and blue diamonds are support vectors outside the tube. x-axis is single covariate, $x$, and y-axis is $g(\xx) = x\beta + \beta_0$.](Figures/svm/regression.png){#fig-svm-over fig-alt="TODO" width=500}
...
$$
\hatg(\xx^*) = \sum^n_{i=1} \alpha_iK(\xx^*,\xx_i) + \beta_0
$$ {#eq-svm-pred}

$K$ is some kernel function, and details (including estimation) of the $\alpha$ Lagrange multipliers are beyond the scope of this book, references are given at the end of this chapter for the interested reader.

Common kernel functions include the linear kernel, $K(x^*,x_i) = \sum^p_{j=1} x_{ij}x^*_j$, radial kernel, $K(x^*,x_i) = \exp(-\omega\sum^p_{j=1} (x_{ij} - x^*_j)^2)$ for some $\omega \in \PReals$, and polynomial kernel, $K(x^*,x_i) = (1 + \sum^p_{j=1} x_{ij}x^*_j)^d$ for some $d \in \PNaturals$.
...
The parameters $\gamma$ and $\epsilon$ often have no clear apriori meaning (especially true in the survival setting predicting abstract rankings) and thus require tuning over a great range of values; no tuning usually results in a poor model fit.
...
For both types of SSVM let: $\xi_i,\xi_i^*,\xi_i'$ be slack variables; $\bb,\beta_0$ be model weights in $\Reals$; $\gamma, \mu$ be regularisation hyper-parameters in $\Reals$; $(\XX, \tt, \dd)$ be the usual training data; and $g(\xx_i) = \bb^T\xx_i + \beta_0$.
...
$$
\hatg(\xx^*) = \sum^n_{i=1} \alpha^*_i K(\xx_i, \xx^*) - \delta_i\alpha_i'K(\xx_i, \xx^*) + \beta_0
$$

Where again $K$ is a kernel function and the calculation of the $\alpha^*, \alpha'$ Lagrange multipliers that are beyond the scope of this book.
...
$$
\hatg(\xx^*) = \sum^n_{i=1} \alpha_i(K(\xx_i, \xx^*) - K(\XX_{j(i)}, \xx^*)) + \beta_0
$$

Where $\alpha$ are again Lagrange multipliers.
