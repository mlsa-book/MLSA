...
In survival analysis, survival support vector machines (SSVMs), are one of the few classes that can directly estimate survival times.
...
Continuing the linear regression example, consider a simple model where the objective is to find the $\bb$ coefficients that minimize $\sum^n_{i=1} (g(\xx_i) - y_i)^2$ where $g(\xx_i) = b + \xx_i\beta_1$ and $(\XX, \yy)$ is training data such that $\XX \in \Reals^{n \times p}$ and $\yy \in \Reals^n$.
...
If no one is censored then the optimisation becomes the regression optimisation in (@eq-svm-opt).
