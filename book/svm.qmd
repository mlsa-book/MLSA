...
Continuing the linear regression example, consider a simple model where the objective is to find the $\dvec{\beta}{p}$ coefficients that minimize $\sum^n_{i=1} (g(\xx_i) - y_i)^2$ where $g(\xx_i) = \alpha + \xx_i^\trans\bb$ and $(\XX, \yy)$ is training data such that $\XX \in \Reals^{n \times p}$ and $\yy \in \Reals^n$.
...
$$
\begin{aligned}
& \min_{\bb,\alpha, \xixi', \xixi^*} \frac{1}{2} \|\bb\|^2 + C \sum^n_{i=1}(\xi'_i + \xi_i^*) \\
& \textrm{subject to}
\begin{dcases}
...
$\forall i\in 1,...,n$ where $g(\xx_i) = \alpha + \xx_i^\trans\bb$ for model weights $\bb \in \Reals^p$ and $\alpha \in \Reals$ and the same training data $(\XX, \yy)$ as above.
...
$$
\begin{aligned}
& \min_{\bb, \alpha, \xixi', \xixi^*} \frac{1}{2}\|\bb\|^2 + C\Big(\sum_{i \in \calU} \xi_i + \sum_{i \in \calL} \xi_i^*\Big) \\
& \textrm{subject to}
\begin{dcases}
...
$$
\begin{aligned}
& \min_{\bb, \alpha, \xixi', \xixi^*} \frac{1}{2}\|\bb\|^2 + C \sum_{i = 1}^n (\xi'_i + \xi_i^*) \\
& \textrm{subject to}
\begin{dcases}
...
$$
\begin{aligned}
& \min_{\bb, \alpha, \xixi} \frac{1}{2}\|\bb\|^2 + \gamma\sum_{i =1}^n \xi_i \\
& \textrm{subject to}
\begin{dcases}
...
This is visualized in @fig-svm-surv-redux where four observations are unsorted on the left -- the survival times are not increasing in the data -- on the right the observations are now sorted by survival time and the arrows indicate compatible pairs.
...
In order for all observations to be used in the optimisation, the algorithm sets the first outcome to be uncensored.


![Van Belle SVM nearest neighbors reduction. Observations are first sorted by survival time and then paired with the nearest uncensored outcome 'to the left'. Red squares are uncensored observations and blue circles are censored.](Figures/svm/svm_reduction.png){#fig-svm-surv-redux fig-alt="Two graphs. Left titled 'Unsorted' and on the x-axis first shows a red box at observation 1, then a blue circle at observation 2, then red box at observation 2, then blue circle at observation 4. Right titled 'Sorted' is the same as left but the observations are now ordered 1, 2, 3, 4. Additionally there are arrows that map observation 4 to 3, 3 to 1, and 2 to 1." width=500}
...
$$
\begin{aligned}
& \min_{\bb, \alpha, \xixi, \xixi', \xixi^*} \frac{1}{2}\|\bb\|^2 + \textcolor{CornflowerBlue}{\gamma\sum_{i =1}^n \xi_i} + \textcolor{Rhodamine}{C \sum^n_{i=1}(\xi_i' + \xi_i^*)} \\
& \textrm{subject to}
\begin{dcases}
...
$\gamma$ is the penalty associated with the regression method and $C$ is the penalty associated with the ranking method setting $\gamma = 0$ results in the regression SVM and $C = 0$ results in the ranking SSVM.
