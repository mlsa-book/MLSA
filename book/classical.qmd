...
\section{A Review of Classical Survival Models} {#sec-surv-models}
...
\subsection{Non-Parametric Distribution Estimators} {#sec-surv-models-uncond}
...
Not only can these estimators be used for analytical comparison, but they also provide intuitive methods for graphical calibration of models (@sec-eval-distr-calib-prob). These models are never stuidied for prognosis directly but as baselines, components of complex models (@sec-car-pipelines-distr), or graphical tools [@Habibi2018; @Jager2008; @Moghimi-dehkordi2008]. The reason for this is due to them having poor predictive performance as a result of omitting explanatory variables in fitting. Moreover, if the data follows a particular distribution, parametric methods will be more efficient  [@Wang2017].
...
\subsection{Continuous Ranking and Semi-Parametric Models: Cox PH} {#sec-surv-models-crank}
...
Despite these disadvantages, the model has been demonstrated to have excellent predictive performance and routinely outperforms (or at least does not underperform) sophisticated ML models  [@Gensheimer2018; @Luxhoj1997; @VanBelle2011b] (and (@sec-bench)). It's simple form and wide popularity mean that it is also highly transparent and accessible.

The next class of models address some of the Cox model disadvantages by making assumptions about the baseline hazard.

\subsection[Conditional Distribution Predictions: Parametric Linear Models]{Conditional Distribution Predictions: Parametric\\Linear Models} {#sec-surv-models-param}
