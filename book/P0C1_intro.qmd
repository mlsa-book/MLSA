...
Some motivating examples are first provided and then important concepts for machine learning survival analysis are introduced.

### Waiting for a bus

Every day you are making survival predictions.
For example, you are making a survival prediction by guessing when a bus will arrive at your bus stop.
...
Whilst it is common in survival analysis to talk about predicting the time to an event, such as in the previous example, this is not the quantity that model usually estimate directly.
Instead, survival models generally predict survival time distributions, which describe the probability of the event occurring over time (a distribution).
This is more informative than predicting a single event time as it naturally incorporates uncertainty (vital for working with censoring) and can also be used to make other types of prediction, discussed further in @sec-survtsk.
...
While the term survival analysis does highlight the closely related nature of the field with medical statistics and in particular predicting survival times (the time until death), this is certainly not the only application of the field (as seen in the earlier bus example).
...
<!-- note to self: check if we define EHA as multiple events, multiple times, or both -->
