---
abstract: This chapter introduces the foundations of machine learning survival analysis and motivates why time-to-event data require methods beyond standard regression and classification. Survival analysis is framed as a general paradigm for modeling the timing of events that may be incompletely observed due to censoring, with applications spanning medicine, engineering, and economics. The chapter begins to clarify key distinctions between survival analysis, event history analysis, and conventional supervised learning (regression and classification). In addition, the chapter starts to outline the prediction tasks that appear throughout the book including predicting survival distributions, time-to-event, and relative risks. Particular attention is given to censoring, truncation, competing risks, and common biases such as delayed entry and immortal time bias. The chapter concludes with a start-to-finish overview of a machine learning survival analysis workflow, providing a conceptual roadmap that sets the stage for the methodological developments in the remainder of the book.
---
...
There are also excellent books dedicated to survival analysis, such as @Collett2014 and @KalbfleischPrentice1973, but they do not include modern machine learning algorithms such as random forests, neural networks or gradient boosting machines.
To-date, the most comprehensive treatment for the combined fields is a light-touch but comprehensive survey in @Wang2017.
Using regression or classification models to solve problems based on survival datasets can lead to biased results as they poorly capture 'censored' observations -- those subjects that do not experience the event of interest within an observation window.
...
This book is focused entirely on survival predictions for observations given some variables ('supervised learning'), this book does not cover in-sample estimation, predicting 'remaining lifetime' for censored observations, nor pattern recognition methods such as clustering.
...
#### Hospital prioritization {.unnumbered .unlisted}
...
Take the example of prioritizing when patients are seen an emergency department.
...
Defining if a model should be called 'machine learning' is surprisingly difficult; there is no clear boundary separating 'classical' statistical estimators from what are now often labelled machine learning methods.
This book defines _machine learning_ in a pragmatic sense to denote statistical models whose parameters or structure are learned from data by optimizing an explicit objective function and whose success is judged by empirical performance on new data.
This definition tightly couples the concept of machine learning with the need for robust evaluation (Part II).
This definition allows for a range of models (Part III) to be considered machine learning, such as simple linear regression.
In fact, in @sec-classical we will describe how the performance of these apparently 'simple' models can compare very favorably to more modern machine learning algorithms - hence their inclusion in this book.
...
i. A _relative risk_: The risk of an event taking place compared to other observations in the same sample (the hospital example above).
...
Returning to the hospital example, a physician cannot process a survival distribution prediction to make urgent decisions, but they could assign limited resources if it is clear that one patient is at substantially greater risk than another -- a relative risk prediction.
...
Right- and left-censoring can occur completely independently of the outcome of interest, or they may directly affect the outcome of interest, in which case the censoring event is known as a _competing risk_ (@sec-eha).
For example, if you are waiting for a bus and decide to walk after five minutes, the bus arrival time is censored as you no longer observe when it will arrive, but your decision to walk does not impact the bus in any way, it's eventual arrival time is unchanged.
In contrast, if the bus broke down, then this breakdown would be a competing risk as it directly affects its probability of arriving.
...
## Reproducibility and online book

An online open-access (CC BY-NC-SA 4.0) version of this book is available for free at [mlsabook.com](mlsabook.com).
