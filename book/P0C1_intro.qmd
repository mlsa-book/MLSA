...
As well censoring, survival data can include _truncation_, which can be thought of as a stricter form of censoring.
Truncation is the process of fully excluding data whereas censoring is the process of partially excluding data.
...
The most common form of censoring is right-censoring (@fig-intro-censoring, middle), this occurs when the true survival time is greater than ('to the right of' if you imagine a number line) the observed censoring time.
...
If the time-to-event outcome is "age at first phone use" then a 23-year-old who does not remember must therefore be left-censored at '23'.

Left-truncation is the more common form of truncation.
It can occur in a few different situations and results in data before the truncation time either being partially or fully removed (@fig-intro-censoring, bottom).
...
The study design uses a convenience sample which means some individuals had TB from the study start and others did not.
Left-truncation would occur if the study excludes the at-risk time of individuals who had TB at the study start, meaning that they were treated as if they were diagnosed with TB on entry to the study - this is partial left-truncation.
Excluding this data would induce survivorship bias - accidentally conditioning on observations that are more likely to survive - as individuals with more aggressive strains are systematically excluded.
One common way to account for this bias would be to record the initial date of diagnosis, the date the treatment began, and specialized methods introduced in @sec-surv.
This is distinct from left-censoring because the event of interest, death, has not yet occurred.
...
The bias in this example is particularly hard to address as the data is completely truncated - the subjects are omitted without the researchers being aware of them - thus any techniques require external data or sensitivity analyses.
...
To control for this bias, one could improve the study design by only conditioning the sample on those (in both arms) who have already survived two years, thereby using partial left-truncation as a corrective strategy.
...
Detecting immortal time bias is hugely important, especially in randomized controlled trials to ensure novel treatments are not oversold.

Right-truncation is rare as it requires very specific, data collection.
