...
As for the single-event case (@sec-surv-estimation-non-param), one can define non-parametric estimators for the cause-specific cumulative hazard (@eq-cause-specific-cumu-hazard) and the CIF (@eq-cif).

Recall from @sec-data-rc the definitions of the unique ordered event times $t_{(k)}, k=1\ldots,m$, risk-set at time $t_{(k)}$, $\calR_{t_{(k)}}$, number of events, $d_{t_{(k)}}$, and number of observations at risk $n_{t_{(k)}}$.
...
The *cause-specific* discrete time hazard (@eq-discrete-hazard) is defined by updating the numerator $d_{t_{(k)}}$ to $d_{e,t_{(k)}}$ (the number of events of type $e$ at time $t_{(k)}$):
<!--  -->
$$
...
$$
<!--  -->
Finally, the Aalen-Johanson (AJ) estimator (@aalenEmpiricalTransitionMatrix1978) for the CIF follows via @eq-discrete-prob as
<!--  -->
$$
...
As we only have one other event (death), the data could be treated as single-event, right-censored data as in @sec-surv and therefore analysed using the Nelson-Aalen estimator (@eq-surv-na).
...
Extensions do exist that relax this Markovian assumption.
Transition probabilities of a multi-state process are often summarized in a matrix of transition probabilities
<!--  -->
$$
\mathbf{P}(\zeta, \tau):=
\begin{pmatrix}
...
In this example, back-transitions are not possible, therefore the lower triangle of the matrix is filled with zeros and $P_{22}(\zeta, \tau) = 1, \forall\ \zeta < \tau$ by virtue of being an absorbing state. The transition matrix is thus given as
<!--  -->
$$
\mathbf{P}(\zeta, \tau) =
  \begin{pmatrix}
...
### Estimating Transition Probabilities {#sec-ms-aalen-johanson}
...
Similarly to the competing risks setting (@sec-aalen-johanson), the Aalen-Johanson estimator can be used by substituting the cause-specific hazards (@eq-cs-discrete-hazard) with the transition-specific hazards:
<!--  -->
$$
...
- $d_{\ell e,t_{(k)}}$ is the number of observations with transition $\ell \rightarrow e$ at time $t_{(k)}$ and
- $n_{\ell; t_{(k)}} = |\mathcal{R}_{\ell;t_{(k)}}|$ is the number of subjects in state $\ell$ at time $t_{(k)}$.

Thus, the risk set $\mathcal{R}_{\ell;t_{(k)}}$ is interpreted as 'subjects at risk of experiencing a transition from $\ell$ at time $t_{(k)}$'.
An event is then interpreted as 'subjects that transition from state $\ell$ to $e$'.
