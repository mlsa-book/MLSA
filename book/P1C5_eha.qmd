...
This section introduces relevant notation (@sec-cr-notation), the non-parametric Aalen-Johansen estimator (@sec-aalen-johanson) followed by a worked example.
...
This doesn't make much difference in continuous time where $\Pr(T > t) = \Pr(T\geq t)$, but may be important in (discrete) approximations (as in @sec-aalen-johanson).
...
Implicitly these are transitions from starting state $0$ to end state $q$, however, since everyone starts in state $0$ this is ignored notationally.
...
$$
H_{\ell q}(u) =  \Pr\big(E(u+du)=q \mid E(u-) = \ell\big) =: p_{\ell q}(u).
$$
...
$$
dH_{\ell \ell}(u) := -\sum_{q \neq \ell} dH_{\ell q}(u),
$$

such that

$$
1 + dH_{\ell\ell}(u) = 1-\sum_{q \neq \ell} dH_{\ell q}(u) = 1 - \sum_{q\neq \ell}p_{\ell q}(u) = p_{\ell\ell}(u).
$$
...
<!--  -->
where $\triangle H_{\ell q}(t_{j}) = H_{\ell q}(t_{j}) - H_{\ell q}(t_{j-1})$ is the increment of the cumulative hazards between to consecutive time points.
...
Similarly to the competing risks setting (@sec-aalen-johanson), one can first define the transition specific hazards,
<!--  -->
$$
...
In contrast to the cumulative incidence functions of the competing-risks setting (@sec-aalen-johanson), the curves targeting *transient* states (such as $\ell \rightarrow 0$ and $\ell \rightarrow 1$) are not monotonically increasing in $\tau$.
