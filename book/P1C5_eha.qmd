...
In @sec-surv, a subject is characterised by a single random variable $Y \in \NNReals$ — the time until *the* event of interest (defined in @sec-data-rc). That works well when exactly one event is possible. Once there are multiple possible events (death by heart disease vs. cancer), recurrences (relapses, hospital re-admissions), or intermediate states (healthy → ill → recovered), a single scalar random variable no longer captures the data-generating process: we need to know **which state the subject is in at every moment in time**.

A *state* is a mutually exclusive classification of subjects at a given time point. In the simplest single-event setting, there are two states: "no event yet" (encoded as $0$) and "event has occurred" (encoded as $1$). In a competing-risks setting with $q$ possible events, the states are $\{0, 1, \ldots, q\}$ with $0$ meaning initial state (often "no event yet") and $e \in \{1, \ldots, q\}$ indicating that event of type $e$ has occurred.

We formalise this as a stochastic process
<!--  -->
$$
E(\tau) \in \{0,\ldots, q\},\ \tau \geq 0,
$$ {#eq-state-process}
<!--  -->
that records the state occupied at time $\tau$. In the single-event case ($q = 1$), $E$ and the $Y$ of @sec-surv carry the same information: $E(\tau) = 1 \iff Y \leq \tau$. In particular, $\{Y \in [\tau, \tau + \dtau) \mid Y \geq \tau\}$ and $\{E(\tau + \dtau) = 1 \mid E(\tau-) = 0\}$ describe the same outcome — "the event occurs in the next instant" and "the process jumps from state $0$ to state $1$ in the next instant" — so the two expressions for the hazard are equivalent:
<!--  -->
$$
\begin{aligned}
h(\tau)
...
The process-valued formulation seems over-engineered in the single-event case but is less verbose when there are multiple states and transitions, as we will see in @sec-competing-risks and @sec-multi-state. For a thorough stochastic-process treatment of event-history analysis, see @aalen2008survivalevent.


## Competing Risks {#sec-competing-risks}

In @sec-surv we assumed a single event of interest, with all other reasons a subject might leave the observation window lumped into right-censoring. **Competing risks** arise when several mutually exclusive events can end observation, and ignoring this structure — treating competing events as censoring — biases survival and hazard estimates (often substantially, as illustrated in @sec-cens-vs-cr). Canonical medical examples are cause-specific mortality (death by cancer vs. heart disease vs. other causes) and ICU outcomes (death in ICU vs. discharge alive); the defining feature is that once any one of the $q$ events occurs, the others can no longer occur. This section introduces the notation for competing risks, the non-parametric Aalen-Johansen estimator, and a worked comparison with the "censor-the-competing-event" approach on the ICU example below.
...
Estimating the CIF (@eq-cif) for cause $e$ requires estimates of two ingredients: the all-cause survival function $S(t)$ and the cause-specific hazard $h_e(t)$.
The strategy is to construct a non-parametric estimator for each ingredient and then combine them by *plugging in* into @eq-cif.
Both ingredients are direct analogues of the single-event constructions in @sec-surv-estimation-non-param.

We re-use the discretisation from the single-event case: recall from @sec-data-rc the unique ordered event times $t_{(k)},\, k=1,\ldots,m$, the risk set $\calR_{t_{(k)}}$, and the number at risk $n_{t_{(k)}}$.
...
The *cause-specific* discrete hazard is the obvious analogue of @eq-disc-haz-non-param, with the numerator restricted to events of type $e$ -- that is, $d_{e,t_{(k)}}$, the number of events of type $e$ observed at $t_{(k)}$:
<!--  -->
$$
...
The analyses that follow ignore this interval structure and treat the transition times as exactly observed -- the standard simplification in this example.
Accounting for the interval-censoring would require NPMLE-type methods along the lines of @sec-surv-estimation-non-param.
...
Throughout we take $\ell \neq e$ in the transition hazard $h_{\ell e}(\tau)$: a transition is by definition a move out of the current state.
The diagonal case $\ell = e$ appears only at the level of transition probabilities (see @eq-ms-trans-prob-mat), where $P_{\ell\ell}(\zeta, \tau)$ is the probability of occupying state $\ell$ at both $\zeta$ and $\tau$ (which, in processes with back-transitions, also includes paths that leave $\ell$ and return).
Furthermore, not every pair $(\ell, e)$ corresponds to a realisable transition: the state diagram of the specific process (e.g.\ @fig-multi-state-prothr-states for `prothr`) determines which transitions are possible a priori, and the hazards of impossible transitions are identically zero.
...
Throughout the remainder of @sec-multi-state we assume the multi-state process is *Markov*: the transition intensity at time $\tau$ depends only on the current state $E(\tau-)$, not on the full state history $\{E(u) : u < \tau\}$.
...
Relaxing the Markov assumption leads to *semi-Markov* and general *non-Markov* models, where the conditioning set must include past sojourn times or full state history -- see @aalen2008survivalevent and @beyersmann.competing.2012 for entry points.
We do not pursue these extensions here.
However, the marginal *state-occupancy* probabilities $P(E(\tau)=e)$ remain consistently estimable from the same Aalen-Johansen machinery even without the Markov assumption; this is taken up in @sec-ms-state-occupancy.
...
The pairwise transition probabilities $P_{\ell e}(\zeta, \tau)$ defined above condition on a specific entry-state $\ell$ at a specific time $\zeta$, and the Aalen-Johansen estimator @eq-ms-aj-trans-prob is consistent for them only under the Markov assumption introduced above.
A closely related but conceptually distinct target is the *state-occupancy probability*
<!--  -->
$$
P_e(\tau) := P(E(\tau) = e) = \sum_{\ell=0}^{q} P(E(0) = \ell)\, P_{\ell e}(0, \tau),
$$ {#eq-ms-state-occupancy}
<!--  -->
the marginal probability that a subject occupies state $e$ at time $\tau$, averaged over the initial-state distribution $P(E(0)=\ell)$.

State-occupancy probabilities are arguably the more natural target for many prediction tasks: questions of the form *"what is the probability that the subject is alive in normal-prothrombin state at $\tau = 365$ days?"* are directly answered by $P_e(\tau)$, not by a pairwise transition probability conditioned on a fixed entry-state.

A practically important property is that the Aalen-Johansen plug-in (@eq-ms-aj-trans-prob), applied with the empirical initial-state distribution rather than conditioned on a fixed $\ell$, remains a consistent estimator of $P_e(\tau)$ *even when the underlying process is not Markov* [@aalen2008survivalevent].
By contrast, the AJ estimator of the pairwise $P_{\ell e}(\zeta, \tau)$ loses its consistency guarantees once the Markov assumption fails.
...
<!--  -->
$$
\hat{P}_e(\tau) = \sum_{\ell=0}^{q} \hat{\pi}_\ell\, \widehat{P}_{\ell e}(0, \tau).
...
For users who wish to commit only to state-occupancy estimates, the same AJ pipeline applies; the additional step is the marginalisation in @eq-ms-state-occupancy-est.
...
This is a coarse but standard simplification; regression-based extensions that incorporate subject-level features into the multi-state estimation are taken up later in the book (see e.g.\ the pseudo-value approach in @sec-pseudo-value-trans-prob).
...
The reason is structural rather than specific to this data: the absorbing-state mass $P_{\ell 2}(0, \tau)$ grows monotonically with $\tau$ and the row-sum constraint @eq-ms-trans-prob-row-sums forces the transient-state probabilities to give up mass to compensate.
...
Between roughly $1000$ and $3000$ days the prednisone arm shows a modest reduction in the $0 \rightarrow 2$ transition probability and an increase in the back-transition $1\rightarrow 0$ relative to placebo, but the gap narrows again towards the end of follow-up.
