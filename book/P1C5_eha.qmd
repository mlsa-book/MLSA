...
Under this assumption, @eq-ms-transition-hazards is unambiguous, and the transition probability $P_{\ell e}(\zeta,\tau)$ is fully determined by the cumulative transition hazards between $\zeta$ and $\tau$.
The transition-probability matrix @eq-ms-trans-prob-mat and its non-parametric Aalen-Johansen estimator (@eq-ms-aj-trans-prob) are then consistent.
...
The pairwise transition probabilities $P_{\ell e}(\zeta, \tau)$ defined above condition on a specific entry-state $\ell$ at a specific time $\zeta$ and rely on the Markov assumption.
...
The pairwise transition probabilities $P_{\ell e}(\zeta, \tau)$, by contrast, lose their interpretation under non-Markov dynamics.
...
The estimands of interest are the transition probabilities $\hat{P}_{\ell e}(0, \tau)$ for starting state $\ell \in \{0, 1\}$ and end state $e \in \{0, 1, 2\}$, computed separately for the prednisone and placebo arms via the Aalen-Johansen estimator (@eq-ms-aj-trans-prob).
Stratifying by treatment treats each arm as a homogeneous Markov sub-population; this is a coarse but standard simplification, and serves as a baseline before the regression-based methods of Part III, where subject-level features refine the estimates.
