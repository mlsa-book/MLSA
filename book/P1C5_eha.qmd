...
Two different methods to represent recurrent events are shown in the figure: (top) the status is reset to $0$ after occurrence of an event; or (bottom) each recurrence of the event is modelled as separate states.
...
Left-truncation\index{truncation!left} is particularly important for the multi-state setting as subjects enter the risk sets for each transition at different time points, which must individually accounted for (@sec-non-param-lt).
...
Treating competing events as censoring will bias survival and hazard estimates, this is illustrated in @sec-cens-vs-cr.
A canonical medical example is cause-specific mortality, for example modelling if death was caused by cancer or heart disease or another cause; another common example is modelling death in an intensive care unit versus discharge.
...
Time under observation ('time') could end in one of three outcomes ('status'): $1$ --- discharge alive; $2$ --- death on ICU; or $0$ --- neither discharge nor death at the end of follow-up, which constitutes right-censoring at the end of study.
...
As discharged patients are healthier than those who remain on ICU, assuming independence between the time until discharge and time until death is unrealistic.
...
Analysis of how these different assumptions (independent censoring vs. competing risks) affects the estimates is discussed in @sec-aalen-johansen and @sec-cens-vs-cr.
...
where $S(u-)$ is the probability of surviving (not experiencing any of the competing events) until the time-point shortly before $u$; and $f_q(u)\du = S(u-)h_q(u)\du$ is the probability of experiencing event $q$ at time point $u$ (which follows analogously to the relationship between density and hazard functions in (@eq-continuous-hazard)).
...
The all-cause survival probability\index{all-cause survival probability} (@eq-all-cause-surv-prob) can be estimated with the Kaplan-Meier estimate\index{Kaplan-Meier estimator} (@eq-km).
...
<!-- FIXME (ANDREAS):
  1 (MAJOR). BELOW WAS USING TAU IN PARENTHESES; CHECK YOU'RE HAPPY WITH MY FIX. I DIDN'T KNOW IF $t_{(k-1)}$ or $t_{(k)}-$ WAS PREFERRED
  2 (MINOR). BELOW YOU HAVE \hat{h} but in eq-cs-discrete-hazard there's no hat on the left hand side even though the right is identical; seems a bit inconsistent -->
$$
F_{AJ,q}(\tau) = \sum_{k:\tbk \leq \tau} \hat{S}_{KM}(t_{(k-1)})\hat{h}^d_q(\tbk)=  \sum_{k:\tbk \leq \tau} \hat{S}_{KM}(t_{(k-1)})\frac{d_{q,\tbk}}{n_{\tbk}}.
$$ {#eq-aalen-johansen}
<!--  -->
...
As a few examples, the proportion of subjects with pneumonia being discharged before $\tau=120$ days is approximately 75%:

$$
\hat{F}_{1}(120 \mid x_{\text{pneu}}=1) = \hat{P}(\tau \leq 120, E(\tau) = 1 \mid x_{\text{pneu}}=1)\approx 0.75,
$$

while approximately 25% died in the ICU:

$$
\hat{F}_2(120 \mid x_{\text{pneu}}=1) = \hat{P}(\tau \leq 120, E(\tau) = 2 \mid x_{\text{pneu}}=1)\approx 0.25.
$$
...
<!-- FIXME (ANDREAS): CHANGE X-LABEL to 'time, \tau' AND Y-LABEL TO hat{F}_q(\tau) -->
...
In that case, the event of interest (death) could be modelled using the methods in @sec-surv, for example using the Nelson-Aalen estimator\index{Nelson-Aalen estimator} (@eq-surv-na).
...
Clearly, the probabilities of dying at time $\tau=120$ are much larger when independent censoring is assumed; for example around 75% in the pneumonia group under the right-censoring assumption, compared to around 25% when discharge is correctly modelled as a competing risk.
This example emphasizes the importance of assessing which modelling framework is appropriate for a given analysis, as the resulting estimates may differ substantially.

<!-- FIXME (ANDREAS): CHANGE X-LABEL to 'time, \tau' AND Y-LABEL TO hat{F}_2(\tau) -->
...
<!-- TODO (ANDREAS; MINOR): Prothrombin lower-case -->

![Transition graph for the liver cirrhosis patients.](Figures/survival/multi-state-prothr-states.png){#fig-multi-state-prothr-states fig-alt="Text in a circle 'Normal prothrombin levels (0)' has an arrow to a circle 'Abnormal prothrombin levels (1) and an arrow to a square 'Death (2)'. State 1 has an arrow back to State 0 and an arrow to State 2." width=60%}
...
Accounting for the interval-censoring would require similar nonparametric maximum likelihood estimator\index{nonparametric maximum likelihood estimator} methods to those described in @sec-surv-estimation-non-param.

<!-- FIXME (ANDREAS; MINOR): WHAT IS THE TRANS COLUMN? REMOVE IF NOT IMPORTANT -->
...
<!-- FIXME (ANDREAS; MAJOR): ENTERS RISK SET FOR WHAT? WHAT IS 'SAID TRANSITIONS'? -->
...
In this case, the patient died and transition $1\rightarrow 2$ was realized after 151 days, while the transition $1 \rightarrow 0$ is a 'counterfactual' transition that could have happened in the time-span between 'tstart=0' and 'tend=151', but did not actually occur in reality.
...
Neither of the transitions occurred, as 'status=0' for both transitions, which means the subject remained in status $0$ (right-censored) until the end of their follow-up at 2770 days.
...
<!-- FIXME (ANDREAS; MINOR): I THINK TABLE CAPTION COULD BE IMPROVED BY RESTATING COLUMN DESCRIPTIONS -->
...
<!--  -->
<!-- FIXME (ANDREAS; MINOR): DO WE REALLY NEED \TAU- ? WE ALREADY HAVE TAU+DTAU -->
$$
...
which is the the probability to occupy a certain state $q$ at time $\tau$ (described in detail in @sec-ms-state-occupation).
...
<!-- FIXME (ANDREAS): 'SOJOURN TIMES' HASN'T BEEN USED BEFORE -->
Relaxing the Markov assumption leads to *semi-Markov*\index{Markov!semi} and general *non-Markov* models, where the conditioning set must include past sojourn times or full state history; these are not discuss further here, instead see @aalen2008survivalevent and @beyersmann.competing.2012 for an overview.
...
In this example, back-transitions are not possible, therefore the lower triangle of the matrix is filled with zeros and $P_{22}(\zeta, \tau) = 1, \forall\ \zeta < \tau$ by virtue of being an terminal state.
...
Then (@eq-ms-trans-prob-ill-death-discrete) followed by replacing $p_{\ell q}(j)$ with $p_{\ell q}(t_j)$.
...
Transition probabilities\index{transition probabilities} are obtained via (@eq-ms-prodint-approx),
<!--  -->
$$
\mathbf{P}(\zeta, \tau) = \prod_{j=1}^{J}\left(\mathbf{I} + \triangle\mathbf{H}_{AJ,\ell q}(t_{(j)})\right).
$$ {#eq-ms-aj-trans-prob}
<!--  -->
...
* Defining the state diagram of the data generating process can be helpful first step to understand the dynamics in the data and to select an appropriate framework (and model) for the analysis.
