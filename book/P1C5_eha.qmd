...
Each row represents one subject identified by `id`. `time` is the time under observation, `status` indicates the outcome observed ($0$: censored at the end of the study, $1$: discharged alive from the ICU, $2$: death in the ICU). `pneumonia` indicates whether a subject already had pneumonia at ICU admission. {#tbl-surv-data-sir.adm}
...
<!--  -->
$$
h_{e}(\tau) = \lim_{\dtau \to 0} \frac{\Pr(E(\tau + \dtau) = e\  \mid E(\tau-)=0)}{\dtau}.
...
For example, the proportion of subjects with pneumonia being discharged until $\tau=120\text{ days}$ is approximately 75% ($\hat{F}_{1}(120 \mid x_{\text{pneu}}=1) = \hat{P}(Y \leq 120, E(Y) = 1 \mid x_{\text{pneu}}=1)\approx 0.75$), while approximately 25% died in the ICU ($\hat{F}_{2}(120 \mid x_{\text{pneu}}=1) = \hat{P}(Y \leq 120, E(Y) = 2 \mid x_{\text{pneu}}=1)\approx 0.25)$.
For patients without pneumonia, $\hat{F}_{1}(120 \mid x_{\text{pneu}}=0) \approx 0.91$ and $\hat{F}_{2}(120 \mid x_{\text{pneu}}=0) \approx 0.09$.
...
In the competing risks setting, the data generating process was characterized by cause-specific transitions hazards $h_e(\tau)$ (@eq-cause-specific-hazard).
...
<!--  -->
$$
h_{\ell e}(\tau) = \lim_{\dtau \to 0} \frac{\Pr(E(\tau + \dtau)=e \mid E(\tau-) = \ell)}{\dtau}
...
A transition is by definition a move out of the current state and therefore $\ell \neq e$ when describing the transition hazard $h_{\ell e}(\tau)$.
Furthermore, not every pair $(\ell, e)$ corresponds to a realisable transition: the state diagram of the specific process (for example, @fig-multi-state-prothr-states for `prothr`) determines which transitions are possible a priori, and the hazards of impossible transitions are identically zero.
...
- The *transition probabilities* $P_{\ell e}(\zeta, \tau):= \Pr(E(\tau) = e \mid E(\zeta) = \ell)$, that is, the conditional probability to be in state $e$ at $\tau$ given occupation of state $\ell$ at $\zeta < \tau$ (@sec-ms-transition-probs).
- the *state-occupation probability* $\Pr(E(\tau) = q)$: the probability to occupy a certain state $q$ at time $\tau$ (@sec-ms-state-occupation).
...
Under this assumption, conditioning on the current state alone (as in equation @eq-ms-transition-hazards) is sufficient to characterise the local dynamics, and the transition probability $P_{\ell e}(\zeta,\tau)$ is fully determined by the cumulative transition hazards between $\zeta$ and $\tau$.
...
Notably, the marginal *state-occupation* probabilities $\Pr(E(\tau)=e)$ remain consistently estimable from the same Aalen-Johansen methodology, even without the Markov assumption (see @sec-ms-state-occupation).
...
Thus, $P_{02}(4,6) = \textcolor{blue}{p_{00}(5)p_{02}(6)} + \textcolor{green}{p_{01}(5)p_{12}(6)} + \textcolor{orange}{p_{02}(5)}$, where $p_{\ell e}(\tau)=\Pr(E(\tau)=e \mid E(\tau -1) = \ell)$ are the probabilities for transitions $\ell \rightarrow e$ between two subsequent discrete time points.
...
$$
H_{\ell e}(u) =  \Pr\big(E(u+du)=e \mid E(u-) = \ell\big) =: p_{\ell e}(u).
$$
...
$d_{\ell e,t_{(k)}}$ is the number of subjects that made the transition $\ell \rightarrow e$ at time $t_{(k)}$; and $n_{\ell; t_{(k)}}$ is the number of subjects in state $\ell$ immediately before $t_{(k)}$.
...
The pairwise transition probabilities, $P_{\ell e}(\zeta, \tau)$, defined above are conditioned on a specific entry-state $\ell$ at a specific time $\zeta$; the Aalen-Johansen estimator (@eq-ms-aj-trans-prob) is only consistent for them under the Markov assumption.
...
The estimands of interest are the four off-diagonal transition probabilities $\hat{P}_{\ell e}(0, \tau)$ for $(\ell, e) \in \{(0,1), (0,2), (1,0), (1,2)\}$, computed separately for the prednisone and placebo arms via the Aalen-Johansen estimator (@eq-ms-aj-trans-prob).
