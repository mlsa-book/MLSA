...
- The *transition probabilities* $P_{\ell e}(\zeta, \tau):= P(E(\tau) = e \mid E(\zeta) = \ell)$, that is, the conditional probability to be in state $e$ at $\tau$ given occupation of state $\ell$ at $\zeta < \tau$ (@sec-ms-transition-probs).
- the *state-occupation probability* $P(E(\tau) = q)$: the probability to occupy a certain state $q$ at time $\tau$ (@sec-ms-state-occupation).
...
Notably, the marginal *state-occupation* probabilities $P(E(\tau)=e)$ remain consistently estimable from the same Aalen-Johansen methodology, even without the Markov assumption (see @sec-ms-state-occupation).
