...
In this generalisation, the observed data is sometimes refered to as *event-history data* and its analysis as *event-history analysis* (EHA).
...
In both cases, the event needs not be terminal (in the sense of death), we just end the observation/modeling after the first occurence of an event.
...
In the *recurrent events setting*, the same event can be observed multiple times on the same subject (for example reccurrent respiratory infections during one year).
Figure @fig-eha shows two different ways to represent recurrent events, (a) reset the status to $0$ after occurence of an event or (b) consider the $1$st, $2$nd, etc. recurrences of the event as separate states.
In some data sets only the recurrence of the event of intrest is present.
...
In the more complex cases, recurrent events are treated as a multi-state process and modeled accordingly, therefore we don't treat recurrent events separately in this book, but excelent accounts are given in @cookStatisticalAnalysisRecurrent2007 and @erdmann2024comparisonnonparametrica.
...
Lower-left: Recurrent events setting with multiple occurences of the same event. Lower-right: Multi-state setting where subjects can transition between multiple transient with possible back-transitions or to terminal states.](Figures/survival/eha-overview.png){#fig-eha fig-alt="Schematic illustration of event history analysis."}
...
$$ {#eq-cs-nelson-aalen}
<!--  -->
which yields a step function for each $e$, with jumps at timepoints $t_{(k)}$.
...
Examplary, the proportion of subjects with pneumonia being discharged until $\tau=120\text{ days}$ is approximately 75% ($\hat{F}_{\text{discharge}}(120) = P(Y\leq 120, E=\text{"discharge"})\approx 0.75$), while approximately 25% died in the ICU ($\hat{F}_{\text{death}}(120) = P(Y\leq 120, E=\text{"death"})\approx 0.25)$.
...
@fig-multi-state depicts two examplary multi-state settings.
...
This specific setting is sometimes refered to as "illness-death model" in medical contexts, as healthy subjects (state 0) can transition to death (state 2) either directly or after prior illness (state 1).
The right panel depicts a more complex process where subjects can transition from intitial state 0 to final stage 3 either directly ($0 \rightarrow 3$) or via intermediate states 1 and 2 ($0 \rightarrow 1 \rightarrow 3$ or $0 \rightarrow 1 \rightarrow 2 \rightarrow 3$).
Note, however, that here backtransitions are possible, thus for any subject, transition histories like $0 \rightarrow 1 \rightarrow 0 \rightarrow 3$ or  $0 \rightarrow 1 \rightarrow 2 \rightarrow 1 \rightarrow 0$ and many others are possible.
...
In this case, the patient died, thus the transition $1\rightarrow 2$ realised after 151 days, while the transition $1 \rightarrow 0$ is a 'counterfactual' transition that could have hapened, but didn't.
...
Other subjects in the data may never enter the risk set for these trasitions by remaining in state 0 until the end of follow up or by directly transitioning to state 2.
The fact that subjects enter the risk sets for differnt transitions at different time points technically constitutes left-truncation and thus should be taken into account accordingly (@sec-ms-aalen-johanson).
...
Similar to competing risks (@sec-competing-risks), transitions between different states (see @fig-multi-state) are characterised by transition-specific hazards, denoted by $h_{\ell\rightarrow e}(\tau)$ or short $h_{\ell e}(\tau)$, where $\ell \in \mathcal{L}$ is the starting state and $e \in \mathcal{E}$ the end state, with $\mathcal{L}$ the set of possible starting states (transient states) and $\mathcal{E}$ the set of possible end states (transient and terminal states).
...
\end{align}$$ {#eq-ms-transition-hazards}
...
Thus, $P_{02}(4,6) = p_{00}(5)p_{02}(6) + p_{01}(6)p_{12}(6) + p_{02}(5)$, where $p_{\ell e}(\tau)=P(E(\tau)=e|E(\tau -1) = \ell)$ are the probabilities for transitions $\ell \rightarrow e$ between two discrete timepoints.
...
where the quantity of interest, $P_{02}(4, 6)$ is given in the top right corner, but other transition probabilities are also readily available. For example, the probability to transition from state $1$ to $2$ between timepoints $\zeta=4$ and $\tau=6$ is given as $P_{12}(4, 6) = p_{11}(5)p_{12}(6) + p_{12}(5)$.

Returning to the continuous time setting where transitions can occur at any timepoint, ideas from the discrete time setting still hold.
...
Increasing the number of intervals to infinity or equivalently, reducing the interval size to infinitesimaly small intervals $du$ where only one transition can be observed, leads to a product integral of instanteneous transition probabilities $p_{\ell e}(u) = P(E(u+du) = e|E(u-)=\ell)$, such that
<!--  -->
$$
\begin{align}
...
Finally, for practical purposes, we express the instanteneous transition probabilities $p_{\ell e}(u)$ in terms of increments of the cumulative hazard using, somewhat informally, the following relationships

- by setting $\triangle \tau$ to some fixed, infinitesimly small value in @eq-ms-transition-hazards, we can write $h_{\ell e}(\tau)\triangle\tau = P\big(E(\tau + \triangle \tau) = e|E(\tau-)=\ell\big)$,
...
<!--  -->
where $\mathbf{I}$ is a $(q+1) \times (q+1)$ identity matrix and $d\mathbf{H}(u)$ is the matrix of increments of the cumulative hazard within infinitesimily small intervals.
...
<!--  -->
where $\triangle H_{\ell e}(t_{j}) = H_{\ell e}(t_{j}) - H_{\ell e}(t_{j-1})$ is the increment of the cumulative hazards between to consequtive timepoints.
...
Thus, risk set $\mathcal{R}_{\ell;t_{(k)}}$ exlcudes subjects who entered a terminal state or were censored before time $t_{(k)}$, but also subjects who are not in state $\ell$ at that time (either because they did not reach it yet or tansition to another state occured beforehand).
...
In constrast to the illness-death model in @fig-multi-state, back transitions are possible and some subjects already start in the "abnormal" state at the beginning of the study.
...
In this example back-transitions are possible, therefore, in contrast to the cumulative incidence functions in the competing risks setting, transition probabilities (to transtient states) are not monotonously increasing over time.
...
Although there appears to be a reduction in $0 \rightarrow 2$ transitions  and an increase in $1\rightarrow 0$ transitions  between timepoints $1000$ and $3000$, this effect doesn't seem to persist until the end fo the follow up.
