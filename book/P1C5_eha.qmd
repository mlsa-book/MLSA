...
This chapters generalizes time-to-event data beyond a single, potentially censored or truncated, outcome of interest.
This chapter explores more complex settings with multiple, potentially mutually exclusive events and recurrences of events.
In this generalization, the observed data is sometimes referred to as *event history data* and its analysis as *event history analysis*\index{event history analysis}.
...
In the *competing risks*\index{competing risks} setting (@fig-eha, upper right panel, @sec-competing-risks), a subject could transition to any of the $q$ mutually exclusive states, thus the subject is initially *at risk* for a transition to multiple states.
Once one of them occurs, the process is considered to have concluded (for the modeling purposes).

In the *recurrent events setting*\index{recurrent events} (@fig-eha lower left panel), the same event can be observed multiple times on the same subject (for example recurrent respiratory infections during one year).
...
Recurrent events are often represented as multi-state processes and their methodology can therefore be inferred from the theory in @sec-multi-state.
For treatment about recurrent events explicitly, see @cookStatisticalAnalysisRecurrent2007.

In the most general case, the *multi-state*\index{multi-state models} setting (@fig-eha lower right panel, @sec-multi-state), there are multiple transient and terminal states\index{transient states}\index{terminal states} with potential back transitions (for example, moving between different stages of an illness with the possibility of (partial) recovery and death as terminal event).

<!-- FIXME (ANDREAS): UPDATE DIAGRAM q -> Q -->
...
In particular, remaining in one of the transient states until the end of follow-up constitutes right-censoring\index{censoring!right} with respect to all possible transitions from that state and left-truncation\index{truncation!left} is particularly important as subjects enter the risk sets for a transition at different time points in context of recurrent events and multi-state settings.
...
This definition works well when exactly one event is possible.
...
Instead, one tries to model which state the subject is in at multiple different moments in time.

A *state* is a mutually exclusive classification of subjects at a given time point.
...
In particular, $\{Y \in [\tau, \tau + \dtau) \mid Y \geq \tau\}$ and $\{E(\tau + \dtau) = 1 \mid E(\tau-) = 0\}$ describe the same outcome — "the event occurs in the next instant" and "the process jumps from state $0$ to state $1$ in the next instant" — so the two expressions for the hazard are equivalent:
<!--  -->
$$
\begin{aligned}
h(\tau)
...
*Competing risks*\index{competing risks} arise when several mutually exclusive events can occur on the same subject.
Ignoring this structure, treating competing events as censoring, biases survival and hazard estimates (illustrated in @sec-cens-vs-cr).
Canonical medical examples are cause-specific mortality, for example, death by cancer versus heart disease versus other causes; and ICU outcomes, for example, death in ICU versus discharge.
...
This section introduces relevant notation (@sec-cr-notation), the non-parametric Aalen-Johansen estimator\index{Aalan-Johansen estimator} (@sec-aalen-johansen) followed by a worked example.
...
Analysis of this data and how the different assumptions (independent censoring vs. competing risks) affects the estimates is discussed in @sec-aalen-johansen and @sec-cens-vs-cr.
...
These absorbing states\index{terminal states} are also often referred to as *competing events*, *event types*, or *causes* (as in cause that ended the observation).
State $q = 0$ is the initial state and a subject that remains in state $0$ until the end of the follow up is considered censored.

The goal is to characterize the process $E(\tau) \in \{0,\ldots, Q\}$ in terms of transition hazards and probabilities.
In extension of (@eq-continuous-hazard-process), define the *cause-specific* hazards as\index{cause-specific hazard},

$$
h_q(\tau) = \lim_{\dtau \to 0} \frac{\Pr(E(\tau + \dtau) = q\  \mid E(\tau-)=0)}{\dtau}.
$$ {#eq-cause-specific-hazard}

Analogous to the single-event case, the cause-specific cumulative hazard\index{cause-specific cumulative hazard} is then defined as,
...
$$ {#eq-all-cause-hazard}
<!--  -->
as well as the *all-cause cumulative hazard*\index{all-cause cumulative hazard}, which can be obtained either via the integral over the all-cause hazard (@eq-all-cause-hazard) or as sum of cause-specific cumulative  hazards (@eq-cause-specific-cumu-hazard):
...
Finally, the probability of experiencing an event $q$ before time $\tau$, which is referred to as the *Cumulative Incidence Function* (CIF)\index{cumulative incidence function}, is given by

$$
\begin{aligned}
F_q(\tau) &= \Pr(E(\tau) = q)  &\ \\
            & = \int_{0}^\tau f_q(u)\ \mathrm{d}u = \int_{0}^\tau S(u-)h_q(u)\ \mathrm{d}u,
\end{aligned}
$$ {#eq-cif}
<!--  -->
where
...
* $f_q(u)\mathrm{d}u = S(u-)h_q(u)\mathrm{d}u$ is the probability of experiencing event $q$ at time point $u$ (which follows analogously to (@eq-continuous-hazard)).

The notation $S(u-)$ is used rather than $S(u)$ to make explicit the modelling of the probability to survive until the time point immediately before $u$. 
This doesn't make much difference in continuous time where $\Pr(T > t) = \Pr(T\geq t)$, but may be important in (discrete) approximations (as in @sec-aalen-johansen).

$F_q(\tau)$ can be interpreted as the proportion of subjects who experienced event of type $q$ until time $\tau$.
...
Estimating the CIF (@eq-cif) for cause $q$ requires estimates of two components: the all-cause survival function $S(t)$ and the cause-specific hazard $h_q(t)$.
The strategy is to construct a non-parametric estimator for each component and then combine them by *plugging in* into (@eq-cif).
Both components are direct analogues of the single-event constructions in @sec-surv-estimation-non-param.

Recall from @sec-data-rc the unique ordered event times $t_{(k)},\, k=1,\ldots,m$, the risk set $\calR_{t_{(k)}}$, and the number at risk $n_{t_{(k)}}$.
Assume the follow-up is partitioned into $m$ disjoint intervals $(t_{(k-1)}, t_{(k)}],\, k=1,\ldots,m$, such that $Y \in (t_{(k-1)}, t_{(k)}] \Leftrightarrow \tilde{Y}=t_{(k)}$, with $\tilde{Y}$ defined as in @sec-distributions-discrete.

Let $d_{q,t_{(k)}}$ be the number of events of type $q$ observed at $t_{(k)}$.
...
Plugging the Kaplan-Meier estimate\index{Kaplan-Meier estimator} (@eq-km) for the all-cause survival probability\index{all-cause survival probability} (the probability that none of the competing events has occurred) together with the cause-specific discrete hazard (@eq-cs-discrete-hazard) into (@eq-cif) yields the Aalen-Johansen (AJ) estimator\index{Aalen-Johansen estimator} (@aalenEmpiricalTransitionMatrix1978) for the CIF of cause $q$:
<!--  -->
$$
F_{AJ,q}(\tau) = \sum_{k:t_{(k)}\leq \tau} \hat{S}_{KM}(\tau-)\hat{h}^d_q(\tau)=  \sum_{k:t_{(k)}\leq \tau} \hat{S}_{KM}(t_{(k-1)})\frac{d_{q,t_{(k)}}}{n_{t_{(k)}}}
$$ {#eq-aalen-johansen}
<!--  -->
...
For illustration of the AJ estimator and the interpretation of the CIFs\index{cumulative incidence function} consider the analysis conducted in @beyersmann.competing.2012, based on the data from @tbl-surv-data-siradm.
...
As a few examples, the proportion of subjects with pneumonia being discharged until $\tau=120$ days is approximately 75%:

$$
\hat{F}_{1}(120 \mid x_{\text{pneu}}=1) = \hat{P}(Y \leq 120, E(Y) = 1 \mid x_{\text{pneu}}=1)\approx 0.75,
$$

while approximately 25% died in the ICU:

$$
\hat{F}_2(120 \mid x_{\text{pneu}}=1) = \hat{P}(Y \leq 120, E(Y) = 2 \mid x_{\text{pneu}}=1)\approx 0.25.
$$
...
<!-- FIXME (ANDREAS): CHANGE 'e' to 'q' IN FIGURE -->
...
It is worth spending some time to consider the difference between independent right-censoring and competing risks\index{censoring!right}\index{competing risks}.
Note that for the estimation of the hazard (@eq-cs-discrete-hazard), occurrences of competing events are implicitly assumed right-censored (as $d_{q,t_{(k)}}$) only counts events of type $q$ and $n_{t_{(k)}}$ contains the same subjects that would remain if events of type $\tilde{q} \neq q$ were considered censored before $t_{(k)}$.
Nevertheless, competing risks are taken into account in the definition of the CIFs (@eq-cif) and thus also in the AJ estimator (@eq-aalen-johansen), as the all-cause survival probability (@eq-all-cause-surv-prob) depends on all cause-specific hazards.

In contrast, assume that in the analysis of the `sir.adm` data, time of discharge was instead considered as independent right-censoring.
In this case, observations could either experience the event of interest (death) or right-censoring and the data could be modelled using the methods in @sec-surv, for example using the Nelson-Aalen estimator\index{Nelson-Aalen estimator} (@eq-surv-na).
The probability of death before some time-point $\tau$ could thus be obtained via $\Pr(Y\leq \tau) = F(\tau) = 1 - S_{NA}(\tau)$.
...
The multi-state process\index{multi-state models} can be considered the most general type of time-to-event process, as other types (single-event, competing risks\index{competing risks}, recurrent events\index{recurrent events}) can be viewed as special cases.
...
![Transition graph for the liver cirrhosis patients.](Figures/survival/multi-state-prothr-states.png){#fig-multi-state-prothr-states fig-alt="Transition probabilities for the prothrombin data example." width=60%}
...
Accounting for the interval-censoring would require similar NPMLE\index{nonparametric maximum likelihood estimator} methods to those described in @sec-surv-estimation-non-param.
...
This is necessary, as all possible transitions are listed, so an indicator is required to identify which transition actually occurred.
...
The fact that subjects enter the risk sets for different transitions at different time points technically constitutes left-truncation and thus should be taken into account accordingly (@sec-ms-aalen-johansen).
...
In the competing risks setting, the data generating process was characterized by cause-specific transitions hazards $h_q(\tau)$ (@eq-cause-specific-hazard).
Implicitly these are transitions from starting state $0$ to end state $q$, however, since everyone starts in state $0$ this is ignored in the notation.
In the multi-state setting on the other hand, subjects can be in different states at different time points.
Transitions between different states are therefore characterized by transition hazards, denoted by $h_{\ell\rightarrow q}(\tau)$ or short $h_{\ell q}(\tau)$, where $\ell$ is the starting state and $q$ the end state.
...
Furthermore, not every pair $(\ell, q)$ corresponds to a realizable transition: the state diagram of the specific process (for example, @fig-multi-state-prothr-states for `prothr`) determines which transitions are possible a priori, and the hazards of impossible transitions are identically zero.

Analogous to the competing risks setting, one can define the _transition cumulative hazard_,
...
that is, the conditional probability to be in state $q$ at $\tau$ given occupation of state $\ell$ at $\zeta < \tau$ (@sec-ms-transition-probs).

Secondly, the *state-occupation probability*, $\Pr(E(\tau) = q)$, which is the the probability to occupy a certain state $q$ at time $\tau$ (@sec-ms-state-occupation).

Throughout the remainder of @sec-multi-state it is assumed that the multi-state process is *Markov*, meaning that the transition intensity at time $\tau$ depends only on the current state $E(\tau-)$ and not on the full state history $\{E(u) : u < \tau\}$.
Under this assumption, conditioning on the current state alone (as in (@eq-ms-transition-hazards)) is sufficient to characterize the local dynamics, and the transition probability $P_{\ell q}(\zeta,\tau)$ is fully determined by the cumulative transition hazards between $\zeta$ and $\tau$.
The non-parametric Aalen-Johansen estimator (@eq-ms-aj-trans-prob) of the transition-probability matrix (@eq-ms-trans-prob-mat) is then consistent.

Relaxing the Markov assumption leads to *semi-Markov* and general *non-Markov* models, where the conditioning set must include past sojourn times or full state history; see @aalen2008survivalevent and @beyersmann.competing.2012 for an overview, which are not discussed in further detail here.
Notably, the marginal *state-occupation* probabilities $\Pr(E(\tau)=q)$ remain consistently estimable from the same Aalen-Johansen methodology, even without the Markov assumption (see @sec-ms-state-occupation).
...
<!--  -->
$$
\sum_{q =0}^Q P_{\ell q} = 1,\forall \ell \in \{0, \ldots,Q\}.
...
![An *illness-death model* where subjects can transition from healthy state ($0$) to death ($2$) directly or via intermediate illness state ($1$)](Figures/survival/illness-death-model.png){#fig-illness-death fig-alt="Graph of the illness-death model." width=60%}
...
Thus, $P_{02}(4,6) = \textcolor{blue}{p_{00}(5)p_{02}(6)} + \textcolor{green}{p_{01}(5)p_{12}(6)} + \textcolor{orange}{p_{02}(5)}$, where $p_{\ell q}(\tau)=\Pr(E(\tau)=q \mid E(\tau -1) = \ell)$ are the probabilities for transitions $\ell \rightarrow q$ between two subsequent discrete time points.
...
Then (@eq-ms-trans-prob-ill-death-discrete) still holds when replacing $p_{\ell q}(j)$ with $p_{\ell q}(t_j)$.
...
First note from equations (@eq-ms-transition-hazards) and (@eq-ms-cumu-hazard) that the instantaneous transition probabilities can be equated to increments of the cumulative hazard, which is the increase in the cumulative hazard within a fixed, infinitesimally small interval $\du$:
$$
H_{\ell q}(u) =  \Pr\big(E(u+\mathrm{d}u)=q \mid E(u-) = \ell\big) =: p_{\ell q}(u).
$$
...
Thus, similar to the competing risks setting, relationship (@eq-ms-prodint-cumu-hazards) implies that knowledge of the transition (cumulative) hazards is sufficient to fully specify the multi-state process.
...
<!--  -->
$$
\mathbf{P}(\zeta, \tau) \approx \prod_{j=1}^{J}(\mathbf{I} + \triangle\mathbf{H}_{\ell q}(t_{j})),
$$ {#eq-ms-prodint-approx}
<!--  -->
where $\triangle \mathbf{H}_{\ell q}(t_{j}) = \mathbf{H}_{\ell q}(t_{j}) - \mathbf{H}_{\ell q}(t_{j-1})$ is the increment of the cumulative hazards between to consecutive time points.


### Non-parametric estimation of transition probabilities {#sec-ms-aalen-johansen}

From (@eq-ms-prodint-approx), it follows that transition probabilities can be estimated by first computing the transition cumulative hazards, $H_{\ell q}(\tau)$.
Similarly to the competing risks setting (@sec-aalen-johansen), one can first define the transition hazards,
<!--  -->
$$
...
$d_{\ell q,t_{(k)}}$ is the number of subjects that made the transition $\ell \rightarrow q$ at time $t_{(k)}$; and $n_{\ell; t_{(k)}}$ is the number of subjects in state $\ell$ immediately before $t_{(k)}$.

The cumulative transition hazards follow as
<!--  -->
$$
H_{NA,\ell q}(\tau) = \sum_{k:t_{(k)}\leq \tau} h^d_{\ell q}(t_{(k)}) = \sum_{k:t_{(k)}\leq \tau}\frac{d_{\ell q,t_{(k)}}}{n_{\ell; t_{(k)}}},
$$
<!--  -->
...
<!--  -->
$$
\mathbf{P}(\zeta, \tau) = \prod_{j=1}^{J}\big(\mathbf{I} + \triangle\mathbf{H}_{NA,\ell q}(t_{(j)})\big).
$$ {#eq-ms-aj-trans-prob}
<!--  -->

### State occupation probabilities {#sec-ms-state-occupation}

The pairwise transition probabilities, $P_{\ell q}(\zeta, \tau)$, defined above are conditioned on a specific entry-state $\ell$ at a specific time $\zeta$; the Aalen-Johansen estimator (@eq-ms-aj-trans-prob) is only consistent for them under the Markov assumption.
A closely related but conceptually distinct target is the *state-occupation probability*,
<!--  -->
$$
P_q(\tau) := \Pr(E(\tau) = q)
...
A practically important property is that the Aalen-Johansen plug-in (@eq-ms-aj-trans-prob), applied with the empirical initial-state distribution rather than conditioned on a fixed $\ell$, remains a consistent estimator of $P_q(\tau)$ even when the underlying process is not Markov [@putter2018; @niessl2023].
By contrast, the AJ estimator of the pairwise $P_{\ell q}(\zeta, \tau)$ loses its consistency guarantees once the Markov assumption fails.
...
<!--  -->
$$
\hat{P}_q(\tau) = \sum_{\ell=0}^{Q} \hat{\pi}_\ell\, \widehat{P}_{\ell q}(0, \tau).
$$ {#eq-ms-state-occupation-est}
<!--  -->
The remainder of this section and the application below report the pairwise transition probabilities under the Markov assumption.
...
The estimands of interest are the four off-diagonal transition probabilities $\hat{P}_{\ell q}(0, \tau)$ for $(\ell, q) \in \{(0,1), (0,2), (1,0), (1,2)\}$, computed separately for the prednisone and placebo arms via the Aalen-Johansen estimator (@eq-ms-aj-trans-prob).
Stratifying by treatment treats each arm as a homogeneous Markov sub-population, with no further covariate adjustment.
This is a coarse but standard simplification; regression-based extensions that incorporate subject-level features into the multi-state estimation are taken up later in the book (for example, the pseudo-value approach in @sec-pseudo-value-trans-prob).

@fig-multi-state-prothr shows these probabilities for both arms.
In contrast to the cumulative incidence functions of the competing-risks setting (@sec-aalen-johansen), the curves targeting *transient* states (such as $\ell \rightarrow 0$ and $\ell \rightarrow 1$) are not monotonically increasing in $\tau$.
This is a feature of the underlying mathematics, the absorbing state mass $P_{\ell 2}(0, \tau)$ grows monotonically with $\tau$ and the row-sum constraint (@eq-ms-trans-prob-row-sums) forces the transient-state probabilities to give up mass to compensate.

For substantive interpretation, the curves for $0 \rightarrow 2$ and $1 \rightarrow 2$ (death) increase monotonically over follow-up in both arms, as expected for absorbing-state probabilities.
Relative to the placebo, between days $1,000$ to $3,000$, the prednisone arm shows a modest reduction in the $0 \rightarrow 2$ transition probability and an increase in the back-transition $1\rightarrow 0$, but the gap narrows towards the end of follow-up.
The overall conclusion is that prednisone does not appear to confer a strong, persistent protective effect at this resolution; meaningful inference about treatment effects would require accompanying uncertainty bands and is beyond the descriptive scope of this stratified Aalen-Johansen analysis.
...
* Non-parametric estimators like the Aalen-Johansen estimator can be used as first step in the analysis of competing risks and multi-state settings.
* The empirical transition matrix provides a general procedure to obtain estimates of transition probabilities based on any learner that can estimate transition hazards.
::::
