...
In the more complex cases, recurrent events are treated as a multi-state process, which we discuss next.
To learn about methods specifically just for modelling recurrent events, we recommend @cookStatisticalAnalysisRecurrent2007 and @erdmann2024comparisonnonparametrica.
...
@tbl-surv-data-sir.adm. shows an excerpt of the `sir.adm` data [@pkgmvna] of patients in an ICU.
...
<!--  -->
$$
H(\tau) = \sum_{e=1}^q H_{e}(\tau) = \int_{0}^\tau h(u)\ du = \sum_{e=1}^{q}\int_{0}^\tau h_{e}(u)\ du
...
<!--  -->
$$
F_{e}(\tau) = P(Y \leq \tau, E = e) =  \int_{0}^\tau f(u-)_e\ \mathrm{d}u = \int_{0}^\tau S(u-)h_{e}(u)\ \mathrm{d}u,\ e\in \{1,\ldots,q\},
$$ {#eq-cif}
<!--  -->
where

* $S(u-)$ is the probability of surviving (not experiencing any of the competing events) until the time-point shortly before $u$
* $S(u-)h_{e}(u)$ is the probability of experiencing event $e$ at time point $u$ (which follows from (@eq-continuous-hazard))
