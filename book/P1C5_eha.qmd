...
This section introduces relevant notation (@sec-cr-notation), the non-parametric Aalen-Johansen estimator\index{Aalen-Johansen estimator} (@sec-aalen-johansen) followed by a worked example.
...
<!-- FIXME (ANDREAS): BELOW WAS USING TAU IN PARENTHESES; CHECK YOU'RE HAPPY WITH MY FIX. I DIDN'T KNOW IF $t_{(k-1)}$ or $t_{(k)}-$ WAS PREFERRED -->
$$
F_{AJ,q}(\tau) = \sum_{k:t_{(k)}\leq \tau} \hat{S}_{KM}(t_{(k-1)})\hat{h}^d_q(t_{(k)})=  \sum_{k:t_{(k)}\leq \tau} \hat{S}_{KM}(t_{(k-1)})\frac{d_{q,t_{(k)}}}{n_{t_{(k)}}}
$$ {#eq-aalen-johansen}
<!--  -->
...
Furthermore, not every pair $(\ell, q)$ corresponds to a realizable transition: the state diagram of the specific process determines which transitions are possible a priori (@fig-multi-state-prothr-states is one example), and the hazards of impossible transitions are identically zero.
...
<!-- TODO (ANDREAS; MINOR): I'M NOT SURE I SEE THE VALUE OF THIS SENTENCE HERE; WHY MENTION STATE-OCCUPATION PROBABILITY AS AN IMPORTANT QUANTITY IF YOU'RE ONLY REFERENCING A DIFFERENT SECTION -->
Secondly, the *state-occupation probability*\index{state-occupation probability}, $\Pr(E(\tau) = q)$, which is the the probability to occupy a certain state $q$ at time $\tau$ (@sec-ms-state-occupation).

<!-- FIXME (ANDREAS): TRANSITION 'INTENSITY' HASN'T BEEN USED ABOVE -->
Throughout the remainder of @sec-multi-state it is assumed that the multi-state process is *Markov*\index{Markov}, meaning that the transition intensity at time $\tau$ depends only on the current state $E(\tau-)$ and not on the full state history $\{E(u) : u < \tau\}$.
...
Relaxing the Markov assumption leads to *semi-Markov*\index{Markov!semi} and general *non-Markov* models, where the conditioning set must include past sojourn times or full state history; see @aalen2008survivalevent and @beyersmann.competing.2012 for an overview, which are not discussed in further detail here.
...
In this example, back-transitions are not possible, therefore the lower triangle of the matrix is filled with zeros and $P_{22}(\zeta, \tau) = 1, \forall\ \zeta < \tau$ by virtue of being an absorbing state.
The matrix of transition probabilities is thus given as
<!--  -->
$$
\mathbf{P}(\zeta, \tau) =
  \begin{pmatrix}
...
For example, the probability to transition from state $1$ to $2$ between time points $\zeta=4$ and $\tau=6$ is given as $P_{12}(4, 6) = p_{11}(5)p_{12}(6) + p_{12}(5)$.
...
<!-- FIXME (ANDREAS): I ADDED MATHRM{d} BELOW AND ADDED APPROX, CAN YOU CONFIRM THIS IS CORRECT? I THINK SO BECAUSE THEN IT'S APPROX THE REGULAR HAZARD WHICH IS CLOSER TO PROBS?-->
$$
...
From (@eq-ms-prodint-approx), it follows that transition probabilities can be estimated by first computing the transition cumulative hazards\index{transition cumulative hazards}, $H_{\ell q}(\tau)$.
Similarly to the AJ approach in the competing risks setting (@sec-aalen-johansen) \index{Aalen-Johansen estimator}, one can first define the transition hazards\index{transition hazard},
<!--  -->
$$
...
The transition cumulative hazards follow as\index{transition cumulative hazard}
<!--  -->
$$
...
and transition probabilities\index{transition probabilities} are obtained via (@eq-ms-prodint-approx) as
<!--  -->
$$
\mathbf{P}(\zeta, \tau) = \prod_{j=1}^{J}\big(\mathbf{I} + \triangle\mathbf{H}_{NA,\ell q}(t_{(j)})\big).
$$ {#eq-ms-aj-trans-prob}
<!--  -->
<!-- TODO (ANDREAS; MINOR) - ABOVE CAN WE WRITE $\triangle\mathbf{H}_{NA,\ell q}(t_{(j)})$ AS $\triangle\mathbf{H}_{NA}(t_{(j)})$ (REMOVING (\ELL Q)) - AND SAME ELSEWHERE WHERE WE HAVE THE MATRIX - IT'S SLIGHTLY INCONSISTENT AT THE MOMENT-->
