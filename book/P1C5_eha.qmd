...
In the simplest single-event setting, there are two states: "no event yet" (which is censoring and encoded by $0$) and "event has occurred" (encoded as $1$).
...
In the single-event case ($Q = 1$), $E(\tau)$ and $Y$ provide the same information: $E(\tau) = 1 \iff Y \leq \tau$.
...
The process-valued formulation may seem over-engineered in the single-event case but is less verbose when there are multiple states and transitions, as we will see in @sec-competing-risks and @sec-multi-state. For a thorough stochastic-process treatment of event-history analysis, see @aalen2008survivalevent.
...
*Competing risks* arise when several mutually exclusive events can lead to observation drop-out.
...
The defining feature of the competing risks framework is that once any one of the $Q$ events occurs, the others can no longer happen.
...
Strictly speaking, prothrombin level is only assessed at scheduled follow-up visits, so the transitions between state $0$ (normal) and state $1$ (abnormal) are *interval-censored*: at visit $k$ we observe the new status, but not the exact time at which the change occurred between visits $k-1$ and $k$.
...
The diagonal case $\ell = q$ appears only at the level of transition probabilities (see @eq-ms-trans-prob-mat), where $P_{\ell\ell}(\zeta, \tau)$ is the probability of occupying state $\ell$ at both $\zeta$ and $\tau$ (which, in processes with back-transitions, also includes paths that leave $\ell$ and return).
Furthermore, not every pair $(\ell, q)$ corresponds to a realisable transition: the state diagram of the specific process (for example, @fig-multi-state-prothr-states for `prothr`) determines which transitions are possible a priori, and the hazards of impossible transitions are identically zero.
...
A natural quantity of interest in the multi-state setting are the transition probabilities $P_{\ell e}(\zeta, \tau):= P(E(\tau) = e \mid E(\zeta) = \ell)$, that is, the probability to be in state $e$ at $\tau$ given occupation of state $\ell$ at $\zeta < \tau$.
...
The marginal *state-occupancy* probabilities $P(E(\tau)=e)$ remain consistently estimable from the same Aalen-Johansen methodology, even without the Markov assumption (see @sec-ms-state-occupancy).
...
### State occupancy probabilities {#sec-ms-state-occupancy}
...
A closely related but conceptually distinct target is the *state-occupancy probability*,
<!--  -->
$$
P_q(\tau) := P(E(\tau) = q) = \sum_{\ell=0}^{Q} P(E(0) = \ell)\, P_{\ell q}(0, \tau),
$$ {#eq-ms-state-occupancy}
<!--  -->
which is the marginal probability that a subject occupies state $q$ at time $\tau$, averaged over the initial-state distribution $P(E(0)=\ell)$.

State-occupancy probabilities are arguably the more natural target for many prediction tasks: questions of the form "what is the probability that the subject is alive in normal-prothrombin state at $\tau = 365$ days?" are directly answered by $P_q(\tau)$, not by a pairwise transition probability conditioned on a fixed entry-state.

A practically important property is that the Aalen-Johansen plug-in (@eq-ms-aj-trans-prob), applied with the empirical initial-state distribution rather than conditioned on a fixed $\ell$, remains a consistent estimator of $P_q(\tau)$ even when the underlying process is not Markov [@aalen2008survivalevent].
...
Concretely, given the AJ estimate $\widehat{\mathbf{P}}(0, \tau)$ and the empirical initial-state distribution $\hat{\pi}_\ell = n_\ell^{(0)}/n$ (where $n_\ell^{(0)}$ is the number of subjects observed in state $\ell$ at $\tau=0$), the state-occupancy estimator is
<!--  -->
$$
\hat{P}_q(\tau) = \sum_{\ell=0}^{Q} \hat{\pi}_\ell\, \widehat{P}_{\ell q}(0, \tau).
$$ {#eq-ms-state-occupancy-est}
<!--  -->
...
Deriving the state-occupancy estimates follows by applying the same AJ pipeline and then marginalizing with (@eq-ms-state-occupancy-est).
