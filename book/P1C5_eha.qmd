...
*Competing risks* arise when several mutually exclusive events can occur on the same subject.
...
This section introduces relevant notation (@sec-cr-notation), the non-parametric Aalen-Johansen estimator (@sec-aalen-johansen) followed by a worked example.
...
These absorbing states are also often referred to as *competing events*, *event types*, or *causes* (as in cause that ended the observation).
...
In extension of (@eq-continuous-hazard-process), define the *cause-specific* hazards as,

$$
h_q(\tau) = \lim_{\dtau \to 0} \frac{\Pr(E(\tau + \dtau) = q\  \mid E(\tau-)=0)}{\dtau}.
$$ {#eq-cause-specific-hazard}

Analogous to the single-event case, the cause-specific cumulative hazard is then defined as,
...
Finally, the probability of experiencing an event $q$ before time $\tau$, which is referred to as the *Cumulative Incidence Function* (CIF), is given by
...
Plugging the Kaplan-Meier estimate (@eq-km) for the all-cause survival probability (the probability that none of the competing events has occurred) together with the cause-specific discrete hazard (@eq-cs-discrete-hazard) into (@eq-cif) yields the Aalen-Johansen (AJ) estimator (@aalenEmpiricalTransitionMatrix1978) for the CIF of cause $q$:
<!--  -->
$$
...
For illustration of the AJ estimator and the interpretation of the CIFs consider the analysis conducted in @beyersmann.competing.2012, based on the data from @tbl-surv-data-siradm.
...
In this case, observations could either experience the event of interest (death) or right-censoring and the data could be modelled using the methods in @sec-surv, for example using the Nelson-Aalen estimator (@eq-surv-na).
...
Strictly speaking, prothrombin level is only assessed at scheduled follow-up visits, so the transitions between state $0$ (normal) and state $1$ (abnormal) are *interval-censored*: at visit $k$ the new status is observed, but not the exact time at which the change occurred between visits $k-1$ and $k$.
...
Accounting for the interval-censoring would require similar NPMLE methods to those described in @sec-surv-estimation-non-param.
...
Transitions between different states are therefore characterized by transition-specific hazards, denoted by $h_{\ell\rightarrow q}(\tau)$ or short $h_{\ell q}(\tau)$, where $\ell$ is the starting state and $q$ the end state.
...
Analogous to the competing risks setting, one can define the transition specific cumulative hazards,
...
From (@eq-ms-prodint-approx), it follows that transition probabilities can be estimated by first computing the transition-specific cumulative hazards, $H_{\ell q}(\tau)$.
Similarly to the competing risks setting (@sec-aalen-johansen), one can first define the transition specific hazards,
<!--  -->
$$
...
* The empirical transition matrix provides a general procedure to obtain estimates of transition probabilities based on any learner that can estimate transition-specific hazards.
::::
