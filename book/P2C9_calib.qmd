...
In more complex settings involving left- or interval-censoring, or truncation, then using graphical plots is simplest for measuring calibration.
In these contexts, predicted survival functions can be compared to the 'true' survival probability estimated with the Kaplan-Meier by using definitions of the Kaplan-Meier estimator that incorporate other censoring and truncation types as required.
