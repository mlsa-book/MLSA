...
D-Calibration [@Andres2018; @Haider2020] extends calibration metrics from classification to test if predictions follow a standard Uniform distribution [@Wilks1990].
...
D-Calibration leverages the fact that the event times $t_i$ are i.i.d. randomly sampled event times from a distribution $T$, which justifies replacing $T$ with $t_i$ in the previous result (@Haider2020 Lemma B.2).
...
The observed number of events is then the number of observations in the corresponding set: $o_g = |\calB_g|$.
