...
In brief, for an observation $i$ one could define the counting process $N_i(\tau) = \II(T_i \leq \tau, \Delta_i = 1)$, which represents the number of events $i$ has experienced at $t$.
...
D-Calibration [@Andres2018; @Haider2020] leverages the fact that the event times, $t_i$, are i.i.d. randomly sampled event times from a distribution $T$, which justifies replacing $T$ with $t_i$ in the previous result [Lemma B.2 @Haider2020].
Hence, a survival model is said to be well-calibrated if the predicted survival probabilities at observed event times follow a standard Uniform distribution: $\hatS_i(t_i) \SU$.
...
Now let $m$ be the total number of observations, then, under the null hypothesis, the expected number of events in each bin is equal: $e_i = m/G$.
...
$$
\calB_g := \{i = 1,\ldots,m : \lceil \hatS_i(t_i) \times G \rceil = g\}
$$

where $i = 1,\ldots,m$ are the indices of the observations, $\hatS_i$ are predicted survival functions, $t_i$ are observed outcome times, and $\lceil \cdot \rceil$ is the ceiling function.
...
$$
D_{\chi^2}(\hat{\mathbf{S}}, \tt) :=  \frac{\sum^B_{i = 1} (o_i - \frac{m}{G})^2}{m/G}
$$
...
Next, @sec-rules will look at scoring rules, which provides a more concrete method to analytically compare the predicted distributions from survival models.
...
Instead of plotting predictions against the Kaplan-Meier (@sec-calib-km), in the competing risks setting one can plot the predicted cumulative incidence function against the 'true' CIF (@sec-aalen-johanson) [@Austin2022; @Wolbers2009].
...
In these contexts, predicted survival functions can be compared to the 'true' survival probability estimated with the Kaplan-Meier (@sec-calib-km) by using non-parametric estimators that can incorporate other censoring and truncation types as required; see @sec-surv-estimation-non-param.
