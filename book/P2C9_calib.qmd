...
Graphical plots can be extended to handle left-truncation by calculating the 'true' survival probability with the Kaplan-Meier, using the left-truncated risk-set definition [@McGough2021], which removes observations from being 'at risk' if their left-truncation time has yet to occur (@eq-risk-lt).
No numerical calibration measures could be found that handle truncation.
